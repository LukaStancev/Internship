...
*----
      ...
      INTEGER, ALLOCATABLE, DIMENSION(:,:) :: BLNAM
...
*        CREATING OR UPDATING DATA IN A BLOCK
     ...
     1      CALL XABORT('MSTR: PUT NOT PERMITTED IN READ-ONLY MODE')
            ...
            NBLOCK=ISTATE(40)
            ALLOCATE(BLNAM(3,NBLOCK+1))
            ALLOCATE(BLTYP(NBLOCK+1))
            ALLOCATE(BLLEN(NBLOCK+1))
            IF (NBLOCK.GT.0) THEN
               CALL LCMGET(IPSTR,'REC-NAMES',BLNAM)
...
*           GO TO REQUESTED DIRECTORY         
            ...
            ALLOCATE(BLLEN(NBLOCK+1))
            IF (NBLOCK.GT.0) THEN
               CALL LCMGET(IPSTR2,'REC-NAMES',BLNAM)
