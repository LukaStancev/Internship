...
*----
      ...
      INTEGER :: IPRINT,IBEG,IEND,IINC,NBLOCK,BLNAM(3,NBLOCK+1),
     1           BLTYP(NBLOCK+1),BLLEN(NBLOCK+1)
      CHARACTER(LEN=12) :: NAME,NAME2  
*----
*  LOCAL VARIABLES
*----
      INTEGER, PARAMETER :: IOUT=6
      INTEGER, PARAMETER :: NSTATE=40
      INTEGER :: ISTATE(NSTATE),KCHAR(3),NARA,ITYP,SIZE,ITYPO,NELEO,
     1           NARA2,II,JJ,SIZE2
...
*     SPECIAL CASE OF STATE-VECTOR MODIFICATION
     ...
     3      ' NOT BE MODIFIED.')
      ...
      ENDIF
      IF (NBLOCK.NE.0) THEN
         READ(NAME2,'(3A4)') (KCHAR(II),II=1,3)
         DO II=1,NBLOCK
            IF((BLNAM(1,II).EQ.KCHAR(1)).AND.
     1         (BLNAM(2,II).EQ.KCHAR(2)).AND.
     2         (BLNAM(3,II).EQ.KCHAR(3))) THEN
               ITYPO=BLTYP(II)
               IF (ITYPO.EQ.0)
...
*----
      IF (.NOT.EXIST) THEN
         ...
         READ(NAME,'(3A4)') (BLNAM(II,NBLOCK+1),II=1,3)
         BLTYP(NBLOCK+1)=ITYP
         BLLEN(NBLOCK+1)=NARA
         CALL LCMPUT(IPSTR2,'STATE-VECTOR',NSTATE,1,ISTATE)
         CALL LCMPUT(IPSTR2,'REC-NAMES',3*(NBLOCK+1),3,BLNAM)
