...
*        RECOVER THE DIRECT FLUX.
         ...
         ELSE
            ...
            IF(LENGT.GT.0) THEN
               IF(LMOD.LE.0) CALL XABORT('OUTDRV: INVALID MODE INDEX.')
               JPFLUX=LCMGID(IPFLUX,'MODE')
               KPFLUX=LCMGIL(JPFLUX,LMOD)
               MPFLUX=LCMGID(KPFLUX,'FLUX')
            ELSE
               CALL LCMLIB(IPFLUX)
               CALL XABORT('OUTDRV: UNABLE TO RECOVER A DIRECT FLUX.')
            ENDIF
         ENDIF
...
*        RECOVER THE ADJOINT FLUX.
         ...
         ELSE
            ...
            ELSE
               CALL LCMLIB(IPFLUX)
               CALL XABORT('OUTDRV: UNABLE TO RECOVER AN ADJOINT FLUX.')
            ENDIF
         ENDIF
      ENDIF
      DO 10 IGR=1,NGRP
