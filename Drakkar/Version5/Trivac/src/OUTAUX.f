*DECK OUTAUX
     ...
     1 NALBP,NZS,MAT,VOL,IDL,EVECT,IHOM,IMPX,MAXALC)
*
*-----------------------------------------------------------------------
*
*Purpose:
* perform homogenization into NZS regions based on averaged fluxes
* contained in EVECT. Create an output extended macrolib containing
* homogenized volumes, integrated fluxes and cross sections.
*
*Copyright:
...
*----
      ...
      REAL, DIMENSION(:,:), ALLOCATABLE :: FLINT,SCAT,CHI,ZUFIS,OUTR,
     1 ALBP
...
*----
      ...
      ALLOCATE(WORK(MAXALC*NGRP),RATE(NZS),FLINT(NZS,NGRP),GAR(NGRP),
     1 SCAT(NZS,NGRP),CHI(NBMIX,NBFIS),ZUFIS(NBMIX,NBFIS),
     2 OUTR(NZS+1,NREAC+3))
...
*----
  ...
  160       CONTINUE
            DO 170 JGR=1,NGRP
            SCAT(IBM,JGR)=SCAT(IBM,JGR)+EVECT(IPFL,JGR)*VOL(K)*GAR(JGR)
  170       CONTINUE
         ENDIF
  180    CONTINUE
         DO 200 IBM=1,NZS
         ...
         IF(SCAT(IBM,JGR).NE.0.0) THEN
            IGMIN=MIN(IGMIN,JGR)
            IGMAX=MAX(IGMAX,JGR)
            SCAT(IBM,JGR)=SCAT(IBM,JGR)/FLINT(IBM,JGR)
         ENDIF
  190    CONTINUE
         IJJ(IBM)=IGMAX
...
*----
      DEALLOCATE(ACCUM)
      DEALLOCATE(WORK,RATE,FLINT,GAR,SCAT,CHI,ZUFIS,OUTR)
