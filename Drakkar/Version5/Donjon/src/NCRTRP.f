...
*----
   ...
   15   IDDIV(IDTMP)=IDDIV(IDTMP)*NID
     ...
     1     //' LIMITS(1).')
          ...
          DO 20 I=1,NID
   20     T1D(I,ID)=T1D(I,ID)/(BURN1-BURN0)
...
*          EQ.(3.3) OF RICHARD CHAMBON'S THESIS.
   ...
   40     T1D(I,ID)=-TERPA(I)
          ...
          DO 50 I=1,NID
   50     T1D(I,ID)=T1D(I,ID)-TERPA(I)*BURN0
          CALL ALTERP(LCUBIC,NID,VREAL,BURN1,.FALSE.,TERPA)
          DENOM=VARVAL*(BURN1-BURN0)
          DO 60 I=1,NID
   60     T1D(I,ID)=(T1D(I,ID)+TERPA(I)*BURN1)/DENOM
          DEALLOCATE(TERPA)
        ELSE
