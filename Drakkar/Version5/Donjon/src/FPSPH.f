...
*         HENTRY(1): modification type(L_OPTIMIZE);
*         HENTRY(2): read-only actual type(L_MACROLIB);
*         HENTRY(3): read-only reference type(L_MACROLIB).
...
*----
     ...
     >    OUTG2)
        ...
        DO 60 IBM=1,NMIX
   60   SPH(IBM)=FLUX2(IBM)/FLUX1(IBM)
        DO 70 IAL=1,NALBP
   ...
   90   SPH(IBM)=SPH(IBM)*REAL(ZNORM1)
        DO 100 IBM=1,NMIX
        IPERT=IPERT+1
  100   VARV(IPERT)=SPH(IBM)
        DO 110 IAL=1,NALBP
        IPERT=IPERT+1
  110   VARV(IPERT)=SPH(NMIX+IAL)
  120   CONTINUE
...
*----
  ...
  130   VARV(IPERT)=VAROLD(IPERT)-P(IPERT)
