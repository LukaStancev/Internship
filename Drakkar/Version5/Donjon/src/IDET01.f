          ...
          DO INY=1,ININY
            ...
            DO INZ=1,ININZ
              ...
              IF(NX*NY*NZ.EQ.0) THEN
                WRITE(HSMG,'(32HIDET01: element not found(1). x=,1p,
     1          e12.4,3H y=,e12.4,3H z=,e12.4)') I,COORD1(INX,I),
     2          COORD2(INY,I),COORD3(INZ,I)
                CALL XABORT(HSMG)
              ENDIF
              IEL=(NZ-1)*NXD*NYD+(NY-1)*NXD+NX
              IF(MAT(IEL).EQ.0) THEN
                WRITE(HSMG,'(32HIDET01: element not found(2). x=,1p,
     1          e12.4,3H y=,e12.4,3H z=,e12.4)') I,COORD1(INX,I),
     2          COORD2(INY,I),COORD3(INZ,I)
                CALL XABORT(HSMG)
              ENDIF
