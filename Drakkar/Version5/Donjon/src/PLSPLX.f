...
*----
      DO 3 I=1,M
         DUMY = 0.0D0
         DO 1 J=1,N
    1       DUMY = DUMY + APLUS(I,J)*BINF(J)
*
         BGAR(I) = B(I)-DUMY
         IF(BGAR(I).GE.0.0) GOTO 3
         DO 2 J=1,N
    2       APLUS(I,J) = -APLUS(I,J)
*
         BGAR(I)   = -BGAR(I)
         B(I)      = -B(I)
         INEGAL(I) = -INEGAL(I)
...
*
    ...
    4    AGAR(1,J) = REAL(MINMAX)*COUT(J)
*
      DO 5 J=1,N
         AGAR(2,J) = 0.0D0
         DO 5 I=1,M
    5       AGAR(2,J) = AGAR(2,J) - APLUS(I,J)
*
      BGAR(MP1) = 0.0
      BGAR(MP2) = 0.0
*
      DO 6 I=1,M
    6    BGAR(MP2) = BGAR(MP2) - BGAR(I)
*----
*  SLACK VARIABLES
*----
      IDIMC = N
      DO 8 I=1,M
         IF(INEGAL(I).EQ.0) GOTO 8
         IDIMC = IDIMC + 1
         GSUP(IDIMC) = 1.0E+25
         DO 7 J=1,M
    7       APLUS(J,IDIMC) = 0.0D0
*
...
*----
      ...
      DELTA=0.0D0
      DO 30 JJ=1,MP2
      IF(IVARS(JJ).EQ.J) GOTO 100
   30 CONTINUE
      DELTA=BASE0(IND,MP1)*AGAR(1,J)+BASE0(IND,MP2)*AGAR(2,J)
      DO 35 I=1,M
   35 DELTA=DELTA+BASE0(IND,I)*APLUS(I,J)
      IF(IMPR.GE.7) WRITE (6,865) J,DELTA
*
      IF(BORNE(J).EQ.GSUP(J)) GOTO 45
      IF(DELTA.GE.0.0) GOTO 100
      IF(K.EQ.0) GOTO 40
   39 IF(DELTA.GE.DELTAM) GOTO 100
   40 K=J
      DELTAM=DELTA
      GOTO 100
   45 IF(DELTA.LE.0.0) GOTO 100
      DELTA=-DELTA
      IF(K.EQ.0) GOTO 40
      IF(K.GT.0) GOTO 39
*
...
*
  ...
  920 CONTINUE
      ...
      DO 925 JJ=1,M
  925 XIJ=XIJ+BASE0(I,JJ)*APLUS(JJ,J)
...
*
  ...
  130 CONTINUE
      ...
      XIJ=BASE0(I,MP1)*AGAR(1,J)+BASE0(I,MP2)*AGAR(2,J)
      DO 135 JJ=1,M
  135 XIJ=XIJ+BASE0(I,JJ)*APLUS(JJ,J)
  ...
  150 CONTINUE
      ...
      DO 155 JJ=1,M
  155 XIKI=XIKI+BASE0(I,JJ)*APLUS(JJ,K)
      XIK(I)=XIKI
*----
*  CASE I: XOBJ(K)=0.0
*----
...
*
  ...
  204 XIK(MP2)=XIK(MP2)+BASE0(MP2,JJ)*APLUS(JJ,K)
*
