...
*         OF RICHARD CHAMBON'S THESIS.
   ...
   30       TERP(I)=-TERPW(I)
            ...
            DO 40 I=1,NBURN
   40       TERP(I)=TERP(I)-TERPW(I)*BURN0
            CALL ALTERP(LCUBIC,NBURN,BURNUP,BURN1,.FALSE.,TERPW)
            DO 50 I=1,NBURN
   50       TERP(I)=(TERP(I)+TERPW(I)*BURN1)/(VARVAL*(BURN1-BURN0))
          ELSE
...
*       DATA STORAGE
        ...
        DO 70 IGR=1,NGRP
        DO 70 IL=1,NL
   70   SCAT(IB,IL,IGR,JGR)=YSCAT(NL*((JGR-1)*NGRP+IGR-1)+IL)
*       JGR IS THE SECONDARY GROUP.
        ...
        ENDDO
        IJJ(IB,IL,JGR)=IGMAX
   80   NJJ(IB,IL,JGR)=IGMAX-IGMIN+1
      ENDIF
  100 CONTINUE
*
