...
*         HENTRY(1): creation type(L_OPTIMIZE).
...
*----
  ...
  110     DERR=DERR+SQRT(WEIGH(I))*DDX*DERIV0(I)
          NORX=NORX**0.5
          DERR=DERR/NORX
...
*----
     ...
     1         //'SUCCESSFUL "PLQ" RESOLUTION')
         ...
         ELSE
            ...
            DO 160 I=1,NVAR
  160       DX(I)=(VARVL2(I)-VARVAL(I))/2.0
            DEALLOCATE(VARVL2)
         ENDIF
         CALL LCMSIX(IPOPT,' ',2)
...
*----
  ...
  350       CQUAD=CQUAD+VARWGT(I)*ODX(I)*ODX(I)
         IF(NORM.NE.0.0) THEN
  ...
  360       CQUAD=MAX(CQUAD,ABS(VARWGT(I)**0.5*ODX(I)))
         IF(NORM.NE.0.0) THEN
            CQUAD=CQUAD/NORM
         ELSE
            CQUAD=0.0
