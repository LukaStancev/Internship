...
*         HENTRY(2): create or modification type(L_OPTIMIZE).
...
*----
      IF(NIFISS.GT.0) THEN
        ...
        IF(IGR.EQ.IGRP) THEN
          DO 50 IFIS=1,NIFISS
   50     PER2(IBMP,IFIS)=GAR2(IBMP,IFIS)/SPH(IBMP,IGR)
        ENDIF
        ...
        IF(IGR.EQ.IGRP) THEN
          DO 60 IFIS=1,NIFISS
   60     PER2(IBMP,IFIS)=GAR2(IBMP,IFIS)/SPH(IBMP,IGR)
        ENDIF
