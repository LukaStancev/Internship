   ...
   70 CONTINUE
      DO 100 IEL=1,NEL
        ...
        DO IV=1,NVTOT
          ...
          ZD=ZD+VTOT(IV)
        ENDDO
   80   BPAX(NEL+1,IEL)=BPAX(NEL+1,IEL)+REAL(ZN/ZD)*1.0E8
  100 CONTINUE
...
*----
     ...
     >     KNADPL(2).EQ.ITNAM(2,JEL)) THEN
        ...
        ENDIF
 110  CONTINUE
      WRITE(HSMG,'(16HSCREIR: ISOTOPE ,2A4,24HIS MISSING AMONG PARTICU,
     > 34HLARIZED ISOTOPES OF THE SAPHYB(1).)') KNADPL(1),KNADPL(2)
      CALL XABORT(HSMG)
*----
*  READ IZEA
*----
...
*----
     ...
     >             KNADPL(2).EQ.ITNAM(2,JREL)) THEN
                ...
                ENDIF
 160          CONTINUE
              WRITE(HSMG,'(16HSCREIR: ISOTOPE ,2A4,16HIS MISSING AMONG,
     >        42H PARTICULARIZED ISOTOPES OF THE SAPHYB(2).)')
     >        KNADPL(1),KNADPL(2)
              CALL XABORT(HSMG)
 ...
 140    CONTINUE
      ELSE IF(TEXT12.EQ.'STABLE') THEN
        DO 141 IREAC=1,MAXR
 141    IF(KPAX(NEL+IREAC,IDEPL).NE.0) KPAX(NEL+IREAC,IDEPL)=-9999
        DO 142 IEL=1,NEL
 142    KPAX(IDEPL,IEL)=0
        CALL REDGET(INDIC,INTG,FLOTT,TEXT12,DBLINP)
*----
