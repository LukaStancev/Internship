...
*----
     ...
     1   16H'' WITH DENSITY =,1P,E13.5,2H .)') HMICRO,DENSIT(ISO)
      ...
      IF(INDXS(1).EQ.1)THEN
        ...
        DO 20 IGR=1,NGRP
   20   TOTAL(IGR)=TOTAL(IGR)+DENSIT(ISO)*WORK1(IGR,1)
      ENDIF
      IF(INDXS(17).EQ.1)THEN
        CALL LCMGET(IPCPO,'STRD',WORK1(1,1))
        DO 21 IGR=1,NGRP
   21   DIFFX(IGR)=DIFFX(IGR)+DENSIT(ISO)*WORK1(IGR,1)
      ELSE IF(INDXS(18).EQ.1)THEN
   ...
   22   DIFFZ(IGR)=DIFFZ(IGR)+DENSIT(ISO)*WORK1(IGR,3)
      ENDIF
      IF(INDXS(3).EQ.1)THEN
...
*----
      IF(LFISS)THEN
        ...
        IF(INDXS(3).EQ.1)THEN
          ...
          DO 50 JGR=1,NGRP
          DO 50 IGR=1,NGRP
   50     SCAT(1,IGR,JGR)=WORK1(IGR,1)*WORK1(IGR,2)*WORK1(JGR,3)
        ELSE
          DO 60 JGR=1,NGRP
          DO 60 IGR=1,NGRP
   60     SCAT(1,IGR,JGR)=0.
        ...
        IF(INDXS(3).EQ.1)THEN
          ...
          DO 70 JGR=1,NGRP
          DO 70 IGR=1,NGRP
   70     SCAT(1,IGR,JGR)=SCAT(1,IGR,JGR)+DENSIT(ISO)*
...
*----
      ...
      IF(INDXS(17).EQ.1)THEN
        DO 170 IGR=1,NGRP
  170   DIFFX(IGR)=1.0/(3.0*DIFFX(IGR))
      ELSE IF(INDXS(18).EQ.1)THEN
        DO 180 IGR=1,NGRP
        DIFFX(IGR)=1.0/(3.0*DIFFX(IGR))
        DIFFY(IGR)=1.0/(3.0*DIFFY(IGR))
  180   DIFFZ(IGR)=1.0/(3.0*DIFFZ(IGR))
      ENDIF
*----
