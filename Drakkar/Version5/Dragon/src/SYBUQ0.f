...
*
*Purpose:
* this subroutine compute the one-group DP-1 leakage and transmission
...
*----
      LGEMPT=.FALSE.
      DO 10 IR=1,NREG
   10 LGEMPT=LGEMPT.OR.(.NOT.LGFULL(IR))
*----
*  INITIALIZATION
*----
      IZ0=0
...
*        COMPUTE LEAKAGE PROBABILITIES.
   ...
   60    PSI(ISW,3,IRS)=PSI(ISW,3,IRS)+SIJ4*COSINU(1,ISW)
*
*        EXTERNAL IS IXI=NH
         IZ0=IZFINI
         PKI3=ZI30*WITRAJ
   ...
   70    PSI(ISE,3,IRS)=PSI(ISE,3,IRS)+SIJ4*COSINU(1,ISE)
*
...
*        COMPUTE LEAKAGE PROBABILITIES.
         PSI(ISW,1,IRS)=PSI(ISW,1,IRS)+SIJ3
         PSI(ISW,2,IRS)=PSI(ISW,2,IRS)+SIJ4*COSINU(2,ISW)
  130    PSI(ISW,3,IRS)=PSI(ISW,3,IRS)+SIJ4*COSINU(1,ISW)
*
*        EXTERNAL IS IXI=NH
         IZ0=IZFINI
         PKI3=ZI30*WITRAJ
         ...
         PSI(ISE,1,IRS)=PSI(ISE,1,IRS)+SIJ3
         PSI(ISE,2,IRS)=PSI(ISE,2,IRS)+SIJ4*COSINU(2,ISE)
  140    PSI(ISE,3,IRS)=PSI(ISE,3,IRS)+SIJ4*COSINU(1,ISE)
*
         IF(TAUX.GE.XLIM5) THEN
            PKI5=0.0
         ELSE
...
*----
  ...
  180 PSI(ISE,3,IRS)=0.25*Z1*(Z2*PSI(ISE,3,IRS)-Z3*DEN0)
      IIJ=0
