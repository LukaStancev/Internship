...
*----
  ...
  10  CONTINUE
      DO 15 IGR=1,NGRO
  15  DELTA(IGR)=LOG(DELTA(IGR)/DELTA(IGR+1))
*
      DO 110 ISO=1,NBISO
      IF(MASKI(ISO)) THEN
...
*                ABOVE.
                 DO 60 IG1=1,MIN(NGRO,NGX)
                 DO 60 IG2=1,NGRO
   60            WORK(IG1)=WORK(IG1)+SCAT(IG1,IG2)*DELTA(IG2)/DELTA(IG1)
                 DO 70 IG1=NGX+1,NGRO
   70            WORK(IG1)=WORK(IG1)+WR2(IG1)
              ELSE
