...
*----
   ...
   30       TOTPT(INOR,JRES)=SIGP(MAXNOR+INOR)
...
*----
         ...
         IF(ILONG.NE.0) THEN
            ...
            IF(GOLD(IRES,IGRP).EQ.-999.) THEN
               ...
               WEIGH(INOR,JRES)=SIGP(INOR)
  180          TOTPT(INOR,JRES)=SIGP(MAXNOR+INOR)
  ...
  190          SIGWS(INOR,JRES)=SIGP(3*MAXNOR+INOR)
            ENDIF
            IF(.NOT.LCM) DEALLOCATE(SIGP)
         ELSE
            WEIGH(1,JRES)=1.0
...
*----
  ...
  230      CONTINUE
        ...
        ENDIF
        DO 240 IND=1,NBNRS
  240   AWPHI(IND)=AWPHI(IND)+PAV(IND,JND)*QQQ*VOL(I)/VOLMER(JND)
  250   CONTINUE
        DO 260 IND=1,NBNRS
  260   MATRIX(INM(IND,INOR,NBNRS),INM(JNBN,JNOR,NBNRS))=AWPHI(IND)
  270   CONTINUE
*
        DO 280 I=1,NBNRS*NORI
  280   MATRIX(I,I)=MATRIX(I,I)+1.0D0
  ...
  290   XFLUX(IND,INOR,1,IRES)=REAL(MATRIX(I1,NBNRS*NORI+1))
...
*----
  ...
  320          WEIGH(INOR,JRES)=SIGP(INOR)
               CALL LCMGET(KPLIB,'SIGQT-SLOW',WSLD(1,JRES))
               CALL LCMGET(KPLIB,'SIGQT-SIGS',SIGWS(1,JRES))
            ELSE
               DO 330 INOR=1,NOR(JRES,IGRP)
               WEIGH(INOR,JRES)=SIGP(INOR)
  330          SIGWS(INOR,JRES)=SIGP(3*MAXNOR+INOR)
            ENDIF
            IF(.NOT.LCM) DEALLOCATE(SIGP)
         ELSE
            WEIGH(1,JRES)=1.0
...
*----
  ...
  370       CONTINUE
         ENDIF
  380    SUN(IOF+KEYFLX(I))=QQQ*WEIGH(INOR,IRES)
*
         IF(IMPX.GT.0) THEN
  ...
  390       T1=T1+WEIGH(INOR,IRES)*XFLUX(IND,INOR,1,IRES)
...
*
  ...
  470   UNGAR(I,IRES,IGRP)=UNGAR(I,IRES,IGRP)+FUN(IOF)
        IASM=IASM+NORI
      ENDIF
  480 CONTINUE
*----
...
*----
     ...
     1     TEXT12
        ...
        DO 520 INOR=1,NORI
        DO 520 IND=1,NBNRS
  520   XFLUX(IND,INOR,IL,IRES)=XFLUX(IND,INOR,1,IRES)
        CALL LCMPUT(KPLI0,'NWT0-PT',NBNRS*MAXNOR*NL,2,XFLUX(1,1,1,IRES))
      ENDIF
  530 CONTINUE
      DEALLOCATE(SUN,FUN,NPSYS)
