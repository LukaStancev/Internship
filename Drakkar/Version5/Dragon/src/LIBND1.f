...
*         transport-corrected total and absorption XS
          CALL XSDISO(7002,5013,IND,GAR1(1,1),IERR)
          CALL XSDISO(7002,5004,IND,GAR1(1,2),IERR)
          IF(NGTHER.GT.0) THEN
            ...
            DO I=1,NBTEM
              ...
              IF(ABS(WW).GT.1.0E-6) THEN
                CALL XSDTHE(7004,5013,-1,I,THERXS(1,1),IERR)
                CALL XSDTHE(7004,5004,-1,I,THERXS(1,2),IERR)
                DO I0=1,NGTHER
                  IOF=NGFR+I0
...
*         fission spectrum, fission XS and nu*fission XS
     ...
     >      //'ectrum does not sum to one')
            ...
            DEALLOCATE(CHI)
*
            CALL XSDISO(7002,5005,IND,GAR1(1,3),IERR)
            CALL XSDISO(7002,5006,IND,GAR1(1,4),IERR)
            IF(NGTHER.GT.0) THEN
              ...
              DO I=1,NBTEM
                WW=TERPT(I)
                IF(ABS(WW).GT.1.0E-6) THEN
                  CALL XSDTHE(7004,5005,-1,I,THERXS(1,1),IERR)
                  CALL XSDTHE(7004,5006,-1,I,THERXS(1,2),IERR)
                  DO I0=1,NGTHER
                    IOF=NGFR+I0
...
*
*         (n,2n) XS
          CALL XSDISO(7001,5007,IND,GAR1(1,7),IERR)
