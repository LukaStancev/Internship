...
* element diffusion approximation (hexagonal geometry). Used in door
* BIVAF of Dragon.
*
*Copyright:
...
*----
   ...
   50    FUNKNO(NUN)=FUNKNO(NUN)+SQRT(REAL(2*I0-1))*FUNKNO(IND1)*
     1   BFR(NUM,2)
      ENDIF
   ...
   70    FUNKNO(NUN)=FUNKNO(NUN)+SQRT(REAL(2*I0-1))*FUNKNO(IND1)*
     1   BFR(NUM,4)
      ENDIF
   ...
   90    FUNKNO(NUN)=FUNKNO(NUN)+SQRT(REAL(2*I0-1))*FUNKNO(IND1)*
     1   BFR(NUM,6)
      ENDIF
  100 CONTINUE
      RETURN
