*DECK DMASOU
      SUBROUTINE DMASOU(IPRINT,IPGPT,IPMAC,IPFLX,NG,NREG,NMIL,NL,
...
* IPGPT   pointer to the DMA data structure.
* IPMAC   pointer to the macrolib structure.
* IPFLX   pointer to the multIG flux.
...
*----
      ...
      TYPE(C_PTR) JPFLX,JPGPT,KPGPT,JPMAC,KPMAC
...
*----
     ...
     1        THEN
          ...
          IF(EPS(IOF).GT.EPSMAX) THEN
            KPGPT=LCMLIL(JPGPT,IOF,NG)
            DO IG=1,NG
              CALL LCMPPL(KPGPT,IG,NUN,2,IKEP(IG))
            ENDDO
          ELSE
            DO IG=1,NG
              CALL LCMDRD(IKEP(IG))
...
*----
      ALLOCATE(SUNK(NUN))
      DO IOF=1,NCST
        CALL LCMLEL(JPGPT,IOF,ILONG,ITYLCM)
        IF(ILONG.NE.0) THEN
          KPGPT=LCMGIL(JPGPT,IOF)
          SUM=0.0D0
          DO IG=1,NG
