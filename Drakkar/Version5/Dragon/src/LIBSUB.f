...
*----
     ...
     1 INAME,IJCEDM,KISONA,KISONR,KTYPE,KNAME,KCOH,KINC,KNTFG,KNIR,KSHIN
...
*----
     ...
     2 IHLIB(2,MAXISO,3),ILLIB(MAXISO),NTFG(MAXISO),NIR(MAXISO))
...
*        BUILD A MICROLIB WITH NDIL+1 ISOTOPES.
     ...
     1   KTYPE(2*(NDIL+1)),KNAME(NDIL+1))
         IF(NTFG(ISOT).GT.0) THEN
            ALLOCATE(KCOH(2*(NDIL+1)),KINC(2*(NDIL+1)),KNTFG(NDIL+1))
         ENDIF
         ...
         ENDIF
         DO 90 I=1,NGRO
   90    KSN((IDIL-1)*NGRO+I)=DILUT(IDIL)
         KTN(IDIL)=TN(ISOT)
  100    CONTINUE
         ALLOCATE(INAME(16*NLIB))
         CALL LCMGET(IPLIB,'ILIBRARYNAME',INAME)
