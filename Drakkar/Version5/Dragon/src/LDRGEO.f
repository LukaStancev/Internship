...
*           INTEGER DATA.
            ...
            IF((NAMT.NE.'MIX').AND.(NAMT.NE.'STATE-VECTOR')) THEN
               DO 40 I=1,ILON1
   40          LDRGEO=LDRGEO.AND.(IDATA1(I).EQ.IDATA2(I))
            ELSE IF(NAMT.EQ.'STATE-VECTOR') THEN
               DO 50 I=1,6
   50          LDRGEO=LDRGEO.AND.(IDATA1(I).EQ.IDATA2(I))
            ENDIF
...
*           SINGLE PRECISION DATA.
            ...
            ZMAX=0.0
            DO 60 I=1,ILON1
   60       ZMAX=MAX(ZMAX,ABS(RDATA1(I)),ABS(RDATA2(I)))
            IF(ZMAX.EQ.0.0) ZMAX=1.0
            DO 70 I=1,ILON1
            EPS=ABS(RDATA1(I)-RDATA2(I))/ZMAX
   70       LDRGEO=LDRGEO.AND.(EPS.LT.1.0E-4)
...
*           CHARACTER*4 DATA.
   ...
   80       LDRGEO=LDRGEO.AND.(CTMP1.EQ.CTMP2)
            DEALLOCATE(IDATA2,IDATA1)
...
*           DOUBLE PRECISION DATA.
            ...
            ZMAX=0.0
            DO 85 I=1,ILON1
   85       ZMAX=MAX(ZMAX,REAL(ABS(DDATA1(I))),REAL(ABS(DDATA2(I))))
            IF(ZMAX.EQ.0.0) ZMAX=1.0
            DO 90 I=1,ILON1
            EPS=ABS(REAL(DDATA1(I)-DDATA2(I)))/ZMAX
   90       LDRGEO=LDRGEO.AND.(EPS.LT.1.0E-4)
