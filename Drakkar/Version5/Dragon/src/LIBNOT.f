...
*
      ...
      DO 10 IG1=1,NGRO
   10 GAS(IG1)=FLUX(IG1,IDIL)-1.0
      CALL LCMPUT(IPLIB,'NWT0',NGRO,2,GAS)
      DO 20 IG1=1,NGRO
   20 GAS(IG1)=TOTAL(IG1,IDIL)*FLUX(IG1,IDIL)-TOTAL(IG1,NDIL+1)
      CALL LCMPUT(IPLIB,'NTOT0',NGRO,2,GAS)
      IF(LSIGF) THEN
         DO 30 IG1=1,NGRO
   30    GAS(IG1)=SIGF(IG1,IDIL)*FLUX(IG1,IDIL)-SIGF(IG1,NDIL+1)
   ...
   50 DO 80 IL=1,INGRO+1
     ...
     1   SIGS(IG1,IL,NDIL+1)
         DO 60 IG2=1,NGRO
   60    GA2(IG1,IG2,IL)=SCAT(IG1,IG2,IL,IDIL)*FLUX(IG2,IDIL)-
     1   SCAT(IG1,IG2,IL,NDIL+1)
      ELSE
         DO 70 IG1=1,NGRO
         GA1(IG1,IL)=0.0
         DO 70 IG2=1,NGRO
   70    GA2(IG1,IG2,IL)=0.0
      ENDIF
   80 CONTINUE
      ...
      IF(LADD(IED)) THEN
         DO 90 IG1=1,NGRO
   90    GAS(IG1)=SADD(IG1,IED,IDIL)*FLUX(IG1,IDIL)-SADD(IG1,IED,NDIL+1)
  ...
  100 CONTINUE
      ...
      DO 110 IG1=1,NGRO
  110 GAS(IG1)=ZDEL(IG1,IDEL,IDIL)*FLUX(IG1,IDIL)-ZDEL(IG1,IDEL,NDIL+1)
