...
*----
      ...
      IF(NSCT.GT.1) THEN
         PL(2,M)=U(M)
         DO 140 L=2,NSCT-1
  140    PL(L+1,M)=((2.0*L-1.0)*U(M)*PL(L,M)-(L-1)*PL(L-1,M))/REAL(L)
      ENDIF
  150 CONTINUE
      PLZ(1)=1.0
      IF(NSCT.GT.1) THEN
         PLZ(2)=-1.0
         DO 160 L=2,NSCT-1
  160    PLZ(L+1)=(-(2.0*L-1.0)*PLZ(L)-(L-1)*PLZ(L-1))/REAL(L)
      ENDIF
*----
*  COMPUTE SURFACES AND VOLUMES.
*----
      DO 200 I=1,LX+1
  200 SURF(I)=4.0*PI*XXX(I)*XXX(I)
      DO 210 I=1,LX
      IDL(I)=(I-1)*NSCT+1
  210 VOL(I)=4.0*PI*(XXX(I+1)**3-XXX(I)**3)/3.0
*----
