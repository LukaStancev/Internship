...
* the finite element technique in a 2-D hexagonal geometry. Used in
* DRAGON.
*
*Copyright:
...
*----
   ...
   40 SOURCE(JND3)=SOURCE(JND3)+FUNKNO(JND3,JGR)*TTTT*GARS
   50 CONTINUE
      RETURN
*----
*  INITIAL ITERATION.
*----
   60 NUM=0
      ...
      PV=0.0
      DO 70 JGR=1,NGCOND
   70 PV=PV+SUNMER(MERG(L),JGR)*FLXMER(MERG(L),JGR)
