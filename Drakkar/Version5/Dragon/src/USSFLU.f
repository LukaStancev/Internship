...
*----
     ...
     2 I0=1,3)
      IF((DENN.NE.0.0).AND.(IBM.NE.0)) THEN
         ...
         DO 40 IGRP=1,NGRP
  40     SIGGAR(IBM,JRES,IGRP,1)=SIGGAR(IBM,JRES,IGRP,1)+DENN*GA1(IGRP)
  ...
  45     CONTINUE
         ...
         ENDIF
         DO 50 IGRP=1,NGRP
  50     SIGGAR(IBM,JRES,IGRP,2)=SIGGAR(IBM,JRES,IGRP,2)+DENN*GA1(IGRP)
      ENDIF
...
*           RECOVER INFINITE DILUTION VALUES.
            ...
            DO 51 IED=1,NED
   51       SAGAR(IND,IRES,IED,IG)=0.0
            DO 55 IDEL=1,NDEL
   55       SDGAR(IND,IRES,IDEL,IG)=0.0
            CALL LCMLEN(KPLIB,'NUSIGF',ILENGT,ITYLCM)
            IF(ILENGT.GT.0) THEN
               CALL LCMGET(KPLIB,'NUSIGF',GAS)
               DO 60 IG=1,NGRP
               DO 60 IND=1,NBNRS
   60          SFGAR(IND,IRES,IG)=GAS(IG)
            ENDIF
            DO 80 IL=1,NL
...
*           JG IS THE SECONDARY GROUP.
            ...
            ENDIF
   70       S0GAR(IND,IRES,IL,JG,IG)=GA2(JG,IG)
   80       CONTINUE
            DO 90 IG=1,NGRP
            DO 90 IND=1,NBNRS
   90       SWGAR(IND,IRES,IG)=SWGAR(IND,IRES,IG)/DELTA(IG)
            DO 110 IED=1,NED
            ...
            IF(ILENGT.GT.0) THEN
               ...
               DO 100 IG=1,NGRP
               DO 100 IND=1,NBNRS
  100          SAGAR(IND,IRES,IED,IG)=GAS(IG)
            ENDIF
  110       CONTINUE
            DO 130 IDEL=1,NDEL
            ...
            IF(ILENGT.GT.0) THEN
               ...
               DO 120 IG=1,NGRP
               DO 120 IND=1,NBNRS
  120          SDGAR(IND,IRES,IDEL,IG)=GAS(IG)
            ENDIF
  130       CONTINUE
*
  ...
  140             ISMAX(IL,IG1)=MAX(ISMAX(IL,IG1),ISM(2,IL))
               ENDIF
  150          NOR(IRES,IG1)=IWRK(IG1)
            CALL LCMSIX(KPLIB,' ',2)
            ...
            IF(LENGT.GT.0) THEN
               ...
               DO 160 IG1=1,NGRP
  160          GOLD(IRES,IG1)=GOLD2(IG1)
               DEALLOCATE(GOLD2)
            ELSE
               DO 170 IG1=1,NGRP
  170          GOLD(IRES,IG1)=1.0
            ENDIF
...
*----
      ...
      LRES=.FALSE.
      DO 280 IRES=1,NIRES
  280 LRES=LRES.OR.(NOR(IRES,IGRP).GT.1)
  290 MASKG(IGRP)=((IGRP.GE.IGRMIN).AND.(IGRP.LE.IGRMAX).AND.LRES)
*----
...
*----
      ...
      IF(MASKG(IGRP)) THEN
         MAXXS=2+NL+NED+NDEL
         DO 350 IL=1,NL
  350    MAXXS=MAXXS+MAX(ISMAX(IL,IGRP)-ISMIN(IL,IGRP)+1,0)
...
*----
      ...
      IF(MASKG(IGRP)) THEN
         LRES=.TRUE.
         DO 370 IRES=1,NIRES
  370    LRES=LRES.AND.((GOLD(IRES,IGRP).EQ.-998.).OR.
