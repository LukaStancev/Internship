...
* AFLINT  adjoint integrated flux.
...
*----
 ...
 330        ZTOT=ZTOT+FFUEL(IREGIO,IFIS)
...
*----
      IF(ILUPS.EQ.1) THEN
        DO 520 IKK=1,NMERGE
        DO 520 IGRC=2,NGCOND
        DO 520 JGRC=1,IGRC-1
        DO 520 IL=1,NL
...
*----
      IF(NALBP.GT.0) THEN
        ...
        IGRFIN=0
        DO 660 IGRC=1,NGCOND
        IGRDEB=IGRFIN+1
        IGRFIN=IGCOND(IGRC)
        DENOM=0.0
        DO 650 IGR=IGRDEB,IGRFIN
        DO 650 IREGIO=1,NREGIO
 650    DENOM=DENOM+FLXINT(IREGIO,IGR,1)
        DO 660 IAL=1,NALBP
        ALBP(IAL,IGRC)=0.0
        DO 660 IGR=IGRDEB,IGRFIN
