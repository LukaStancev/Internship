...
*----
   ...
   10    CONTINUE
      ...
      ELSE IF(LENGT.EQ.0) THEN
         ...
         DO 15 J=1,NGRO
   15    DELTA(J)=LOG(ENER(J)/ENER(J+1))
      ENDIF
...
*----
            ...
            DO 100 IDEL=0,NDEL0
            SIGF(IG1,IDEL)=0.0
  100       CHI(IG1,IDEL)=0.0
            DO 105 ISP=1,NBESP
  105       CHI4G(IG1,ISP)=0.0
            GOLD(IG1)=0.0
            DO 110 IL=1,NL
            SIGS(IG1,IL)=0.0
            DO 110 IG2=1,NGRO
  110       SCAT(IG1,IG2,IL)=0.0
            DO 120 IED=1,NED
  120       SADD(IG1,IED)=0.0
            DO 125 IG=1,3*LBIN
  125       BIN(IG)=0.0
  ...
  180       CONTINUE
            IF(LGOLD) THEN
               DO 190 IG=1,NGRO
  190          GOLD(IG)=GOLD(IG)+TERPM*GOLD2(IG)
            ENDIF
            DO 200 IG=1,3*LBIN
  200       BIN(IG)=BIN(IG)+TERPM*BIN2(IG)
  210       CONTINUE
...
*----
  ...
  250       CONTINUE
            DO 260 ISP=1,NBESP
               LNZERO=.FALSE.
               DO 255 IG=1,NGRO
  255          LNZERO=LNZERO.OR.(CHI4G(IG,ISP).NE.0.0)
               IF(LNZERO) THEN
  ...
  340    CONTINUE
         ...
         IF(LBIN.GT.0) THEN
            ...
            LOGT=.FALSE.
            DO 350 I=1,LBIN
  350       LOGT=LOGT.OR.(BIN(2*LBIN+I).NE.0.0)
            IF(LOGT) THEN
