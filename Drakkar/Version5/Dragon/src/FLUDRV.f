*DECK FLUDRV
      SUBROUTINE FLUDRV(IPRT,IPFLUX,IPTRK,IPMACR,IFTRAK,IPSYS,IPHASE,
     1 ITPIJ,CXDOOR,ITRANC,TITLE,B2,INITFL,LFORW,LEAKSW,IREBAL,NGRP,
     2 NMAT,NIFIS,NANIS,NLF,NLIN,NFUNL,OPTION,NUN,MAXINR,EPSINR,MAXOUT,
     3 EPSUNK,EPSOUT,IFRITR,IACITR,ITYPEC,ILEAK,NREG,NSOUT,MATCOD,
     4 KEYFLX,VOL,REFKEF)
*
*-----------------------------------------------------------------------
*
...
* INITFL  flux initialization flag (=0: uniform flux).
...
*----
      ...
      TYPE(C_PTR) IPFLUX,IPTRK,IPMACR,IPSYS
...
*  LOCAL VARIABLES
*----
      PARAMETER  (IUNOUT=6,NSTATE=40,NDIMO=2,IPFLUP=0,IPGPT=0,IGPT=0)
      TYPE(C_PTR) JPMACR,KPMACR,JPFLUX
...
*----
      ...
      REAL, ALLOCATABLE, DIMENSION(:,:) :: FLUXO,XSTOT,XSTRC,FXSOR,XSTK
...
*----
     ...
     > XSDIA(0:NMAT,0:NANIS,NGRP),FXSOR(0:NMAT,NGRP),
...
*         transport-corrected SIGS1 values.
   ...
   80      CONTINUE
     ...
     >                                 XSCHI(IMAT,1,IGR),IMAT=1,NMAT)
        ...
        IF( ITYPEC.EQ.0 )THEN
           FXSOR(0,IGR)=0.0
           CALL LCMGET(KPMACR,'FIXE',FXSOR(1,IGR))
        ENDIF
        DO 100 IANI=0,NANIS
...
*
      ...
      ELSE
         LREBAL=(IREBAL.EQ.1).AND.(.NOT.LEAKSW)
      ENDIF
      CALL FLU2DR(IPRT,IPMACR,IPFLUX,IPSYS,IPTRK,IPFLUP,IPGPT,IGPT,
...
*----
      DEALLOCATE(VSCAT,XSTK,XSNUF,XSCHI,FXSOR,XSDIA,XSTRC,XSTOT,FLUXO)
      RETURN
*----
*  FORMATS
*----
