...
*           RIEMANN INTEGRATION METHOD WITH NEWTON-RAPHSON ACCELERATION.
            ...
            ENDIF
            DO 20 I=1,NRAT
   20       EAV=EAV+COEF(I,IGRP)*SQRT(DENOM(I,IGRP))
   ...
   30       BS=BS+DELM*SIGS/(SIGA+SIGS+SNI)
            BA=BA/UG
            BS=BS/UG
            ...
            SIGA=MAX(0.0,TBIN(LBIN+IGF)-SBIN(LBIN+IGF))
            SIGS=GC(IGRP,IRS)*MAX(0.0,SBIN(LBIN+IGF))
   40       CBS(I)=CBS(I)+DELM*SIGS/(SIGA+SIGS+EAV)
            CBS(I)=CBS(I)/UG
   ...
   55       CONTINUE
   60       TAUXA=TAUXA+DELM*SIGA*MAX(0.0D0,DBLE(TTT))
   ...
   80       DBS=DBS-DELM*SIGS/(SIGA+SIGS+SNI)**2
            BA=BA/UG
            BS=BS/UG
            DBA=DBA/UG
            DBS=DBS/UG
...
*           ORIGINAL STAMM'LER APPROXIMATION.
            DO 100 I=1,NRAT
  100       EAV=EAV+XCOEF(I,IRS,IGRP)*SQRT(XDENO(I,IRS,IGRP))
  ...
  110 LBIN=LBIN+NFS(IGRP)
      ...
      IF(IMPX.GE.5) THEN
         LD=.FALSE.
         DO 120 IGRP=1,NGRO
  120    LD=LD.OR.NOCONV(MIX(ISO),IGRP)
