*DECK FLU2DR
      SUBROUTINE FLU2DR(IPRT,IPMACR,IPFLUX,IPSYS,IPTRK,IPFLUP,IPGPT,
     ...
     4 KEYFLX,VOL,XSTOT,XSTRC,XSDIA,FXSOR,XSNUF,XSCHI,LREBAL)
*
*-----------------------------------------------------------------------
*
*Purpose:
...
* IPGPT   pointer to the GPT fixed source LCM object (if ITYPEC=1).
...
*         sections.
* FXSOR   fixed sources if ITYPEC=0.
...
*----
      TYPE(C_PTR) IPMACR,IPFLUX,IPSYS,IPTRK,IPFLUP,IPGPT
     ...
     2 ILEAK,MATCOD(NREG),KEYFLX(NREG,NLIN,NFUNL)
      REAL EPSINR,EPSUNK,EPSOUT,VOL(NREG),XSTOT(0:NMAT,NGRP),
     1 XSTRC(0:NMAT,NGRP),XSDIA(0:NMAT,0:NANIS,NGRP),FXSOR(0:NMAT,NGRP),
...
*----
      PARAMETER(NSTATE=40)
      TYPE(C_PTR) IPREB,J1,JPGPT,JPFLUX,JPMACR,KPMACR,JPSYS,IPSTR,JPSTR,
     1 KPSTR,JPFLUP1,JPFLUP2
      INTEGER JPAR(NSTATE)
      CHARACTER CAN(0:9)*2,MESSIN*5,MESSOU*5,HTYPE(0:5)*4
      INTEGER INDD(3)
...
*----
     ...
     1 MATALB
      REAL, ALLOCATABLE, DIMENSION(:) :: DIFF,XSCAT,GAMMA,V,FL
...
*----
      ...
      IF(LFORW) THEN
         IF(ITYPEC.EQ.1) THEN
           J1=LCMGID(IPGPT,'DSOUR')
           JPGPT=LCMGIL(J1,IGPT)
      ...
      IF((ILINIT.GT.NUNKNO).AND.(CXDOOR.EQ.'SN')) THEN
         CALL XABORT('FLU2DR: MORE SPN UNKNOWNS THAN SN UNKNOWNS. '
     1   //'CANNOT GUARANTEE INTEGRITY OF IMPORTED SOLUTION. '
     2   //'CONSIDER INCREASING SN ORDER OR DECREASING SPN ORDER.')
      ENDIF
      IF(LFORW) THEN
      ...
      IF(ITYPEC.EQ.0) THEN
         ...
         DO 10 IR=1,NREG
         IND=KEYFLX(IR,IE,1)
         IF(IND.NE.0) FLUX(IND,IG,4)=FXSOR(MATCOD(IR),IG)
   10    CONTINUE
   15    CONTINUE
      ELSE IF(ITYPEC.EQ.1) THEN
         IF(LFORW) THEN
            CALL LCMGDL(JPGPT,IG,FLUX(1,IG,4))
         ELSE
            CALL LCMGDL(JPGPT,NGRP-IG+1,FLUX(1,IG,4))
         ENDIF
      ENDIF
   20 CONTINUE
*-------
...
*-------
      DO 25 IG=1,NGRP
      IF((CXDOOR.EQ.'SN').AND.(FLUX(1,1,2).NE.1.0)) THEN
         LDSA = JPAR(19)
         IF(LDSA.NE.1) CALL XABORT('FLU2DR: DSA OPTION REQUIRED '
     1      //'TO BUILD SPN NUMBERING. SPN CALCULATION ESTIMATE '
     2      //'INITIALISATION WITHOUT DSA YET TO BE IMPLEMENTED. ')
         CALL SNEST(IPTRK,NREG,NUNKNO,MATCOD,KEYFLX,FLUX(:,IG,2))
      ENDIF
   25 CONTINUE
*----
      ...
      IF(ITYPEC.EQ.0) THEN
        ...
        DO IG=1,NGRP
          DO IE=1,NLIN
            DO IR=1,NREG
              IND=KEYFLX(IR,IE,1)
              IF(IND.GT.0) FLUX(IND,IG,4)=FXSOR(MATCOD(IR),IG)
            ENDDO
          ENDDO
        ENDDO
      ELSE IF(ITYPEC.EQ.1) THEN
