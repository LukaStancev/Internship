...
*        RECOVER PROBABILITY TABLE VALUES FROM PT-PHYS DIRECTORY.
         ...
         DO 10 IL=1,NL
         IPP=IPP+1
   10    SIGSPT(INOR,IRES,IL)=SIGP((IPP-1)*MAXNOR+INOR)
         DO 30 IL=1,NL
         DO 20 JG=1,NGRP
   20    SIG0PT(INOR,IRES,IL,JG)=0.0
         DO 30 JG=ISM(1,IL),ISM(2,IL)
         IPP=IPP+1
   30    SIG0PT(INOR,IRES,IL,JG)=SIGP((IPP-1)*MAXNOR+INOR)
         DO 40 IED=1,NED
         IPP=IPP+1
   40    SIGAPT(INOR,IRES,IED)=SIGP((IPP-1)*MAXNOR+INOR)
         DO 50 IDEL=1,NDEL
   50    SIGDPT(INOR,IRES,IDEL)=0.0
         DO 60 IDEL=1,NDEL0
         IPP=IPP+1
   60    SIGDPT(INOR,IRES,IDEL)=SIGP((IPP-1)*MAXNOR+INOR)
...
*        USE INFINITE DILUTION VALUES.
   ...
   75    CONTINUE
         ...
         DO 80 IED=1,NED
   80    SIGAPT(1,IRES,IED)=SAGAR(IND,IRES,IED)
         DO 90 IDEL=1,NDEL
   90    SIGDPT(1,IRES,IDEL)=SDGAR(IND,IRES,IDEL)
         DO 100 IL=1,NL
         SIGSPT(1,IRES,IL)=SSGAR(IND,IRES,IL)
         DO 100 JG=1,NGRP
  100    SIG0PT(1,IRES,IL,JG)=S0GAR(IND,IRES,IL,JG)
      ENDIF
  110 CONTINUE
*----
...
*----
  ...
  120 CGAR(IOF)=0.0
      ...
      JOF=JOF+1
  130 CGAR(JOF)=CGAR(JOF)+SIG0PT(KINOR,K,IL,JGRP)*WW
  140 IOF=JOF
      IOF=JOF
      WW=XFLUX(I,KINOR,1)*WEIGH(KINOR,K)
      DO 150 IED=1,NED
      IOF=IOF+1
  150 CGAR(IOF)=CGAR(IOF)+SIGAPT(KINOR,K,IED)*WW
      DO 160 IDEL=1,NDEL0
      IOF=IOF+1
  160 CGAR(IOF)=CGAR(IOF)+SIGDPT(KINOR,K,IDEL)*WW
      IOF=IOF+NDEL-NDEL0
      IF((ISUBG.EQ.4).AND.(GOLD(K).EQ.-999.)) THEN
         IOF=IOF+1
...
*
      ...
      DO 180 JGRP=1,NGRP
  180 S0GAR(I,K,IL,JGRP)=0.0
      DO 190 JGRP=ISMIN(IL),ISMAX(IL)
      IOF=IOF+1
  190 S0GAR(I,K,IL,JGRP)=CGAR(IOF)/PHGAR(I,K)
      DO 200 IED=1,NED
      IOF=IOF+1
  200 SAGAR(I,K,IED)=CGAR(IOF)/PHGAR(I,K)
      DO 210 IDEL=1,NDEL0
      IOF=IOF+1
  210 SDGAR(I,K,IDEL)=CGAR(IOF)/PHGAR(I,K)
      IOF=IOF+NDEL-NDEL0
