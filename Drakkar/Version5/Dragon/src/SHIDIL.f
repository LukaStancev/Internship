...
*----
   ...
   40 DB(I)=0.0D0
      ...
      IF((I.NE.N).AND.(COEF(I).NE.(0.0,0.0))) THEN
         ...
         DDC(I0)=DDC(I0-1)
         DO 50 J=I0-1,1,-1
   50    DDC(J)=DDC(J-1)+DDC(J)*DENOM(I)
         DDC(0)=DDC(0)*DENOM(I)
      ENDIF
   60 CONTINUE
      DO 70 I=0,NRAT-1
      DA(I)=DA(I)+DBLE(COEF(N)*DENOM(N)*DDC(I))
   70 DB(I)=DB(I)+DBLE(COEF(N)*DDC(I))
      DO 80 I=0,NORS-1
      DA(I)=DA(I)/DB(NORS-1)
   80 DB(I)=DB(I)/DB(NORS-1)
*
      DO 100 IALP=1,2*NRAT-1
      GAR1=DA(NORS-1)
      DO 90 I=NORS-2,0,-1
   90 GAR1=DA(I)+GAR1*SIGX(IALP)
      SDDK(IALP)=DILUT(IALP)/GAR1
  100 SDDK2(IALP)=SDDK(IALP)*SDDK(IALP)
*----
...
*----
  ...
  110 TOFIT(IALP,2)=DILUTM/SDDK(IALP)
...
*        NORS-TERMS RATIONAL APPROXIMATION.
         ...
         DO 120 I=2,NORS
         SGN=-SGN
  120    C(I-1)=SGN*(DB(I-2)/DIST(K)**(I-2)+DC(I-1))
         C(NORS)=-SGN
         ...
         DO 130 J=NORS-1,1,-1
         SGN=-SGN
  130    DD=DD+SGN*DB(J-1)*SIGXI**(J-1)/DIST(K)**(J-1)
         DO 140 J=1,NORS
         IF(J.NE.I) DD=DD/(SIGXI-CDENOM(J))
  140    CONTINUE
  150    XCOEF(I,K)=CMPLX(DD)
      ELSE
         CALL XABORT('SHIDIL: PADE COLLOCATION FAILURE.')
      ENDIF
  160 CONTINUE
*
  170 DO 180 J=1,NBNRS
      DO 180 I=NORS+1,NRAT
  180 XCOEF(I,J)=(0.0,0.0)
      IF(IMPX.GE.10) THEN
  ...
  230    GAR1=GAR1+VST(K)
         ...
         DO 240 I=1,NRAT
         DD=SIGX(IALP)*DIST(K)+XDENO(I,K)
  240    E1=E1+XCOEF(I,K)*VST(K)/(GAR1*DD)
  250    GAR2=GAR2+PICX(IALP,K)*VST(K)/GAR1
