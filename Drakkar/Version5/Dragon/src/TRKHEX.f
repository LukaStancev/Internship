...
C
C          FUNCTION   :  COMPUTE THE TRACKING INFORMATION RELATED TO A
C                        HEXAGONAL HETEROGENEOUS ASSEMBLY FOR A GIVEN
C                        ANGLE IN A 3D OR 2D GEOMETRY.
C          CREATED    :  28-03-1991
C          REVISED    :  13-10-1993
C          AUTHOR     :  M.OUISLOUMEN
C
C   2-  PROGRAMME PARAMETERS:
C
C INPUT PARAMETERS.
C
C   IPRT       : PRINT OPTION                           
C   NCEL       : NUMBER OF CELLS OF THE ASSEMBLY.
C   FVOL(I)    : FIRST ZONE IN THE CELL I
C   NVOL(I)    : FIRST VOLUME NUMBER IN THE CELL I
C   LVOL(I)    : NUMERO DU 1ER VOLUME DANS LA NUMEROTATION ORIGINALE
C   NCYL(I)    : NUMBER OF CYLINDER IN THE CELL I
C   REMESH     : CONTIENT, PAR ORDRE DE NUMEROTATION CONVENU, LES
C                COORDONNEES DES CENTRES DES DIFFERENTS HEXAGONES (POUR
C                Z C'EST LA HAUTEUR) SUIVIS DES POSITIONS DES CYLINDRES
C                ET LEURS RAYONS DANS L'ORDRE SUIVANT:
C                X(1),X(2),...,Y(1),Y(2),...,Z(1),Z(2),...,X(1),Y(1),
C                Z(1),R(1),...
C   A          : LENGHT OF ONE OF THE HEXAGONE
C   COS1,2,3   : COSINUS DIRECTEURS DE LA TRAJECTOIRE.
C   PAS1       : PAS D'AVANCEMENT DANS LA DIRECTION Y
C   PAS2       : PAS D'AVANCEMENT DANS LA DIRECTION Z
C   STAIRS(I)  : NUMERO MAXIMUM DES CELLULES SUR LE PLAN I
C   FACST(I)   : IDEM MAIS POUR LES SURFACES
C   NDIM       : INDICE DE DIMENSION : =2 => 2D , =3 => 3D
C   IFILE      : NUMERO LOGIQUE DU FICHIER DE TRACKS
C   IANGL      : ANGLE NUMBER
C   NSECT(I)   : 6*(NSECT(I)-1) SECTEURS DANS LE VOLUME I
C   FACB(I)    : PREMIERE SURFACE DE LA FACE SUPERIEURE OU INFERIEURE
C   SECTOR     : =.TRUE. SI NSECT(I).NE.0 QQS I
C   SURB(I)    : DEBUT DE NUMEROTATION DE LA FACE I
C   VOISIN(6,I): LES 6 VOISINS DE LA CELLULE I SUR LE PLAN X-Y
C   NSOUT      : TOTAL NUMBER OF SURFACES IN GLOBAL GEOMETRY
C
C OUTPUT PARAMETERS.
C
C   FICHIER DE TRACKING / IFILE
C   IANGL      : ANGLE NUMBER
C
C SCRATCH STORAGE
C
C   POP     : DIMENSION NCEL*MAXCYL
C   MAXCYL  : MAXIMUM DE CYLINDRE PAR CELLULE
C   MAT     : DIMENSION MAXCYL*NCEL
C   T0      : DIMENSION 2*(MAXCYL+1)
C   TSEC    : DIMENSION 2*(MAXCYL+1)*(1+MAX(NSECT(I)))
C   V0      : DIMENSION 2*(MAXCYL+1)
C   VSEC    : DIMENSION 2*(MAXCYL+1)*(1+MAX(NSECT(I)))
C   RAUX    : DIMENSION MAXCYL
C
C
C                                            .Z
C                                            .
...
C                I   ++++++++   I          O ..............   X
C                I   I      I   I
C                I   I      I   I           SYSTEME DE COORDONNEES
C                I   I      I   I           ----------------------
C                  + I      I +
C                    ++++++++
C
...
C----
     ...
     >                 FTX,FTY
      ...
      CHARACTER        HSMG*131
C
C         FONCTION NECESSAIRE POUR LA ROTATION ET LA RECHERCHE
C         DES SURFACES EXTERNES
C
      IFONC(N,L)= 2+(N-1)*(L+3*(N-2))
      IFCOUR(N)=NINT( (4.+SQRT(1.+4.*FLOAT(N-1)/3.)
     +                 +SQRT(1.+4.*FLOAT(N-2)/3.))*.25)
C
C         TRACKING PAR RAPPORT AU PLAN YOZ. LES DIMENSIONS DU PLAN
C         A TRACKER SONT DETERMINEES EN FONCTION DES ANGLES
C
      DO 767 ITAB=1,5
        SURF1(ITAB)=0
        SURF2(ITAB)=0
 ...
 767  CONTINUE
      ...
      IF(LGDIM) THEN
        ...
        POID1=POIDS
C
C--L'ASSEMBLAGE A TRAITER EST CONFINE DANS UN CYLINDRE DE RAYON=RAYON
C  FERME PAR DEUX CALOTTES SPHERIQUES A CES EXTREMITES
C
        ALP=SCOS1+SCOS3
        ZLA=SCOS1/R2
C
C--EXTEMUMS DE Z0: LIMITES SELON L'AXE Z DU PLAN A TRACKER
C
  ...
  777 CONTINUE
      ...
      IF(LGDIM) THEN
C
C--DOMAINE DE VARIATION DE Y0 POUR Z0 FIXE
C
        CZX1=Z0-ZZ
        CZX2=Z0+ZZ
        CZMIN=MIN(CZX1,CZX2)
  ...
  611   SSQ=SCOS1*(R2-Y0*Y0)+RSCOS2
      ...
      GOTO 42
C
C         INTERSECTION DES PLANS DE L'HEXAGONE AVEC LA DROITE
C
   10 YY=Y-Y0
      TERM1=SQRT3*(X+A)
      TERM2=SQRT3*(X-A)
...
C
C         RECHERCHE DES FACES DE SORTIE
C
   15 IFACE2=9
      IFACE1=9
      MFAC=0
   ...
   20 CONTINUE
C
C CAS OU LA FACE N'A PAS ETE COMPTE
C
      DO 720 IFF=1,LFACE
...
C
C        CAS OU LA DROITE EST TANGENTE A UNE CELLULE PERIPHERIQUE
C
        IF(LGDEB) THEN
          I=ICELL+1
      ...
      ENDIF
C
C--RECHERCHE DU PLUS GRAND PARCOURS
C
      IF(LGFAC) THEN
...
C
C    CAS OU IFACE1 ET IFACE2  FORMENT LE MEME COIN
C
        ...
        ENDIF
C
C  ON PREND LA FACE 7 OU 8 DANS LE CAS D'UN COIN
C
        IF(LGFAC) THEN
        DO 21 IC8=1,MFAC
   ...
   21   CONTINUE
      ...
      MFAC2=0
C
C  STOCKAGE POUR LE TRAITEMENT DES FACES DU COIN
C
      IF(LGFAC)THEN
        DO 301 IFF=1,MFAC
  ...
  301   CONTINUE
      ENDIF
C
C   DETERMINATION DES FACES D'ENTREE OU DE SORTIE
C
      IF(.NOT.LGDEB) THEN
        ICEL0=ICELL
        ISTEP=0
 ...
 556  CONTINUE
      ICELL0=ICELL
C
C  ELEMINATION DES PARCOURS NULS SAUF LE DERNIER.
C
...
C
 ...
 931  CONTINUE
  930 LGDEB=.FALSE.
C
C STOKAGE DES ANCIENS VOLUMES PARCOURUS
C
  ...
  555   CONTINUE
      ...
      ENDIF
C
C        CALCUL DES PARCOURS OPTIQUE. LA CELLULE PEUT CONTENIR DES
C        CYLINDRES D'AXES DIFFERENTS
C
        KPER=KPER+1
        ...
        MAT2(KPER)=ICELL
C
C  COMPTAGE DES CELLULES DU COIN EN PERIPHERIE DANS MAT2
C
      LGOUT1=.FALSE.
      LGOUTU=.FALSE.
        IF(LGFAC) THEN
...
C***
   ...
   22   CONTINUE
        ...
        DO 25 ICYL=1,NCYL(ICELL)
           ...
           RAUX(LAUX)=R*R
C
C--TOUS LES CYLINDRES SONT SUPPOSES D'AXES Z
C
             YYC=Y0-YCYL
...
C
  ...
  233    CONTINUE
         IF(NSECT(IFV+V0(I)).GT.1) THEN
           ...
           LGPASS=.TRUE.
C
C  CAS OU LA TRACK ARRIVE SUR LE CENTRE DE LA CELLULE
C
           IF(V0(I).EQ.1) THEN
...
C
C  2.E-2 CORRESPAND A PEU PRES A 1 DEGRE
C
  ...
  125 CONTINUE
      ...
      JCC0=IFV
C
C  CAS OU IL N'Y A PAS D'INTERSECTION AVEC LES SECTEURS
C
      IF(KMAX.EQ.2) THEN
...
C
C             INTERSECTION DE LA DIRECTION AVEC LES SECTEURS
C
        JSEC=3*(NSECT(JCC)-1)
        JSEC2=JSEC+JSEC
        ANG0=PI/FLOAT(JSEC)
...
C
C LES PARCOURS NULS AU CENTRE DE LA CELLULE NE SONT PAS ELEMINES
C
C          IF(LGDIM) THEN
           IZ0=1
           IF(IFACE1.EQ.7) THEN
  ...
  114      CONTINUE
C
C  CE TESTE EST NECESSAIRE SI TOUTES LES ZONES NE SONT PAS SECTORISEES
C  ON SUPPOSE ICI QU'ON A AFFAIRE A DES CYLINDRES CONCENTRIQUES
C
           IF(NCYL(ICELL).GT.1) THEN
             XAUX=X-COS1*THYP
             YAUX=Y-COS2*THYP-Y0
  ...
  211           XT=COS1*THYP
                YT=COS2*THYP+Y0
C
C TRAITEMENT DU PROBLEME DU CENTRE DE LA CELLULE SECTORISEE
C
                IF(JC.EQ.1) THEN
...
C
  ...
  529 CONTINUE
C
C  CALCUL DES VOLUMES DE SORTIES
C
      LSV=NVOL(ICELL)-1
  ...
  228 CONTINUE
      IFV=NVOL(ICELL)-1
C
C       PARCOURS OPTIQUES ET VOLUMES ASSOCIES
C
      IF(DIRUP) THEN
        I1=1
        I2=KMAX-1
 ...
 329    CONTINUE
      ENDIF
C
C       RECHERCHE DES PROCHAINS VOLUMES A TRACKER
C
      IF(DIRUP) THEN
        START=.FALSE.
        IVOUT=ICELL
...
C
   34 CONTINUE
C
C       RECHERCHE DE LA FACE DE SORTIE
C
   35 CONTINUE
      IMAT0=IMAT
      NFAC=1
...
C
  ...
  300      CONTINUE
   ...
   37 KFACE=IVOUT-IFONC(NCOUR,0)-ISTAIR
      ...
      ENDIF
C
C  PRISE EN COMPTE DES SECTEURS
C
  ...
  371   CONTINUE
      ...
      ENDIF
      ENDIF
C------ DISTRIBUTION DES NEUTRONS SUR DES FACES DES PLANS SUPERIEURS
C       OU INFERIEURS AUTRE QUE LE PLAN CONSIDERE
      IF(LGPG1) THEN
        IF(DIRUP) THEN
          IF(PASSU) THEN
            IF(IVOUT.LE.STAIRS(1)) THEN
...
C
      ...
      ENDIF
C
C       STOCKAGE DES PARCOURS OPTIQUES
C
   38 CONTINUE
...
C
  384 CONTINUE
      ...
      Z0=Z00
C
C       POSSIBILITE DE LA CONTINUITE DE LA TRACK SUR D'AUTRES VOLUMES
C       APRES AVOIR INTERSECTEE UN VOLUME PERIPHERIQUE
C
      LGPER=.FALSE.
      IF(NCEL.GT.1) THEN
      LGPER=.TRUE.
C
C-- RECHERCHE DES CELLULES DANS CORN OU LA TRACK PEUT ABOUTIR
C
      IX=1
      I=CORN(IX)
      LGDEB=.TRUE.
      ...
      ENDIF
C
C       DEPLACEMENT HORIZENTAL PUIS VERTICAL DANS LE PLAN YOZ
C
   40 Y0=Y0+PASY
...
CCC   I=1
  ...
  401 Y00=Y0
      ...
      LGPAS0=.FALSE.
C
C        RECHERCHE DE LA PREMIERE CELLULE INTERSECTEE
C
   42 CONTINUE
   50 CONTINUE
         IF(LGPER) THEN
  ...
  421      CONTINUE
   ...
   64       CONTINUE
C----
C  MODIFIED FOR PRECISION ON LINUX
C----
            YT1=YDROIT-YDR(5)
