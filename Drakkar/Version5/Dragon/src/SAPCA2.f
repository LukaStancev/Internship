...
* NISTS   number of particularized fissile isotopes, fission products
*         and macros.
...
*----
      ...
      IF(ILONG.GT.0) THEN
         ...
         DO 33 IMIL=1,NMIL
   33    SPH(IMIL,IGR)=1.0/WORK1(IMIL)
      ELSE
         DO 34 IMIL=1,NMIL
   34    SPH(IMIL,IGR)=1.0
      ENDIF
...
*----
  ...
  151          WORK2(IGR)=WORK2(IGR)+2.0*WORK1(IGR)
         ...
         ELSE IF(NOMREA(IREA).EQ.'EXCESS') THEN
            ...
            IF(ILONG.GT.0) THEN
               CALL LCMGET(KPTEMP,'N3N',WORK1)
               DO 152 IGR=1,NG
  152          WORK2(IGR)=WORK2(IGR)+2.0*WORK1(IGR)
            ENDIF
         ELSE IF(NOMREA(IREA).EQ.'FISSION') THEN
  ...
  155          WORK2(IGR)=WORK2(IGR)*FLOTT
            ...
            IF(ILONG.GT.0) THEN
               ...
               DO 160 IGR=1,NG
  160          WORK2(IGR)=WORK2(IGR)+WORK1(IGR)*FLOTT
            ENDIF
  ...
  175       WORK2(IGR)=DATA4(IGR,IGR)
         ELSE IF(NOMREA(IREA).EQ.'DIFFUSION') THEN
            ...
            IF((IL.EQ.1).AND.(ILONG.GT.0)) THEN
               CALL LCMGET(KPTEMP,'N2N',WORK1)
               DO 176 IGR=1,NG
  176          WORK2(IGR)=WORK2(IGR)-WORK1(IGR)
            ENDIF
            CALL LCMLEN(KPTEMP,'N3N',ILONG,ITYLCM)
            IF((IL.EQ.1).AND.(ILONG.GT.0)) THEN
               CALL LCMGET(KPTEMP,'N3N',WORK1)
               DO 177 IGR=1,NG
  177          WORK2(IGR)=WORK2(IGR)-2.0*WORK1(IGR)
            ENDIF
            DO 180 IGR=1,NG
  180       RDATAX(ADRX(IREA,IISO,NADRX+1)+(IL-1)*NG+IGR-1)=WORK2(IGR)
            GO TO 240
         ELSE IF(NOMREA(IREA).EQ.'TRANSP-CORR') THEN
            IF((ITRANC.EQ.1).AND.(NL.GE.2)) THEN
  ...
  186       IADR2(IGR+1)=MAX(IADR2(IGR+1),IJJ2(IGR))
  190       CONTINUE
            IADR2(1)=1
            DO 195 IGR=1,NG
  195       IADR2(IGR+1)=IADR2(IGR)+(IADR2(IGR+1)-IFDG2(IGR)+1)
  ...
  200       IDATAP(ADRX(IREA,IISO,NADRX+1)+5+NG+IGR)=IADR2(IGR)
     ...
     1      ITYPRO)
            ...
            JOFS=JOFS+1
  210       RDATAX(ADRX(IREA,IISO,NADRX+1)+JOFS-1)=DATA4(IGR,JGR)*ZIL
            GO TO 240
         ELSE
...
*
         EXIST=.FALSE.
         DO 220 IGR=1,NG
  220    EXIST=EXIST.OR.(WORK2(IGR).NE.0.0)
         IF(EXIST) THEN
            ADRX(IREA,IISO,NADRX+1)=IOR+1
            IOR=IOR+NG
            IF(IOR.GT.MAXRDA) CALL XABORT('SAPCA2: RDATAX OVERFLOW(3).')
            DO 230 IGR=1,NG
  230       RDATAX(ADRX(IREA,IISO,NADRX+1)+IGR-1)=WORK2(IGR)
         ELSE
...
*----
  ...
  260 ADRX(NREA+2,NISO+IMAC,NADRX+1)=0
      DO 340 IREA=1,NREA
      IF(NOMREA(IREA).EQ.'DIFFUSION') THEN
         ...
         DO 270 IGR=1,NG
         JOFS=JOFS+1
  270    RDATAX(ADRX(IREA,NISO+IMAC,NADRX+1)+JOFS-1)=DATA2(IGR,IL)
      ELSE IF(NOMREA(IREA).EQ.'PROFIL') THEN
         DO 290 IMAC=1,NMAC
  ...
  280    IDATAP(ADRX(IREA,NISO+IMAC,NADRX+1)+5+NG+IGR)=IADR(IGR)
  290    IDATAP(ADRX(IREA,NISO+IMAC,NADRX+1)+6+2*NG)=IADR(NG+1)
      ELSE IF(NOMREA(IREA).EQ.'TRANSFERT') THEN
         IF(IOI.EQ.0) CALL XABORT('SAPCA2: MUST FIRST DEFINE PROF.')
         DO 300 IMAC=1,NMAC
         ADRX(IREA,NISO+IMAC,NADRX+1)=IOR+1
         ...
         JOFS=JOFS+1
  300    RDATAX(ADRX(IREA,NISO+IMAC,NADRX+1)+JOFS-1)=DATA3(JGR,IGR,IL)
      ELSE
         EXIST=.FALSE.
         DO 310 IGR=1,NG
  310    EXIST=EXIST.OR.(DATA1(IGR,IREA).NE.0.0)
         DO 330 IMAC=1,NMAC
         IF(EXIST) THEN
            ADRX(IREA,NISO+IMAC,NADRX+1)=IOR+1
            IOR=IOR+NG
            ...
            DO 320 IGR=1,NG
  320       RDATAX(ADRX(IREA,NISO+IMAC,NADRX+1)+IGR-1)=DATA1(IGR,IREA)
          ELSE
...
*----
  ...
  370       IADR2(IGR)=IDATAP(ADRX(IREA-1,IISO,NADRX+1)+5+NG+IGR)
            ...
            JOFS=JOFS+1
  380       RDATAX(IMACR+I-1)=RDATAX(IMACR+I-1)-DENISO(IISO)*
     1                        RDATAX(JMACR+JOFS-1)
         ELSE
            DO 390 IGR=1,IGRTOT
  390       RDATAX(IMACR+IGR-1)=RDATAX(IMACR+IGR-1)-DENISO(IISO)*
...
*----
  440 WRITE(DIRNAM,'(''mili'',I8)') IMIL
      ...
      IF(NISOTS.GT.0) THEN
         ...
         DO 445 ISO=1,NISOTS
  445    DECAYC(ISO)=DECAYC(ISO)*1.0E-8
...
*
      ...
      IF(NPRC.GT.0) THEN
         ...
         DO 450 IPRC=1,NPRC
         DO 450 IGR=1,NG
  450    EXIST=EXIST.OR.(DNUSIG(IGR,IPRC).NE.0.0)
         IF(EXIST) THEN
            NMILNR=NMILNR+1
            IF(LCRON) THEN
  ...
  460       DENOM=DENOM+DNUSIG(IGR,NPRC+1)*FLUX(IGR)
            TGENRS=TGENRS/DENOM
            DO 480 IPRC=1,NPRC
            WORKD(IPRC)=0.0
            DO 470 IGR=1,NG
  470       WORKD(IPRC)=WORKD(IPRC)+DNUSIG(IGR,IPRC)*FLUX(IGR)
  480       WORKD(IPRC)=WORKD(IPRC)/DENOM
