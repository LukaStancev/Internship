...
*
*Parameters: output
* NREG   number of regions in the macro-geometry.
* MAT    index-number of the mixture type assigned to each macro-volume.
* VOL    macro-volumes.
*
*-----------------------------------------------------------------------
*
...
*
         IF((IBIHET.EQ.1).OR.(IBIHET.EQ.2)) THEN
           ...
           CALL LCMPUT(KPSYS,'RRRR',NMILG,3,RRRR)
           CALL LCMPUT(KPSYS,'QKOLD',NG*NSMAX*NMILG,3,QKOLD)
           CALL LCMPUT(KPSYS,'QKDEL',NG*NSMAX*NMILG,3,QKDEL)
           CALL LCMPUT(KPSYS,'PKL',NG*NSMAX*NSMAX*NMILG,3,PKL)
           CALL LCMPUT(KPSYS,'COEF',NMILG*(1+NG*NSMAX)**2,4,COEF)
           DEALLOCATE(NCO)
           DEALLOCATE(COEF)
           DEALLOCATE(PKL,QKOLD,QKDEL,RRRR)
         ELSEIF((IBIHET.EQ.3).OR.(IBIHET.EQ.4)) THEN
           CALL LCMPUT(KPSYS,'P1I',NG*NMILG,3,P1I)
           CALL LCMPUT(KPSYS,'P1DI',NG*NMILG,3,P1DI)
           CALL LCMPUT(KPSYS,'P1KI',NG*NSMAX*NMILG,3,P1KI)
           CALL LCMPUT(KPSYS,'SIGA1',NG*NMILG,3,SIGA1)
