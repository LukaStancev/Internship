...
*              REORDER THE 'FISSIONINDEX' MATRIX.
               ...
               DO 30 IFIS=1,NFISSI
               DO 20 IBM=1,IDATA(2)
   20          INDFIS(IBM,IFIS)=IWRK((IFIS-1)*IDATA(2)+IBM)
               DO 30 IBM=IDATA(2)+1,NBMIX
   30          INDFIS(IBM,IFIS)=0
               DEALLOCATE(IWRK)
            ELSE
   ...
   40          CONTINUE
               IF(LSAME) THEN
                  ...
                  DO 50 IGR=1,NGRO
   50             LSAME=LSAME.AND.(ABS(CHI1(IGR)-CHI2(IGR)).LE.1.0E-3)
     ...
     1                               .AND.(NDEL.GT.0)) THEN
                     ...
                     DO 60 IGR=1,NDEL
   60                LSAME=LSAME.AND.(LAM1(IGR).EQ.LAM2(IGR))
                  ENDIF
   70             CONTINUE
               ENDIF
               NFISSI=NFISSI+1
