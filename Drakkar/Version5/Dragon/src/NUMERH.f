...
90    POURCE(IKK)=0.0
      DO 110 IBLK=1,NMBLK
      IKK=INUM(IBLK)
110   POURCE(IKK)=POURCE(IKK)+GG3(IBLK)
*----
...
140      DDD(IC)=1.0/6.0
      ELSE IF(NCOUR.EQ.8) THEN
         SURFA=1.5*SQRT(3.0)*XX(IKG)*XX(IKG)
         DO 150 IC=1,6
150      DDD(IC)=XX(IKG)*ZZ(IKG)/(2.0*SURFA+6.0*XX(IKG)*ZZ(IKG))
...
*        DP-1 APPROXIMATION IN 2-D.
         ...
         JSURF=(I1-1)*3+JCOUR
         DVX(JSURF)=FR1
320      MIX(JSURF)=JND+JCOUR
330      DVX(JSURF)=ZSIGN*FR1
         DO 350 I1=IJAS,1,-1
         ...
         ALB(JSURF)=FR1
         SUR(JSURF)=FR2
340      IFR(JSURF)=JND+JCOUR
350      ALB(JSURF)=ZSIGN*FR1
         NCOUR=18
