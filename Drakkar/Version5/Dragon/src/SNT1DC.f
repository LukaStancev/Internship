...
*----
      ...
      DO 145 IL=0,NLF-1
      DO 145 IM=0,IL
      IF(MOD(IL+IM,2).EQ.1) GO TO 145
      IOF=IOF+1
      IF(IOF.GT.NSCT) GO TO 146
      DO 140 M=1,M2
  140 PLZ(IOF,M)=PNSH(IL,IM,U(M),-SQRT(1.0-U(M)*U(M)),0.0)
      DO 141 M=1,NPQ
  141 PL(IOF,M)=PNSH(IL,IM,TPQ(M),UPQ(M),VPQ(M))
  145 CONTINUE
*----
*  COMPUTE SURFACES AND VOLUMES.
*----
  146 DO 200 I=1,LX+1
  200 SURF(I)=2.0*PI*XXX(I)
      DO 210 I=1,LX
      IDL(I)=(I-1)*NSCT+1
  210 VOL(I)=PI*(XXX(I+1)*XXX(I+1)-XXX(I)*XXX(I))
*----
