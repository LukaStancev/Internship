...
*----
   ...
   30    READ(NIN)
      ...
      ELSE
         ...
         VECT(1)=1.0E6*E0
         DO 40 I=1,NGRO
   40    VECT(I+1)=1.0E6*E0*EXP(-UUU(I))
...
*----
  ...
  170    CONTINUE
      ...
      ELSE IF(IMMOND.EQ.10) THEN
         DO 180 J=1,NGRO
  180    SIG1(J)=PHI(J,ISEC)
      ELSE IF(IMMOND.EQ.11) THEN
         DO 185 J=1,NGRO
  185    VECT(J)=1.0/(3.0*PHI(J,ISEC))
...
*        DO TEMPERATURE INTERPOLATION FOR THE THERMAL TRANSFER MATRICES.
     ...
     1         (SS11(K),K=1,IX)
         ...
         IF(IT(IC).EQ.0) THEN
            ...
            DO 210 J=1,IX
            KJ=NGRO-J+1
  210       PP(J,K)=PP1(J,K)+(PP1(J,K)-PP(J,K))*XX
            DO 220 J=1,IX
            DO 220 K=1,IX
  220       PRI(IIIC+(J-1)*IX+K-1)=PP(J,K)
         ELSE
*           ANISOTROPES
            DO 240 K=1,IX
            KI=NGRO-K+1
            DO 240 J=1,IX
            KJ=NGRO-J+1
  240       PP1(J,K)=PP1(J,K)+(PP1(J,K)-PP(J,K))*XX
            DO 250 J=1,IX
            DO 250 K=1,IX
  250       PRI(IIIC+(J-1)*IX+K-1)=PP1(J,K)
         ENDIF
         IANIS(NKDEB)=IT(IC)
         ITY(NKDEB)=7
         NEXT(NKDEB)=IX*IX
...
*----
  ...
  300 SIG1(K)=SIG1(K)/SIGS(K,1)
      ...
      VECT(K)=REAL(X2-DDE)
      X2=DDE
  305 VTHER(K)=2.0*VECT(K)/DELTA(IE)
  ...
  310    SSS(J)=SIGS(K,1)
      ...
      RENORM=0.0
      DO 320 J=1,IX
  320 RENORM=RENORM+VTHER(J)*VECT(J)*PP(J,I)
      RENORM=RENORM/(VTHER(I)*VECT(I))
      RENORM=1.0/RENORM
      DO 330 J=1,IX
  330 PP(J,I)=PP(J,I)*SSS(I)*RENORM
      DO 340 J=1,IX
      AUX=VTHER(J)*VTHER(J)
      DO 340 I=1,IX
  340 PP(I,J)=PP(I,J)/AUX*VTHER(I)*VTHER(I)
      I2=IIIC+IX*IX
      IF(I2.GT.MAXTRA) THEN
         WRITE(HSMG,960) 'MAXTRA'
         CALL XABORT(HSMG)
      ...
      DO 350 J=1,IX
      DO 350 K=1,IX
  350 PRI(IIIC+(J-1)*IX+K-1)=PP(J,K)
      IANIS(NKDEB)=0
      ITY(NKDEB)=7
      NEXT(NKDEB)=IX*IX
      NEXU(NKDEB)=IX
...
*----
  ...
  396 CONTINUE
     ...
     1 NLET,VECT,DEL,NKDEB,IANIS,ITY,NEXT,NEXU,NEXV,NEXW,III)
      DO 400 IG1=1,IG2
  400 SCAT(IG2,IG1,IL+1)=VECT(IG2-IG1+1)*DELTA(IG2)/(ZL*DELTA(IG1))
      DO 410 IG1=IG2+1,NGRO
  410 SCAT(IG2,IG1,IL+1)=VECT(IG2+NGRO-IG1+1)*DELTA(IG2)/(ZL*DELTA(IG1))
  420 CONTINUE
*
      IF(IL.EQ.0) THEN
*        PROCESS NEXCESS INFORMATION.
         ...
         SSS(IG1)=-SIGS(IG1,1)
         DO 425 IG2=1,NGRO
  425    SSS(IG1)=SSS(IG1)+SCAT(IG2,IG1,1)
...
*
      IF(IL.GT.0) THEN
         ...
         SIGS(IG1,IL+1)=0.0
         DO 450 IG2=1,NGRO
  450    SIGS(IG1,IL+1)=SIGS(IG1,IL+1)+SCAT(IG2,IG1,IL+1)
      ENDIF
  480 CONTINUE
      DO 490 IG1=1,NGRO
  490 VECT(IG1)=SIGA(IG1)+SIGS(IG1,1)-SSS(IG1)
*----
...
*----
      ...
      DO 510 IG1=1,NGRO
  510 SCAT(IG2,IG1,1)=SCAT(IG2,IG1,1)/SIGS(IG1,1)
*
      ISS=0
      IAS=0
      IFS=0
...
*----
  ...
  560    DT=SQRT(TN(IMT))-SQRT(TETAB(ITET))
         ...
         DO 570 K=1,NSEI
         DO 570 J=1,N6
  570    SEFF(K,J,I)=(SEAUX(K,J)-SEFF(K,J,I))*DT+SEAUX(K,J)
      ENDIF
...
*        COMPUTE THE EFFECTIVE FLUX.
         TMP1=0.0D0
         DO 640 IG2=1,N2-1
  640    TMP1=TMP1+SCAT(L,IG2,1)*PHI(IG2,ISS)*DELTA(IG2)/DELTA(L)
...
*           USE A SLOWING-DOWN BALANCE EQUATION.
  ...
  650       TMP=TMP+SCAT(L,IG2,1)*PHI(IG2,ISS)*DELTA(IG2)/DELTA(L)
...
*----
      IF(ISS.NE.0) THEN
         DO 670 IG1=1,NGRO
         DO 670 IG2=1,NGRO
  670    SCAT(IG2,IG1,1)=SCAT(IG2,IG1,1)*SIGS(IG1,1)
         INGRO=NL-1
         DO 680 IL=NL-1,0,-1
  ...
  685    DO 695 IL=1,NL-1
         IF(ITYPRO(IL+1).GT.0) THEN
            ...
            DO 690 IG1=1,NGRO
  690       SCAT(IG2,IG1,IL+1)=SCAT(IG2,IG1,IL+1)*SIGS(IG1,1)/VECT(IG1)
         ENDIF
  695    CONTINUE
*
