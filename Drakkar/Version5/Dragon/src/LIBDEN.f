*DECK LIBDEN
     ...
     1 IPISO,MIX,DEN,MASK,MASKL,NED,NAMEAD,ITRANC,MAXNFI,LSAME,ITSTMP,
     2 TMPDAY)
*
*-----------------------------------------------------------------------
*
...
*----
     ...
     1 MIX(NBISO),NED,ITRANC,MAXNFI,ITSTMP,NAMEAD(2,NED)
...
*----
     ...
     1 TEXT2*2
      LOGICAL EXIST,MASKK,LOGL,LALL,LWP1,LSTRD,LH,LOVERV,LDIFF,LFISS,
     1 LWT0,LWT1
...
*  ALLOCATABLE ARRAYS
*----
      INTEGER, ALLOCATABLE, DIMENSION(:) :: IPOS,IJJ,NJJ,IWRK
      INTEGER, ALLOCATABLE, DIMENSION(:,:) :: NJJM,IJJM,INDFIS
      REAL, ALLOCATABLE, DIMENSION(:) :: GA1,GA2,SCAT,VOLMIX,NWTMIX,VOLI
...
*----
     ...
     1 INDFIS(NBMIX,MAXNFI),IJJ(NGROUP),NJJ(NGROUP))
      ALLOCATE(GA1(NGROUP+1),GA2(NGROUP*NGROUP),GA3(NDEL,MAXNFI),
     1 GAR(NBMIX,NBLK+1),GAF(NBMIX,NGROUP,NBLK),SCAT(NGROUP*NBMIX),
     2 CHECK(NBMIX,NGROUP,NL),ZNUS(NBMIX*MAXNFI*NESP,NGROUP,0:NDEL),
...
*              REORDER THE 'FISSIONINDEX' MATRIX.
               ...
               DO 30 IFIS=1,NFISSI
               DO 20 IBM=1,NBM0
   20          INDFIS(IBM,IFIS)=IWRK((IFIS-1)*NBM0+IBM)
...
*----
   ...
   60 GAR(IBM,IBLK)=0.0
      DO 70 LLL=1,NGROUP
      DO 70 IBM=1,NBMIX
   70 GAF(IBM,LLL,IBLK)=0.0
      DO 80 ISOT=1,NBISO
   80 LMADE(ISOT)=DEN(ISOT).EQ.0.0
      DO 140 ISOT=1,NBISO
...
*     ORDER TO REDUCE CPU TIME)
   ...
   90 IOF0=IOF0+NJJ(LLL)
      ...
      IF((MASK(IBM).OR.(.NOT.MASKK)).AND.(MIX(ISOT).EQ.IBM)) THEN
         ...
         IOF=IOF+1
  100    GAF(IBM,IG1,IBLK)=GAF(IBM,IG1,IBLK)+GA2(IOF)*DEN(ISOT)*FACT
      ENDIF
  110 CONTINUE
      LMADE(ISOT)=.TRUE.
...
*
  ...
  210    CHECK(IBM,IG1,M)=CHECK(IBM,IG1,M)+SCAT(IPOSDE)
  220    GAR(IBM,1)=SCAT(IPOS(IBM)+IJJM(IBM,IBLK)-LLL)
  ...
  240 CONTINUE
*----
*  PROCESS THE REACTION VECTORS TOTAL, TOTAL-P1, STRD, H-FACTOR, OVERV
*  AND TRANC.
*----
      LWP1=.FALSE.
      LSTRD=.FALSE.
...
*-
         ...
         DO 260 LLL=1,NGROUP
  260    GAF(IBM,LLL,1)=GAF(IBM,LLL,1)+GA1(LLL)*DENISO
         CALL LCMLEN(JPLIB,'NTOT1   '//NORD(IXSPER),ILONG,ITYLCM)
         IF(ILONG.GT.0) THEN
            LWP1=.TRUE.
            CALL LCMGET(JPLIB,'NTOT1   '//NORD(IXSPER),GA1)
            DO 270 LLL=1,NGROUP
  270       GAF(IBM,LLL,3)=GAF(IBM,LLL,3)+GA1(LLL)*DENISO
         ENDIF
         IF(LDIFF) THEN
            ...
            IF(ILONG.GT.0) THEN
               ...
               DO 280 LLL=1,NGROUP
  280          GAF(IBM,LLL,5)=GAF(IBM,LLL,5)+GA1(LLL)*DENISO
            ENDIF
         ENDIF
         ...
         IF(ILONG.GT.0) THEN
            ...
            DO 290 LLL=1,NGROUP
  290       GAF(IBM,LLL,7)=GAF(IBM,LLL,7)+GA1(LLL)*DENISO
         ENDIF
         ...
         IF(ILONG.GT.0) THEN
            ...
            DO 300 LLL=1,NGROUP
  300       GAF(IBM,LLL,9)=GAF(IBM,LLL,9)+GA1(LLL)*DENISO
         ENDIF
         IF(ITRANC.NE.0) THEN
            CALL LCMGET(JPLIB,'TRANC   '//NORD(IXSPER),GA1)
            DO 310 LLL=1,NGROUP
  310       GAF(IBM,LLL,11)=GAF(IBM,LLL,11)+GA1(LLL)*DENISO
         ENDIF
  315    DENISO=DENISO*TIMFCT
*-
  320    CONTINUE
         IF(LOVERV) THEN
            DO 330 LLL=1,NGROUP
...
*----
  ...
  465       CONTINUE
            ...
            DO 466 IFIS=1,NFISS0*NESP
            DO 466 IBM=1,NBM0
            ZNUS((IFIS-1)*NBMIX+IBM,LLL,0)=WRK1(IBM,IFIS)
  466       ZCHI((IFIS-1)*NBMIX+IBM,LLL,0)=WRK2(IBM,IFIS)
            DO 475 IDEL=1,NDEL
            ...
            IF(ILONG.NE.0) THEN
               ...
               DO 470 IFIS=1,NFISS0*NESP
               DO 470 IBM=1,NBM0
               ZNUS((IFIS-1)*NBMIX+IBM,LLL,IDEL)=WRK1(IBM,IFIS)
  470          ZCHI((IFIS-1)*NBMIX+IBM,LLL,IDEL)=WRK2(IBM,IFIS)
            ENDIF
  475       CONTINUE
         ENDIF
  480    CONTINUE
...
*-
              ...
              DO 530 LLL=1,NGROUP
  530         ZNUS(IOF,LLL,0)=ZNUS(IOF,LLL,0)+GA1(LLL)*DENISO
              IF(NDEL.GT.0) THEN
                 ...
                 IF(ILONG.GT.0) THEN
                    ...
                    DO 540 LLL=1,NGROUP
  540               ZNUS(IOF,LLL,IDEL)=ZNUS(IOF,LLL,IDEL)+GA1(LLL)*
     1              DENISO
                 ENDIF
                 ...
                 IF((ILONG.GT.0).AND.(IXSPER.EQ.1)) THEN
                    ...
                    CALL LCMGET(JPLIB,TEXT12,GA1)
                    DO 550 LLL=1,NGROUP
  550               ZCHI(IOF,LLL,IDEL)=GA1(LLL)
                 ENDIF
              ENDIF
              IF(IXSPER.EQ.1) THEN
                 CALL LCMGET(JPLIB,'CHI     '//NORD(IXSPER),GA1)
                 DO 560 LLL=1,NGROUP
  560            ZCHI(IOF,LLL,0)=GA1(LLL)
              ENDIF
  570         DENISO=DENISO*TIMFCT
           ENDIF
  580      CONTINUE
         ELSE
*          NESP>1 MULTIPLE FISSION SPECTRA CASE
           DO 660 ISP=1,NESP
           DO 660 KFIS=1,NFISSI
           DO 660 IBM=1,NBMIX
           IWFIS=INDFIS(IBM,KFIS)
           IF((IWFIS.NE.0).AND.(MASK(IBM).OR.(.NOT.MASKK))) THEN
              IF(LSAME) THEN
...
*-
  ...
  610         ZNUS(IOF,LLL,0)=ZNUS(IOF,LLL,0)+GA1(LLL)*DENISO
              IF((NDEL.GT.0).AND.(ISP.EQ.1)) THEN
                 ...
                 IF(ILONG.GT.0) THEN
                    ...
                    CALL LCMGET(JPLIB,TEXT12,GA1)
                    DO 620 LLL=1,NGROUP
  620               ZNUS(IOF,LLL,IDEL)=ZNUS(IOF,LLL,IDEL)+GA1(LLL)*
     1              DENISO
                 ENDIF
                 WRITE(TEXT12,'(3HCHI,I2.2,3X,A4)') NDEL,NORD(IXSPER)
                 CALL LCMLEN(JPLIB,TEXT12,ILONG,ITYLCM)
  ...
  630               ZCHI(IOF,LLL,IDEL)=GA1(LLL)
              ...
              IF(IXSPER.EQ.1) THEN
                 ...
                 IF(ILONG.EQ.NGROUP) THEN
                    CALL LCMGET(JPLIB,TEXT12,GA1)
                    DO 640 LLL=1,NGROUP
  640               ZCHI(IOF,LLL,0)=GA1(LLL)
                 ENDIF
              ENDIF
  650         DENISO=DENISO*TIMFCT
           ENDIF
  660      CONTINUE
         ENDIF 
*-
...
*-
         ...
         DO 700 LLL=1,NGROUP
  700    GAF(IBM,LLL,1)=GAF(IBM,LLL,1)+GA1(LLL)*DENISO
  710    DENISO=DENISO*TIMFCT
*-
  720    CONTINUE
  730    CONTINUE
      ENDIF
...
*----
      IF(NDEL*NFISSI.GT.0) THEN
         IF(NFISS0.GT.0) THEN
            ...
            ENDIF
         ENDIF
         DO 820 KFIS=NFISS0+1,NFISSI
         DO 820 IDEL=1,NDEL
  820    GA3(IDEL,KFIS)=0.0
         CALL LCMSIX(IPLIB,' ',2)
         DO 830 KFIS=1,NFISSI
         DO 830 IBM=1,NBMIX
         IWFIS=INDFIS(IBM,KFIS)
         IF((IWFIS.NE.0).AND.(MASK(IBM).OR.(.NOT.MASKK))) THEN
            JPLIB=IPISO(IWFIS)
...
*----
  880 DEALLOCATE(LMADE)
      DEALLOCATE(IPGRP)
      DEALLOCATE(ZCHI,ZNUS,CHECK,SCAT,GAF,GAR,GA3,GA2,GA1)
