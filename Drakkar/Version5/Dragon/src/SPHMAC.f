...
*----
      ...
      ENDIF
      DO 45 IBM=1,NMERGE
   45 ZDENUM=ZDENUM+SIGMA(IBM)*FLXMER(IBM,IGR)
      IF(ITRANC.NE.0) THEN
*        TRANSPORT CORRECTION.
         ALLOCATE(SIG1(NMERGE))
         CALL LCMGET(KPMACR,'TRANC',SIG1)
         DO 50 IBM=1,NMERGE
   50    SIGMA(IBM)=SIGMA(IBM)-SIG1(IBM)
         DEALLOCATE(SIG1)
      ENDIF
      IF(ILEAKS.EQ.1) THEN
...
*        TRANSPORT CORRECTION.
         ...
         CALL LCMGET(KPMACR,'TRANC',SIG1)
         DO 120 IBM=1,NMERGE
  120    SIGMA(IBM)=SIGMA(IBM)-SIG1(IBM)
         DEALLOCATE(SIG1)
      ENDIF
...
*----
     ...
     1 ZLEAK(IBM,IGR)
      DO 180 JGR=1,NGCOND
  180 OUTG(IGR)=OUTG(IGR)+SUNMER(IBM,JGR,IGR,1)
*
      DO 200 INL=1,ISCAT
      DO 200 IGR=1,NGCOND
      DO 200 IBM=1,NMERGE
