...
*        TRANSPORT CORRECTION.
         ...
         DO 50 IMAT=1,NBMIX
   50    SIGMA(IMAT)=SIGMA(IMAT)-SIG1(IMAT)
         DEALLOCATE(SIG1)
      ENDIF
      IF((ILEAKS.EQ.2).OR.(ILEAKS.EQ.3)) THEN
...
*        USE WITH THE FUNDAMENTAL CURRENT EQUATION OF THE ECCO MODEL.
  ...
  55     ZDEN=ZDEN+FLUXES(IREG,IGR,1)*VOLUME(IREG)
         ZNUM=ZNUM/ZDEN
      ENDIF
      DO 60 IREG=1,NREGIO
      IBM=MATCOD(IREG)
...
*        TRANSPORT CORRECTION.
         ...
         CALL LCMGET(KPMACR,'TRANC',SIG1)
         DO 120 IMAT=1,NBMIX
  120    SIGMA(IMAT)=SIGMA(IMAT)-SIG1(IMAT)
         DEALLOCATE(SIG1)
      ENDIF
  ...
  140       WORK(IREG,1)=WORK(IREG,1)+SS
         ENDIF
  150    CONTINUE
      ENDIF
*----
...
*----
  ...
  160    SUD=SUD+SIGMA(IBM)*FACTOR*WORK(IREG,2)
