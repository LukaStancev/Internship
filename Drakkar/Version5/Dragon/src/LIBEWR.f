...
*----
     ...
     >             NPZ(LPZ),IWISO(2*MAXISO),IBURN(2,MLDEP)
*----
*  EXTERNAL FUNCTIONS
*----
...
*----
        NFP=0
        DO 101 JBRN=1,NBURN
          IF(IBURN(2,JBRN).EQ.IDECAY.AND.
     >            RBURN(1,JBRN).GE.0.0) THEN
            JSO=LIBWID(NEL,IWISO,IBURN(1,JBRN))
            NDECAY=NDECAY+1
            TOTLAM=TOTLAM+DBLE(RBURN(1,JBRN))
            IF(JSO.GT.0) THEN
              KPAX(JSO,ISO)=KDECAY
              BPAX(JSO,ISO)=RBURN(1,JBRN)
              KPAX(NEL+KCAPTU,JSO)=1
            ENDIF
            KPAX(NEL+KDECAY,ISO)=1
          ELSE IF(IBURN(2,JBRN).EQ.IFISSI.AND.
     >            RBURN(1,JBRN).GE.0.0) THEN
            KPAX(NEL+KFISSP,ISO)=1
            BPAX(NEL+KFISSP,ISO)=RBURN(1,JBRN)*CONVE
          ELSE IF(IBURN(2,JBRN).EQ.ICAPTU) THEN
            JSO=LIBWID(NEL,IWISO,IBURN(1,JBRN))
            IF(JSO.GT.0) THEN
              KPAX(JSO,ISO)=KCAPTU
              BPAX(JSO,ISO)=RBURN(1,JBRN)
          ...
          ELSE IF(IBURN(2,JBRN).EQ.IN2N) THEN
            JSO=LIBWID(NEL,IWISO,IBURN(1,JBRN))
            IF(JSO.GT.0) THEN
              KPAX(JSO,ISO)=KN2N
              BPAX(JSO,ISO)=RBURN(1,JBRN)
            ...
            ENDIF
            KPAX(NEL+KN2N,ISO)=1
          ELSE IF(IBURN(2,JBRN).EQ.IN3N) THEN
            JSO=LIBWID(NEL,IWISO,IBURN(1,JBRN))
            IF(JSO.GT.0) THEN
              KPAX(JSO,ISO)=KN3N
              BPAX(JSO,ISO)=RBURN(1,JBRN)
            ...
            ENDIF
            KPAX(NEL+KN3N,ISO)=1
          ELSE IF(IBURN(2,JBRN).EQ.IFISSP.AND.
     >            RBURN(1,JBRN).GE.0.0) THEN
            JSO=LIBWID(NEL,IWISO,IBURN(1,JBRN))
            IF(JSO.GT.0) THEN
              NFP=NFP+1
