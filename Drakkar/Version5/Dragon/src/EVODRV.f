...
*----
      ...
      IF((LENGTH.GE.2).AND.(LENGTH.LE.6)) THEN
         ...
         IF(ITIXS.EQ.0) THEN
            DO 90 IP=1,2
            DELTAT(1,IP)=1.0
   90       DELTAT(2,IP)=XT(IP)/8.64E-4-TMPREF
         ELSE
            XREF=XT(1)/8.64E-4
            DO 100 IP=1,2
            DELTAT(1,IP)=1.0
  100       DELTAT(2,IP)=XREF-TMPREF
         ENDIF
      ELSE
         DO 110 IP=1,2
         DELTAT(1,IP)=1.0
...
*        COMPUTE THE HEAVY-ELEMENT MASS PER MIXTURE.
         ...
         ENDIF
  140    CONTINUE
  150    ENERG(IBM)=ENERG(IBM)*REAL(AVCON)
...
*----
     ...
     3  SIG(1,1,1,IP),VPHV(1,IP))
        ...
        VPHD=0.0
        DO 190 ICMB=1,NCOMB
  190   VPHD=VPHD+VPHV(MILVO(ICMB),IP)
        VPH(IP)=VPHD
        IF(INR.NE.0) THEN
...
*----
     ...
     3  SIG(1,1,1,IP),VPHV(1,IP))
        ...
        VPHD=0.0
        DO 220 ICMB=1,NCOMB
  220   VPHD=VPHD+VPHV(MILVO(ICMB),IP)
        VPH(IP)=VPHD
        IF(INR.NE.0) THEN
           FNORM=VPH(IP)/REAL(VPHINI)
           IF(INR.EQ.3) CALL LCMPUT(IPDEPL,'FORM-POWER',1,2,PFACT)
  ...
  240        CONTINUE
             ...
             ELSE IF(ITIM.GE.2) THEN
                ...
                IF(LENGT.EQ.NLENGT) THEN
                  ...
                  DO 250 IQ=1,NREAC+1
                  DO 250 IS=1,NVAR+1
  250             SIG(IS,IQ,IBM,2)=SIG(IS,IQ,IBM,1)+
     ...
     1            45HPOLATED MICRO REACTION RATES AT END-OF-STAGE.)')
                ELSE
                  ...
                  DO 260 IQ=1,NREAC+1
                  DO 260 IS=1,NVAR+1
  260             SIG(IS,IQ,IBM,2)=SIG(IS,IQ,IBM,1)
     ...
     2            7HAGE(1).)')
          ...
          ELSE
             ...
             DO 265 IQ=1,NREAC+1
             DO 265 IS=1,NVAR+1
  265        SIG(IS,IQ,IBM,2)=SIG(IS,IQ,IBM,1)
     ...
     1       46HOF-STAGE MICRO REACTION RATES AT END-OF-STAGE.)')
        ...
        VPHD=0.0
        DO 270 ICMB=1,NCOMB
  270   VPHD=VPHD+VPHV(MILVO(ICMB),IP)
        VPH(IP)=VPHD
*
        DO 280 ICMB=1,NCOMB
        IBM=MILVO(ICMB)
