...
*----
     ...
     +            ICHX,IDIM,LC,L4,MAXKN,MKN,ITYLCM
      INTEGER     I,J
      REAL        XM,XP,YM,YP,SXM,SXP,SYM,SYP,DG,LTOT,FACT
      REAL        E(25)
*----
*  ALLOCATABLE ARRAYS
*----
...
*----
      XXX(1)=0.0
      DO 10 I=1,LX
   10 XXX(I+1)=XXX(I)+XX(I)
      YYY(1)=0.0
      DO 20 I=1,LY
   20 YYY(I+1)=YYY(I)+YY((I-1)*LX+1)
      ...
      DO I=1,NP
        ...
        X(I)=(XXX(2)-XM)*FACT
        X(NP*(LX-1)+I)=XXX(LX)+(XP-XXX(LX))*FACT
        DO J=1,LX-2
          X(NP*J+I)=XXX(J+1)+(XXX(J+2)-XXX(J+1))*FACT
        ENDDO
        Y(I)=(YYY(2)-YM)*FACT
        Y(NP*(LY-1)+I)=YYY(LY)+(YP-YYY(LY))*FACT
        DO J=1,LY-2
...
*         Nodal collocation method (MCFD)
     ...
     +    1,3*LY,AXYZ)
        ...
        SXM=0.0
        DO J=1,LY
          DG=(MIN(YP,YYY(J+1))-MAX(YM,YYY(J)))/REAL(NP)
          DO I=1,NP
...
*         Nodal collocation method (MCFD)
     ...
     +    3*LX,1,AXYZ)
        ...
        SYM=0.0
        DO J=1,LX
          DG=(MIN(XP,XXX(J+1))-MAX(XM,XXX(J)))/REAL(NP)
          DO I=1,NP
            SXM=SXM+AXYZ((J-1)*NP+I)*DG
          ENDDO
        ENDDO
