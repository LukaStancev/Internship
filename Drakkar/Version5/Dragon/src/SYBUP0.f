...
*        HEXAGONAL CELL.
         ...
         SUM=0.0
         DO 90 J=0,5
   90    SUM=SUM+PIJS(IOF+J)
         DO 100 J=0,5
  100    PIJS(IOF+J)=SUM
         DO 110 J=6,5+I
  110    PIJS(IOF+J)=6.0*PIJS(IOF+J)
  120    IOF=IOF+6+I
      ENDIF
*----
*  FIRST APPLY THE ORTHONORMALIZATION FACTOR
*----
      DO 125 I=1,(NSURF+NREG)*(NSURF+NREG+1)/2
  125 PIJS(I)=PIJS(I)*ZZR(IOFR)*ZZR(IOFR)
*----
...
*----
      DO 130 I=1,NSURF
      DO 130 J=1,NSURF
  130 PSS(I,J)=PIJS(INDPOS(I,J))/SURF(I)
      DO 140 I=1,NSURF
      DO 140 J=1,NREG
  140 PIS(I,J)=PIJS(INDPOS(I,NSURF+J))/VOL(J)/SIGT(J)
      DO 150 I=1,NREG
      DO 150 J=1,NREG
  150 PIJ(I,J)=PIJS(INDPOS(NSURF+I,NSURF+J))/(VOL(I)*SIGT(I)*SIGT(J))
      DEALLOCATE(PIJS)
*----
