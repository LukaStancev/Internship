...
* element technique in a 2-D Cartesian geometry. Used in DRAGON.
*
*Copyright:
...
*----
      ...
      IND1=KN(NUM1+1)+I0-1
   30 SOURCE(IND1)=SOURCE(IND1)+FUNKNO(IND1,JGR)*VOL(K)*GARS
   40 NUM1=NUM1+5
   50 CONTINUE
   70 CONTINUE
      RETURN
...
*----
   80 NUM1=0
      ...
      PV=0.0
      DO 90 JGR=1,NGCOND
   90 PV=PV+SUNMER(MERG(L),JGR)*FLXMER(MERG(L),JGR)
