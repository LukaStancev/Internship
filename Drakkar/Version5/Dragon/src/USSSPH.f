...
* S0GAR   microscopic transfer scattering x-s (isotope,secondary,
*         primary).
* SAGAR   microscopic self-shielded additional x-s.
* SDGAR   microscopic self-shielded delayed nu-sigf x-s.
* SWGAR   microscopic secondary slowing-down cross sections (ISUBG=4).
*
*Parameters: output
* PHGAR   SPH-corrected averaged fluxes.
* SFGAR   SPH-corrected microscopic self-shielded fission x-s.
* SSGAR   SPH-corrected microscopic self-shielded scattering x-s.
* S0GAR   SPH-corrected microscopic transfer scattering x-s
*         (isotope,secondary,primary).
* SAGAR   SPH-corrected microscopic additional x-s.
* SDGAR   SPH-corrected microscopic self-shielded delayed nu-sigf x-s.
* SWGAR   SPH-corrected microscopic secondary slowing-down cross
*         sections (ISUBG=4).
* SPH     SPH factors.
*
*-----------------------------------------------------------------------
...
*----
  ...
  100   CONTINUE
        DO 110 IND=1,NBNRS
  110   FLNEW(IND)=FLNEW(IND)/VOLMER(IND)
*----
*  SPH FACTOR CONTROL.
*----
        DO 120 IND=1,NBNRS
  ...
  170   CONTINUE
        DO 180 IED=1,NED
  180   SAGAR(IND,IRES,IED,IGRP)=SAGAR(IND,IRES,IED,IGRP)*SPHNEW
        DO 190 IDEL=1,NDEL
  190   SDGAR(IND,IRES,IDEL,IGRP)=SDGAR(IND,IRES,IDEL,IGRP)*SPHNEW
*
        IF(IMPX.GT.1) THEN
           IF(LTIT) THEN
