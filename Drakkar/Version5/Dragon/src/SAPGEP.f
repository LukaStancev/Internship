...
*
*Purpose:
* recover remaining global parameters and local values. Update the
...
*        RECOVER LOCAL VARIABLES FROM THE MICROLIB IN EDIT OBJECT.
         ...
         DO 60 IBM=1,NMIL
         CALL COMBIB(IPEDIT,0,PARTYP(IPAR),IBM,NAMLCM,TEXT8,MAXNBI,
     1   VALPAR)
         IF(PARTYP(IPAR).EQ.'TEMP') VALPAR=VALPAR-273.16
   60    RVALO((IBM-1)*NVLC+LOCADR(IPAR))=VALPAR
