...
*----
      DO 70 IDIL=1,NDIL
      DO 40 IPAR=1,NPAR+2
   40 SEFF(IPAR,IDIL)=0.0
      DO 50 IOR=1,INOR
      ASTPD=SIGERD(IDIL)*PROSIC(IOR,1,INOR)/(PROSIC(IOR,2,INOR)+
     1 SIGERD(IDIL))
      SEFF(1,IDIL)=SEFF(1,IDIL)+ASTPD
      DO 50 IPAR=2,NPAR+2
   50 SEFF(IPAR,IDIL)=SEFF(IPAR,IDIL)+ASTPD*PROSIC(IOR,IPAR,INOR)
      DO 60 IPAR=2,NPAR+2
   60 SEFF(IPAR,IDIL)=SEFF(IPAR,IDIL)/SEFF(1,IDIL)
   70 CONTINUE
*----
...
*----
      ...
      DO 80 IDIL=1,NDIL
      DO 80 I=2,NPAR+2
   80 ERROR=MAX(ERROR,ABS(SEFF(I,IDIL)-SEFFER(I,IDIL))/
     1 ABS(SEFFER(I,NDIL)))
...
*----
  100 DO 110 IPAR=1,2+NPAR
      DO 110 I=1,NOR
  110 PROSIG(I,IPAR)=PROSIC(I,IPAR,NOR)
*----
