...
C-------------------------- LHXUNH -------------------------------------
C
C                FUNCTION : LECTURE ET ANALYSE DE LA GEOMETRIE SUR LCM
C                REVISED  : JUNE 13-1991
C                AUTHOR   : M. OUISLOUMEN
C
C  INPUT PARAMETERS
C
C     GEONAM  :  NOM DE LA GEOMETRIE SUR LCM
C
C  OUTPUT PARAMETERS
C
C      MESH   : DIMENSION DU VECTEUR REMESH (VOIR ROUTINE TRKHEX)
C      NCELA  : NOMBRE DE CELLULES DANS L'ASSEMBLAGE APRES DUPLICATION
C      IPLANZ : NOMBRE DE PLANS SUIVANT L'AXE Z
C      NCPHY  : NOMBRE DE CELLULES PHYSIQUE
C      ICODE  : CONTIENT LES INDEX DES ALBEDOS PHYSIQUES
C      ZCODE  : CONTIENT LES ALBEDOS GEOMETRIQUES
C      NREGIO : NOMBRE DE REGION PHYSIQUE
C      MVOSU  : DIMENSION DE VOLSUR(VOIR MESHST)
C      MCODE  : =1 SI NCODE(5)=5 ,=2 SI NCODE(6)=5 ,=0 SI NON
C
C------------------------ LHXUNH ---------------------------------------
C
      USE GANLIB
...
C
      ...
      IPLANI=0
C
C         LECTURE DES DIFFERENTS BLOCS SUR LCM
C
      DO 10 I=1,NLCM
         CALL LCMLEN(IPGEOM,LCMNM(I),LNLCM(I),ITP)
   10 CONTINUE
C
C        ELEMINATES OPTIONS NOT CHECKED BY THIS ROUTINE
C
      DO 20 I=1,NIXS
...
C
C         ELEMINATES THE INVALID OPTIONS
C
      DO 30 IST=1, NIST
...
C
   ...
   35 CONTINUE
C
C       ELEMINATIONS DES OPTIONS IMCOMPATIBLES
C
      DO 40 I=1,6
...
C
   ...
   38       CONTINUE
      ...
      ENDIF
C
C        TYPE DE LA SYMETRIE CONSIDEREE
C
     ...
     +   //'SYMETRIE')
      IF(NDIM.EQ.2) THEN
C
C           TRAITEMENT DE LA GEOMETRIE EN 2D
C
        NCELA=NCEL
        NCELAP=NCEL
C
C             DUPLICATION DE LA SYMETRIE CONSIDEREE
C
        IF(IHEX.LT.9) THEN
...
C
   ...
   47      CONTINUE
C
C            CAS OU SIDE EST ENTREE UNE SEULE FOIS
C
          LGSIDE=.TRUE.
...
C
  ...
  187       CONTINUE
          ...
          ENDIF
C
C             RESTORAGE DES DONNEES
C
          CALL LCMSIX(IPTRK,'DATA_DUP',1)
C
C--- CE VECTEUR EST UTILE DANS LA ROUTINE MESHST
...
C
  ...
  255         CONTINUE
   ...
   70      CONTINUE
           DEALLOCATE(ICELL)
C
C                 COMPATIBILITE DES DONNEES DES CELLULES D'UN MEME PLAN
C
           LZZ=LZ
           IAUX=0
           LISP=0
...
C
         IF(IHEX.LT.9) THEN
C
C             DUPLICATION DE LA GEOMETRIE CONSIDEREE
C
           ALLOCATE(IBB(ISTOR(IHEX)*NCEL))
...
C
   ...
   95      CONTINUE
         ...
         IF(NCODE(5) .EQ. 5 .OR. NCODE(5) .EQ. 10) THEN
C
C             DUPLICATION DE LA SYMETRIE SELON L'AXE Z-
C
...
C
  ...
  135      CONTINUE
         ...
         ELSEIF(NCODE(6) .EQ. 5 .OR. NCODE(6) .EQ. 10) THEN
C
C           DULPLICATION DE LA GEOMETRIE SELON Z+
C
           IF(NCODE(6) .EQ. 5) THEN 
...
C
  ...
  210      CONTINUE
         ...
         DEALLOCATE(IGG,SURL)
C
C              MODIFICATION DU VECTEUR STATE
C
         ISTATE(5)=LZZ
         IPLANZ=LZZ
      ENDIF
