...
*----
     ...
     1   '. L_LIBRARY EXPECTED.')
      ...
      CALL LCMSIX(IPLIB,'DEPL-CHAIN',1)
         CALL LCMGET(IPLIB,'STATE-VECTOR',IPAR)
         IF(IPAR(1).NE.NDEPL) CALL XABORT('EVO: INVALID NUMBER OF D'
     1   //'EPLETING ISOTOPES.')
         NSUPS=IPAR(7)
         NREAC=IPAR(8)
      CALL LCMSIX(IPLIB,' ',2)
...
*           RECOVER MIXTURE FLUXES FROM L_FLUX
            ...
            DO 30 I=1,NREG
            KEYFLX=IDL(I)
   30       FLUXE((IGR-1)*NREG+I)=FUNKN(KEYFLX)
            DEALLOCATE(FUNKN)
            DO 50 IBM=1,NBMIX
            DO 50 IGR=1,NGRO
            FLXMIX=0.0
...
*        RECOVER MIXTURE VOLUMES AND FLUXES FROM L_POWER
     ...
     1   //'GROUPS IN L_POWER AND MICROLIB.')
         ...
         VX(NTOT)=VMAP(ICH,JB)
         DO 65 IGR=1,NGRO
   65    FLMIX((NTOT-1)*NGRO+IGR)=FMAP(ICH,JB,IGR)
   70    CONTINUE
...
*----
      IF(INR.EQ.4) THEN
         ...
         IF(MIXPWR(IBM).EQ.1) THEN
            VTOT=VTOT+VX(IBM)
            DO 75 IGR=1,NGRO
   75       VPH=VPH+VX(IBM)*FLMIX(NGRO*(IBM-1)+IGR)
         ENDIF
   80    CONTINUE
         FIT=VPH/VTOT
         INR=1
