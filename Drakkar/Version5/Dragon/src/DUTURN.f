...
C-------------------------- DUTURN -------------------------------------
C
C         FUNCTION : DONNE L'ORIENTATION DES CELLULES D'UN ASSEMBLAGE
C                    A PARTIR DE L'ORIENTATION DE LA SYMETRIE IHEX
C         REVISED  : JUNE, 12 - 1991
C         AUTHOR   : M. OUISLOUMEN
C
C INPUT PARAMETERS
C
C   IHEX   :    TYPE DE SYMETRIE S30,SA60,SB60,...              I
C   NCEL   :    NOMBRE DE CELLULES DANS LA SYMETRIE IHEX        I
C   NCELA  :    NOMBRE DE CELLULES DANS L'ASSEMBLAGE            I
C   CELL(I):    NUMERO DE LA CELLULE I DANS L'ASSEMBLAGE        I(NCELA)
C   TURN(*):    ORIENTATION DES CELLULES DANS LA SYMETRIE IHEX  I(NCEL)
C
C OUTPUT PARAMETERS
C
C   TURND(*) :  ORIENTATION DES CELLULES D'ASSEMBLAGE           I(NCELA)
C
C---------------------------- DUTURN -----------------------------------
C
...
C
   20 CONTINUE
C
C       DUPLICATION DE L'ORIENTATION DANS LA SYMETRIE S30
C
      TURND(1)=TURN(1)
C
C       DUPLICATION DE LA 2EME COURONE
C
      ITURN=ITAB(2)
      TURND(2)=TURN(2)
      DO 25 I=3,7
   ...
   25 CONTINUE
C
C      DUPLICATON DES AUTRES COURONES
C
      JCEL=3
      DO 30 IC=3,NCOUR
...
C
   40 CONTINUE
C
C        DUPLICATION DE L'ORIENTATION DES GEOMETRIES SA60 ET SB60
C
      TURND(1)=TURN(1)
      JCEL=2
      LGSA6=IHEX.EQ.2
...
C
   60 CONTINUE
C
C             DUPLICATION DE L'ORIENTATION DE LA GEOMETRIE S90
C
      TURND(1)=TURN(1)
      JCEL=2
      DO 75 IC=2,NCOUR
...
C
   80 CONTINUE
C
C        DUPLICATION DE L'ORIENTATION DE LA SYMETRIE R120
C
      TURND(1)=TURN(1)
      JCEL=2
      DO 95 IC=2,NCOUR
...
C
  100 CONTINUE
C
C         DUPLICATION DE L'ORIENTATION DES SYMETRIES R180,SA180 ET SB180
C
      TURND(1)=TURN(1)
      LGR8=.FALSE.
      LGSA=.FALSE.
