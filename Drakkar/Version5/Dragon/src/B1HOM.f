...
*        TIBERE-TYPE ANISOTROPIC STREAMING.
         ...
         DO 30 IGRO=1,NGRO
           ...
           IOF=IOF+1
   10      FL2(IOF)=FLUX(IUNK,IGRO)
           DO 20 IUNK=1,NUNKNO/4
           IOF=IOF+1
           CURN=0.0D0
           DO 15 IDIR=1,3
   15      CURN=CURN+B2T(IDIR)*FLUX(NUNKNO/4*IDIR+IUNK,IGRO)
   20      FL2(IOF)=REAL(CURN)
   30    CONTINUE
...
*----
  ...
  100     CONTINUE
     ...
     1    REAL(CURN)
  110     FLUX(KEYFLX(L)+3*NUNKNO/4,I)=FLUX(KEYFLX(L)+3*NUNKNO/4,I)*
     1    REAL(CURN)
  120   CONTINUE
      ENDIF
*
