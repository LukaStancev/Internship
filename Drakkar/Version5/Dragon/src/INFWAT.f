...
C--------------------------------------------------
C     THIS ROUTINE WILL COMPUTE WATER DENSITY FOR CANDU FUELS
C
C     INPUT:  TEMPC = TEMPERATURE (CELSIUS)
C             PURWGT= D2O PURITY  (IN WGT%)
C
C     OUTPUT: DENSTY= DENSITY     (G/CM**3)
C
C                                     R. ROY (09/02/95)
C
C--------------------------------------------------
      IMPLICIT NONE
C
C     TEMPERATURE DEPENDENT DENSITY CALCULATION IS DONE ACCORDING TO:
C
C         DENSITY = D(H2O) * D(D2O) /
C                   ( WGT%(H2O) * D(D2O) + WGT%(D2O) * D(H2O) )
C
...
C------------------------------------------------------------------
C
C >>> TABLES ORIGINALLY CAME FROM ROUTINE dmats.f (WIMS)
C
C
C * D2O DATA CONSISTENT WITH AECL 7531, TABLE FROM J. PHYS. CHEM. REF.
C      DATA, VOL11, NO.1, 1982, P6 (SAME AUTHORS)
C
...
C------------------------------------------------------------------
C
C     COMPUTE DENSITIES FOR LIGHT AND HEAVY WATER (PURE)
C      AT SUCH TEMPERATURE:
C
C      1. USE  DIRECT INTERPOLATION FOR LIGHT WATER:
C
          DENH2O =     AIKINT(TEMPC,TMPH2O,TDNH2O,85,1.0E-5)
C
C      2. INVERSE THE INTERPOLATION FOR HEAVY WATER:
C
...
C
      ...
      WGTH2O = 1.00 - WGTD2O
C
C     COMPUTE GLOBAL DENSITY A MIX OF LIGHT AND HEAVY WATER:
C
