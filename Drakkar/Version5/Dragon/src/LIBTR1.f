...
*     MASS RATIO OF EACH MATERIAL/ISOTOPE IN THE CALCULATION DOMAIN:
  ...
  307 SIGZ(ISUBM)=A(LMC+MULT+6*(ISUBM-1)+2)
...
*----
      ...
      IF(.NOT.LSUBM1) THEN
         LTERP=.TRUE.
         DO 324 IK=1,NGRO
  324    LTERP=LTERP.AND.(TERP(NGRO*(ISUBM-1)+IK).EQ.0.0)
         IF(LTERP) GO TO 600
      ENDIF
*----
...
*----
      DO 520 JJ=(J-1)*INC+1,MIN(J*INC,NOUTG)
      NP=IA(LN+JJ)
...
*----
  530 IF(LSUBM1.AND.(HTYPE.EQ.'NSCAT')) THEN
         ...
         L5=0
         DO 550 JJ=(J-1)*INC+1,MIN(J*INC,NOUTG)
         NP=IA(LN+JJ)
         IF(NP.EQ.0) GO TO 550
         DO 540 IP=1,NP
         JJP=IA(LG+JJ)-IP+1
  540    SAVE(JJP)=SAVE(JJP)+XS(L5+IP)
  550    L5=L5+NP*LORD
      ENDIF
  560 DEALLOCATE(XS)
  570 CONTINUE
...
*----
     ...
     1      VECT(I)
         ELSE
            FLUX(I,IM)=1.0
         ENDIF
  740    TOTAL(I,IM)=TOTAL(I,IM)*VECT(I)
         IF(IMPX.GT.5) THEN
...
*           FISSION SOURCE NORMALIZATION
  ...
  790       SIGF(JJ,IM)=SIGF(JJ,IM)*VECT(JJ)
