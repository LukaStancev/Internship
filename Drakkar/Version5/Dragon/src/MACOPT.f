...
*----
   ...
   20       GAR(2*NMIX+IBM)=REAL(VARV(IPERT))
   ...
   50       GAR(2*NMIX+IBM)=GAR(2*NMIX+IBM)*FACT
            CALL LCMPUT(KPMAC,'DIFFX',NMIX,2,GAR)
            CALL LCMPUT(KPMAC,'DIFFY',NMIX,2,GAR(NMIX+1))
            CALL LCMPUT(KPMAC,'DIFFZ',NMIX,2,GAR(2*NMIX+1))
...
*----
   ...
   75   ALB(IAL,IGR)=(1.0-2.0*FACT)/(1.0+2.0*FACT)
...
*----
         ...
         DO 80 IBM=IBM1,IBM2
         IPERT=IPERT+1
   80    SPH(IBM,IGR)=REAL(VARV(IPERT))
         DO 90 IAL=1,NALBP
         IPERT=IPERT+1
   90    SPH(NMIX+IAL,IGR)=REAL(VARV(IPERT))
