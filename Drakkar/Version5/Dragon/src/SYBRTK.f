...
*        EQUAL WEIGHT INTEGRATION POINTS.
   ...
   15    WXJ(I)=ZXJ(I)/REAL(NX)
         DO 20 I=1,NA2
         ZA(I)=(2.0*REAL(I)-1.0)/REAL(NA2)-1.0
   20    WA(I)=2.0/REAL(NA2)
      ENDIF
      IZ(1)=5
      IZ(2)=NREG+1
      IZ(3)=2
...
*        FLURIG CHANGE OF VARIABLE.
   ...
   30 C=CORDE
      IF(KMAX.NE.1) THEN
         ...
         SUM=SUM+DEL
         Z(LR+KMAX+I)=DEL
   40    VAP(K+I)=VAP(K+I)+DEL*S
      ENDIF
      LR=LR+2*KMAX-1
      DEL=X/CO-SUM
      Z(LR)=DEL
   50 VAP(NREG)=VAP(NREG)+DEL*S
      DO 60 K=KMAX-1,1-KMAX,-1
   60 IZ(LI+K+KMAX)=3+ABS(K)+1+NREG-KMAX
      LI=LI+2*KMAX
      IZ(L5)=2*KMAX-1
      IZ(L5+1)=NX
      IZ(L5+2)=2 ! ISURF
...
*----
   ...
   80    Z(L3+KMAX+I)=Z(L3+KMAX+I)*VAP(K0+I)
         Z(L3+1)=Z(L3+1)*VEX1
...
*        FLURIG CHANGE OF VARIABLE.
  ...
  130 C=CORDE
      IF(KMAX.NE.1) THEN
         ...
         SUM=SUM+DEL
         Z(LR+KMAX+I)=DEL
  140    VAP(K+I)=VAP(K+I)+DEL*S
      ENDIF
      LR=LR+2*KMAX-1
      DEL=B/SI-SUM
      Z(LR)=DEL
  150 VAP(NREG)=VAP(NREG)+DEL*S
      DO 160 K=KMAX-1,1-KMAX,-1
  160 IZ(LI+K+KMAX)=3+ABS(K)+1+NREG-KMAX
      LI=LI+2*KMAX
      IZ(L5)=2*KMAX-1
      IZ(L5+1)=NX
      IZ(L5+2)=2 ! ISURF
...
*----
  ...
  200    VAP(I)=SUM
         ...
         DO 210 I=1,KMAX-2
         Z(L3+KMAX-I)=Z(L3+KMAX-I)*VAP(K0+I)
  210    Z(L3+KMAX+I)=Z(L3+KMAX+I)*VAP(K0+I)
         Z(L3+1)=Z(L3+1)*VEX1
         Z(L3+2*KMAX-1)=Z(L3+2*KMAX-1)*VEX2
  220    L3=L3+2*KMAX
      ENDIF
...
*        FLURIG CHANGE OF VARIABLE.
  ...
  250 C=CORDE
      IF(KMAX.NE.1) THEN
         ...
         SUM=SUM+DEL
         Z(LR+KMAX+I)=DEL
  260    VAP(K+I)=VAP(K+I)+DEL*S
      ENDIF
      LR=LR+2*KMAX-1
      DEL=A/SI-SUM
      Z(LR)=DEL
  270 VAP(NREG)=VAP(NREG)+DEL*S
      DO 280 K=KMAX-1,1-KMAX,-1
  280 IZ(LI+K+KMAX)=3+ABS(K)+1+NREG-KMAX
      LI=LI+2*KMAX
      IZ(L5)=2*KMAX-1
      IZ(L5+1)=NX
      IZ(L5+2)=0 ! ISURF
...
*----
  ...
  310    VAP(I)=SUM
         ...
         DO 320 I=1,KMAX-2
         Z(L3+KMAX-I)=Z(L3+KMAX-I)*VAP(K0+I)
  320    Z(L3+KMAX+I)=Z(L3+KMAX+I)*VAP(K0+I)
         Z(L3+1)=Z(L3+1)*VEX1
         Z(L3+2*KMAX-1)=Z(L3+2*KMAX-1)*VEX2
  330    L3=L3+2*KMAX
      ENDIF
