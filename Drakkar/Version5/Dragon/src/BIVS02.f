...
* (Cartesian geometry). Used in door BIVAF of Dragon.
*
*Copyright:
...
*----
  ...
  110    SG=-SG
      ...
      IF(BFR(NUM2+2).GT.0.0) THEN
         DO 120 I0=1,IELEM
         IND1=KN(NUM1+1)+I0-1
  120    FUNKNO(NUN)=FUNKNO(NUN)+SQRT(REAL(2*I0-1))*FUNKNO(IND1)*
     1   BFR(NUM2+2)
      ENDIF
  ...
  130    SG=-SG
      ...
      IF(BFR(NUM2+4).GT.0.0) THEN
         DO 140 I0=1,IELEM
         IND1=KN(NUM1+1)+(I0-1)*IELEM
  140    FUNKNO(NUN)=FUNKNO(NUN)+SQRT(REAL(2*I0-1))*FUNKNO(IND1)*
     1   BFR(NUM2+4)
      ENDIF
  150 NUM1=NUM1+5
      NUM2=NUM2+4
