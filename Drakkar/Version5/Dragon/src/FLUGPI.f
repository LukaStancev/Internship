...
* COPTIO  type of leakage coefficients; can be 'LKRD' (recover leakage
...
*         =16,26,36,46,56,66: heterogeneous method with pijk and
*         fixed (b->16) or search (x->26 y->36 z->46 r->56 all->66).
 ...
 112      CONTINUE
     ...
     >  //'R VARIABLE EXPECTED')
        ...
        ELSE IF(CARLIR.EQ.'ON') THEN
          INITFL=0
        ELSE
          CALL XABORT('FLUGPI: OFF/ON KEYWORD EXPECTED')
        ENDIF
