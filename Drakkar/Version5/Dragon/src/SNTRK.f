...
*----
     ...
     1 MAXIT,ICL1,ICL2,NSTART,NSDSA
      REAL EPSI
      LOGICAL LFIXUP,LDSA,LBIHET,LIVO
*----
*  LOCAL VARIABLES
*----
      PARAMETER(NSTATE=40)
...
*----
     ...
     1   19HNFORMATION FOLLOWS:)')
         ...
         IF(ITYPE.EQ.7) THEN
            ...
            CALL TRITRK(MAXPTS,IPTRK,IPGEOM,IMPX,IELEM,ICOL,ICHX,ISEG,
     1      IMPV,NLFSA,NVD,ISPN,ISCSA,NADI)
         ELSE
            CALL BIVTRK(MAXPTS,IPTRK,IPGEOM,IMPX,IELEM,ICOL,NLFSA,NVD,
     1      ISPN,ISCSA)
         ENDIF
         CALL LCMSIX(IPTRK,' ',2)
      ENDIF
