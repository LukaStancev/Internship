...
*         surface).
* TITR    title.
* START   beginning-of-iteration flag (START=.true. if SHISN2 is called
*         for the first time).
...
*----
      ...
      IF(LSHI(ISO).GT.0) THEN
         ...
         IF((LENGT.EQ.NGRO).AND.(.NOT.START).AND.LGC) THEN
            ...
            DO 10 LLL=1,NGRO
   10       SIGOLD(LLL,IBM)=(1.0-GAR(LLL))*SIGOLD(LLL,IBM)
         ELSE
...
*
      ...
      LOGDO=.FALSE.
      DO 60 I=1,NBNRS
   60 LOGDO=LOGDO.OR.NOCONV(IRNBM(I),LLL)
      IF(LOGDO) THEN
         NPSYS(LLL)=LLL
*
...
*        CROSS SECTIONS IN EACH RESONANT MIXTURE.
         ...
         ENDIF
   70    CONTINUE
   80    SIGT(IRNBM(I),LLL)=SIGT(IRNBM(I),LLL)-SIGRES(I)
         DO 90 IBM=1,NBM
         SIG0(IBM,LLL)=0.0
         SIG1(IBM,LLL)=0.0
...
*        USE A VERY CHEAP APPROXIMATION TO START ITERATIONS.
  ...
  130    DILUT(IALP,LLL)=DILAV(LLL)
      ELSE
         DO 160 IALP=1,NALPHA
         DO 150 LLL=1,NGRO
         IF(NPSYS(LLL).NE.0) THEN
            DO 140 I=1,NBNRS
  140       SIG1(IRNBM(I),LLL)=FACT(IALP)*SIGE(INRS,LLL)
         ENDIF
  150    CONTINUE
         ALLOCATE(FUN(NUN*NGRO))
...
*----
  ...
  170    SIGX(IALP)=FACT(IALP)*SIGE(INRS,LLL)
         IMPX2=IMPX
...
*        RESTORE SIGT ARRAY.
         ...
         ENDIF
  230    CONTINUE
  240    SIGT(IRNBM(I),LLL)=SIGT(IRNBM(I),LLL)+SIGRES(I)
      ENDIF
  250 CONTINUE
  260 CONTINUE
      CALL LCMSIX(IPLIB,' ',2)
...
*----
  ...
  290 MASKI(ISO)=(LSHI(ISO).GT.0).AND.LOGDO
      IMPX2=MAX(0,IMPX-1)
      ...
      IF((LSHI(ISO).GT.0).AND.(IBM.GT.0).AND.(DEN(ISO).NE.0.)) THEN
         ...
         DO 300 LLL=1,NGRO
  300    TOTAL(LLL,MIX2(ISO))=TOTAL(LLL,MIX2(ISO))-GAR(LLL)
         DO 310 LLL=1,NGRO
