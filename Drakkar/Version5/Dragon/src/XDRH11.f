...
*----
   ...
   20    CHORD=CHORD+(RS(K+1,J)**IPOW-RS(K,J)**IPOW)*SIG(K)
   ...
   25       DP1=DP1+FRT*VOLK(J,K)*QKDEL(J,K,IBM)*SIG(K)
         ELSE
            DO 40 K=1,NS(J)
            QKD=1.0D0
            DO 30 N=1,NS(J)
   30       QKD=QKD-QKN(K,N)*SIG(N)
            QKDEL(J,K,IBM)=REAL(QKD)
   40       DP1=DP1+FRT*VOLK(J,K)*QKDEL(J,K,IBM)*SIG(K)
         ENDIF
         IF(ITER.EQ.1) THEN
            DO 50 K=1,NS(J)
   50       QKOLD(J,K,IBM)=QKDEL(J,K,IBM)
            IF(CHORD.GE.1.0E4) THEN
               DO 60 K=1,NS(J)
   60          PKL(J,K,K,IBM)=1.0/SIGMA(MIXGR(K,J,IBM))
            ELSE
               DO 70 K=1,NS(J)
               DO 70 N=1,NS(J)
   70          PKL(J,K,N,IBM)=QKN(K,N)
            ENDIF
         ENDIF
         DEALLOCATE(SIG)
   80    CONTINUE
...
*----
      ...
      DO 120 K=1,NS(J)
  120 DP0=DP0+FRT*VOLK(J,K)*QKOLD(J,K,IBM)*SIGMA(MIXGR(K,J,IBM))
  130 CONTINUE
  ...
  170 SIGMS(IR1+IBM)=REAL(DP0)*SIGMA(IR1+IBM)
*----
