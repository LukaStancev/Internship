...
*
*Parameters: input
* MAXTRA  available storage for apollo compacted transfer cross sections
...
*           ELASTIC SLOWING-DOWN MATRIX.
  ...
  183             DAUX=DAUX+PRI(I)*FLOAT(LARG)
               ...
               J=J-1
  184          DAUX=DAUX+PRI(I)*FLOAT(J)
            ELSE
               DO 82 MDEL=MDELB,MDELH
...
*           STANDARD GALOCHE.
  ...
  801       IF(LLL.GT.NEX1) GO TO 803
            ...
            DO 804 I=1,NEX2
            IPR=IPR+1
  804       STR(I)=STR(I)+PRI(IPR)
            LN3=LLL-NEX3+1
            DO 807 I=LN3,LLL
            IPR=IPR+1
  807       STR(I)=STR(I)+PRI(IPR)
            MML=LLL
            GO TO 240
...
*           THERMAL TRANSFER MATRIX.
 ...
 5003       STR(MM)=STR(MM)+PRI(IPR+MM+NGRO-LLL)
            DO 5004 MM=1,NGRO-LLL
 5004       STR(MM+LLL)=STR(MM+LLL)+PRI(IPR+MM)
            MML=LLL-NGROIN
...
*           GREULING-GOERTZEL SLOWING DOWN MATRIX.
            ...
            DO 360 MM=LN1,MML
            J=J+1
  360       STR(MM)=STR(MM)+REAL(PRI(J)*DAUX)
         ENDIF
