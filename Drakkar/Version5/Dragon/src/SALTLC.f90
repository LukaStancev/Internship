...
!
!Parameters: input
! IFTEMP  pointer to a temporary TRACKING file in update or creation mode.
! IPRINT  print level.
...
SUBROUTINE SALTLC(IFTEMP,IPRINT,IGTRK,NDIM,NFREG,NBANGL,RENO,NBDR,IFMT,DENUSR,DANGLT, &
  ...
  DO IANGL=1,NBANGL
     ...
     DO PIECE=1,NPIECE
       ...
       IF(IERR/=-1) THEN
          !             a trajectory has been completed: store
          !             angle order nber, total weight and wr
          ANGLE=DACOS(EX)
          ITRAC2(CNT0+4)=IANGL
          ...
          ! next line
          IF(CNT+NNN+MINLEN>=NMAX2) CALL XABORT('SALTLC: NMAX2 overflow(1)')
          NBTRAC=NBTRAC+1
          !
