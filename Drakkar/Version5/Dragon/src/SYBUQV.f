...
*----
      LGEMPT=.FALSE.
      DO 10 IR=1,NREG
   10 LGEMPT=LGEMPT.OR.(.NOT.LGFULL(IR+NSURF-1))
*----
*  INITIALIZATION
*----
      IZ0=0
...
*        COMPUTE OPTICAL LENGTHS.
         DO 20 IX=1,NH
         IR=ZZI(IZ0+IX)
   20    POPTX(IX)=ZZR(IZR+IX)*SIGT(IR)
...
*        COMPUTE REMAINING PSI FROM LAST REGION TO EAST SIDE.
   ...
   50    PIJ(IIJ)=PIJ(IIJ)+PPLUS*WITRAJ
      ELSE
...
*        COMPUTE LEAKAGE AND TRANSMISSION PROBABILITIES PSI, PSS
         ...
         IIJ=MAX(IRT*(IRT+1)/2+ISE,ISE*(ISE+1)/2+IRT)
   80    PIJ(IIJ)=PIJ(IIJ)+PKI3
*
*        COMPUTE REMAINING PSI FROM LAST REGION TO EAST SIDE.
         IZ0=IZ0+1
         ISE=ZZI(JZ0)
         IIJ=MAX(IRS*(IRS+1)/2+ISE,ISE*(ISE+1)/2+IRS)
   90    PIJ(IIJ)=PIJ(IIJ)+ZI30*WITRAJ
         IIJ=NSURF*(NSURF+1)/2-1
         DO 100 JR=1,NREG
         IIJ=IIJ+NSURF+JR
  100    PIJ(IIJ)=PIJ(IIJ)*2.0
      ENDIF
*----
