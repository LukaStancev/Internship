...
*----
      ...
      DO 50 IU=1,NGROUP
      DO 40 IBM=1,NBMIX
   40 VPHV(IBM)=VPHV(IBM)+VX(IBM)*FLUMIX(IU,IBM)
      DO 50 ICMB=1,NCOMB
      IBM=MILVO(ICMB)
...
*----
  ...
  120 GAR=0.0D0
      DO 130 IU=1,NGROUP
  130 GAR=GAR+DBLE(XSREC(IU,IREAC-1)*FLUMIX(IU,IBM))
...
*           FITD IS THE DECAY POWER IN WATT PER CUBIC CENTIMETER.
     ...
     1         E11.4,12H FIT(INPUT)=,E11.4,1H.)') FITD,FIT
      ...
      IF(INR.GT.0) THEN
         ...
         DO 270 IQ=1,NREAC
         DO 270 IS=1,NVAR+1
  270    SIG(IS,IQ,IBM)=SIG(IS,IQ,IBM)*REAL(FNORM)
      ELSE
         CALL XDRSET(VPHV,NBMIX,0.0)
      ENDIF
*----
