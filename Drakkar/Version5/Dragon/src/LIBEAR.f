...
*        RECOVER A VECTOR CROSS SECTION.
     ...
     1   ICHDKL,IDK,NV)
         ...
         IF(TEXT8.EQ.'SIGA') THEN
            LTEST=.FALSE.
            DO 170 IG=1,NV
  170       LTEST=LTEST.OR.(RTSEGM(IDK+IG-1).NE.0.0)
            IF(LTEST) KPAX(NEL+3,ISO)=1
         ELSE IF(TEXT8.EQ.'NEXCESS') THEN
            LTEST=.FALSE.
            DO 180 IG=1,NV
  180       LTEST=LTEST.OR.(RTSEGM(IDK+IG-1).NE.0.0)
            IF(LTEST) KPAX(NEL+4,ISO)=1
         ELSE IF(TEXT8.EQ.'SIGF') THEN
            LTEST=.FALSE.
            DO 190 IG=1,NV
  190       LTEST=LTEST.OR.(RTSEGM(IDK+IG-1).NE.0.0)
...
*----
     ...
     1   ICHDKL_PTR)
         ...
         IF(TEXT8.EQ.'CREA-P') THEN
            LTEST=.FALSE.
            DO 220 IG=1,NV
  220       LTEST=LTEST.OR.(RTSEGM(IDK+IG-1).NE.0.0)
            IF(LTEST) KPAX(NEL+8,ISO)=1
         ELSE IF(TEXT8.EQ.'CREA-H2') THEN
            LTEST=.FALSE.
            DO 225 IG=1,NV
  225       LTEST=LTEST.OR.(RTSEGM(IDK+IG-1).NE.0.0)
            IF(LTEST) KPAX(NEL+11,ISO)=1
         ELSE IF(TEXT8.EQ.'CREA-H3') THEN
            LTEST=.FALSE.
            DO 230 IG=1,NV
  230       LTEST=LTEST.OR.(RTSEGM(IDK+IG-1).NE.0.0)
