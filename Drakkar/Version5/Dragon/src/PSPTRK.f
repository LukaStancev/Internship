...
* of a 2-D geometry from an \moc{EXCELL} generated
* \dds{tracking} data structure.
*
*Copyright:
...
*
*Update(s):
* 2006/03/15: Add the possibility to process \moc{NXT}
*             processed 2-D geometry.
*
*
*Parameters: input
* IPRINT  print level.
...
*         \begin{itemize}
*         \item ITYPE=0 when the geometry is colored by region;
*         \item ITYPE=1 when the geometry is colored by mixture;
*         \item ITYPE=2 when the geometry is colored by flux
*                       (one group);
*         \item ITYPE=3 when the geometry is colored by flux
*                       (multigroup);
*         \item ITYPE=4 when the geometry is colored by mixture for
*                       homogenization.
*         \end{itemize}
* ICOLR   color set used where:
*         \begin{itemize}
*         \item ICOLR=-4 HSB filling with no contour;
*         \item ICOLR=-3 CYMK filling with no contour;
*         \item ICOLR=-2 RGB filling with no contour;
*         \item ICOLR=-1 BW filling with no contour;
*         \item ICOLR= 0 no filling with contour;
*         \item ICOLR= 1 BW filling with contour;
*         \item ICOLR= 2 RGB filling with contour;
*         \item ICOLR= 3 CMYK filling with contour;
*         \item ICOLR= 4 HSB filling with  contour;
*         \end{itemize}
...
*----
      ...
      ENDIF
*----
*  Get state vector from \dds{tracking}
