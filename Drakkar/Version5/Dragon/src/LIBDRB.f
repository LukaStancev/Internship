...
*        IG2 IS THE SECONDARY GROUP.
   ...
   40       LENGT=LENGT+NJJ(I)
            ...
            IGAR=IGAR+1
   45       SCAT(IG2,IG1,IL+1)=GAR(IGAR)*SIGS(IG1,IL+1)
         ELSE IF(.NOT.LPCAT) THEN
            DO 50 IG1=1,NGRO
   50       SCAT(IG1,IG1,IL+1)=SIGS(IG1,IL+1)
         ENDIF
      ELSE IF(LPCAT) THEN
         DO 70 IG1=1,NGRO
         TMP=0.0D0
         DO 60 IG2=1,NGRO
   60    TMP=TMP+SCAT(IG2,IG1,IL+1)
   70    SIGS(IG1,IL+1)=REAL(TMP)
         LSCAT(IL+1)=.TRUE.
      ENDIF
   80 CONTINUE
      LSCAT(1)=.TRUE.
...
*----
  ...
  390    TOTAL(IG1)=TOTAL(IG1)+TERP(IDIL,IG1)*SIGT(IG1,IDIL)
         IF(KSIGFR.GT.0) THEN
            DO 400 IDEL=0,NDEL
            IF(IDEL.EQ.0) THEN
               HNUSIG='NUSIGF'
  ...
  400       SIGF(IG1,IDEL)=SIGF(IG1,IDEL)+TERP(IDIL,IG1)*GAR(IG1)
         ENDIF
         IF(KCHIR.GT.0) THEN
            DO 410 IDEL=0,NDEL
            IF(IDEL.EQ.0) THEN
  ...
  405       CHI(IG1,IDEL)=CHI(IG1,IDEL)+TERP(IDIL,IG1)*GAR(IG1)
  410       CONTINUE
            DO 416 ISP=1,NBESP
            WRITE(HCHI,'(5HCHI--,I2.2)') ISP
            CALL XDRSET(GAR,KCHIR,0.0)
  ...
  415         CHI4G(IG1,ISP)=CHI4G(IG1,ISP)+TERP(IDIL,IG1)*GAR(IG1)
            ENDIF
  416       CONTINUE
         ENDIF
         DO 450 IL=0,NL-1
  ...
  420       LENGT=LENGT+NJJ(I)
            ...
            IGAR=IGAR+1
  430       SCAT(IG2,IG1,IL+1)=SCAT(IG2,IG1,IL+1)+TERP(IDIL,IG1)
     1      *GAR(IGAR)
         ENDIF
         IF(LPDIL(IL+1)) THEN
  ...
  440       SIGS(IG1,IL+1)=SIGS(IG1,IL+1)+TERP(IDIL,IG1)*GAR(IG1)
         ENDIF
  450    CONTINUE
         IF(KPHIR.GT.0) THEN
  ...
  470       SADD(IG1,IED)=SADD(IG1,IED)+TERP(IDIL,IG1)*GAR(IG1)
         ENDIF
  480    CONTINUE
         CALL LCMSIX(IPDRL,' ',2)
  490    CONTINUE
...
*----
         ...
         IF(LPDIL(IL+1).AND.(.NOT.LSDIL(IL+1))) THEN
            ...
            LENGT=0
            DO 500 I=1,NGRO
  500       LENGT=LENGT+NJJ(I)
            CALL XDRSET(GAR,LENGT,0.0)
            CALL LCMGET(IPDRL,'PCAT'//CM,GAR)
            IGAR=0
            DO 520 IG2=1,NGRO
            DO 510 IG1=1,NGRO
  510       SCAT(IG2,IG1,IL+1)=0.0
            DO 520 IG1=IJJ(IG2),IJJ(IG2)-NJJ(IG2)+1,-1
            IGAR=IGAR+1
  520       SCAT(IG2,IG1,IL+1)=GAR(IGAR)*SIGS(IG1,IL+1)
         ELSE IF((.NOT.LPDIL(IL+1)).AND.LSDIL(IL+1)) THEN
            DO 540 IG1=1,NGRO
            TMP=0.0D0
            DO 530 IG2=1,NGRO
  530       TMP=TMP+SCAT(IG2,IG1,IL+1)
            IF(IL.EQ.0) THEN
...
*           SCATTERING CROSS SECTIONS ARE NOT SELF-SHIELDED.
            TMP=-TOTAL(IG1)
            DO 560 IG2=1,IG1-1
  560       TMP=TMP+SCAT(IG1,IG2,1)*ZNPHI(IG2)*DELTA(IG2)/DELTA(IG1)
...
*           SCATTERING CROSS SECTIONS ARE SELF-SHIELDED.
            ZNGAR=-TOTAL(IG1)
            DO 570 IG2=1,IG1
  570       ZNGAR=ZNGAR+SCAT(IG1,IG2,1)*DELTA(IG2)/DELTA(IG1)
         ENDIF
