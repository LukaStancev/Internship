...
*----
   ...
   10 VOLI=(4.0/3.0)*PI*RAYRE(IP+1)**3
...
*        NO VOIDED REGIONS DETECTED.
   ...
   50    AUXI(ITR,3)=AUXI(ITR,2)*Z(IZ)
         ...
         PIJ(IP,JP)=PIJ(IP,JP)+W*(CIJ2+CIJ3)/(AUXI(IP,2)*AUXI(JP,2))
   60    TAUIJ=TAUIJ+AUXI(JP,3)
         TAU=TAU+2*AUXI(IP,3)
   70    TAU1J=TAU1J+AUXI(IP,3)
      ENDIF
*
      DO 80 I=1,NPIJ
      DO 80 J=I,NPIJ
...
*----
      IF(ALBEDO.NE.0.0) THEN
         ...
         AUXI(IK,3)=1.0
         DO 90 JK=1,NPIJ
   90    AUXI(IK,3)=AUXI(IK,3)-PIJ(IK,JK)*AUXI(JK,2)
  100    PSS=PSS-4.0*AUXI(IK,1)*AUXI(IK,2)*AUXI(IK,3)/SURF
         AUX0=ALBEDO/(1.0-ALBEDO*PSS)
         DO 110 JK=1,NPIJ
         AUX1=AUX0*(4.0*AUXI(JK,1)/SURF)*AUXI(JK,3)
         DO 110 IK=1,NPIJ
  110    PIJ(IK,JK)=PIJ(IK,JK)+AUXI(IK,3)*AUX1
      ENDIF
*----
