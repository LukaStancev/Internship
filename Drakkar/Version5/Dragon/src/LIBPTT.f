...
*----
   ...
   20    SIGP(1,J)=XSDIL(1,J+1)
     ...
     1 (XSDIL(IDIL,1)-XSDIL(IDIL,3)))-1.0D0)
      PREC1=MAX(PREC1,ERR)
   30 LCONV=LCONV.OR.(ABS(XSDIL(IDIL,1)).EQ.ABS(XSDIL(IDIL+1,1)))
...
*----
   ...
   45 ALLOCATE(WABS(NDIL+1))
      DO 60 IDIL=1,NDIL+1
   60 WABS(IDIL)=REAL(XSDIL(IDIL,1)-(1.0D0-XGOLD)*XSDIL(IDIL,3))
...
*----
      ...
      IF(XSDIL(NDIL+1,IPART+1).EQ.0.0) THEN
         DO 250 I=1,NOR
  250    SIGP(I,IPART)=0.0
         GO TO 300
      ENDIF
      DO 260 IDIL=1,NDIL
      TOFIT(IDIL,1)=DILUT(IDIL)
  260 TOFIT(IDIL,2)=XSDIL(IDIL,IPART+1)/SQRT(SDDK(IDIL))
      CALL ALDFIT(NDIL,NOR-1,TOFIT(1,1),TOFIT(1,2),SDDK,DA)
      IF(DA(NOR-1).EQ.0.0) THEN
         DO 265 I=1,NOR
  265    SIGP(I,IPART)=XSDIL(NDIL+1,IPART+1)
         GO TO 300
      ENDIF
      DO 270 I=0,NOR-1
  270 DA(I)=DA(I)*XSDIL(NDIL+1,IPART+1)/DA(NOR-1)
*----
...
*----
  ...
  330 INOR=INOR+1
      ...
      IF(ABS(WEIGH(INOR)).LE.5.0E-7) THEN
         DO 350 JNOR=INOR+1,NOR
         WEIGH(JNOR-1)=WEIGH(JNOR)
         SIGX(JNOR-1)=SIGX(JNOR)
         DO 350 J=1,NPART
  350    SIGP(JNOR-1,J)=SIGP(JNOR,J)
         INOR=INOR-1
         NOR=NOR-1
      ENDIF
      GO TO 330
...
*----
  ...
  390 CC=CC+WEIGH(I)
      DO 400 I=1,NOR
  400 WEIGH(I)=WEIGH(I)/REAL(CC)
      CC=0.0D0
      DO 410 I=1,NOR
  410 CC=CC+WEIGH(I)*SIGX(I)
      IF(CC.NE.0.0) THEN
         DO 420 I=1,NOR
  420    SIGX(I)=SIGX(I)*XSDIL(NDIL+1,1)/REAL(CC)
      ENDIF
      DO 450 J=1,NPART
      CC=0.0D0
      DO 430 I=1,NOR
  430 CC=CC+WEIGH(I)*SIGP(I,J)
      IF(CC.NE.0.0) THEN
         DO 440 I=1,NOR
  440    SIGP(I,J)=SIGP(I,J)*XSDIL(NDIL+1,J+1)/REAL(CC)
      ENDIF
  450 CONTINUE
*----
...
*----
  ...
  460    EE=EE+WEIGH(I)*(SIGX(I)-SIGP(I,2))/DENOM
...
*
  ...
  490    TEST(J)=0.0D0
         ...
         DO 500 J=1,NPART
  500    TEST(J+2)=TEST(J+2)+WEIGH(INOR)*SIGP(INOR,J)
