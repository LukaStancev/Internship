...
260      CONTINUE
      ENDIF
*
270   GG3(IBLK)=FRX*FRY*FRZ
      DO 280 I=1,NCOUR*NMBLK
280   IFR(I)=MIX(IFR(I))
*----
...
310   POURCE(IKK)=0.0
      DO 330 IBLK=1,NMBLK
      IKK=INUM(IBLK)
330   POURCE(IKK)=POURCE(IKK)+GG3(IBLK)
*----
...
*        DP-1 APPROXIMATION IN 2-D.
         ...
         JSURF=(I1-1)*3+JCOUR
         DVX(JSURF)=FR1
470      MIX(JSURF)=JND+JCOUR
480      DVX(JSURF)=ZSIGN*FR1
         DO 500 I1=IJAS,1,-1
         ...
         ALB(JSURF)=FR1
         SUR(JSURF)=FR2
490      IFR(JSURF)=JND+JCOUR
500      ALB(JSURF)=ZSIGN*FR1
         NCOUR=12
...
530         DIRZ(I)=DIRR(1+(I-1)/3)
         ELSE IF ((MULTC.EQ.4).AND.(NCOUR.EQ.4)) THEN
            DO 540 I=1,4
            ISMZ(I)=2*ISM(1+(I-1)/2,I1)+MOD(I-1,2)-1
540         DIRZ(I)=DIRR(1+(I-1)/2)
         ELSE
            DO 550 I=1,NCOUR
            ISMZ(I)=ISM(I,I1)
550         DIRZ(I)=DIRR(I)
