...
*----
   ...
   30    QKD=QKD-QKN(K,N)*SIG(N)
         ...
         FACT=FRT*VOLK(J,K)*QKDEL(J,K,IBM)*SIG(K)
         DP0=DP0+FACT
   40    DP1=DP1+FACT*RAPP
      ENDIF
      IF(CHORD.GE.1.0E4) THEN
         DO 50 K=1,NS(J)
   50    PKL(J,K,K,IBM)=1.0/SIGMA(MIXGR(K,J,IBM))
      ELSE
         DO 60 K=1,NS(J)
         DO 60 N=1,NS(J)
   60    PKL(J,K,N,IBM)=QKN(K,N)
      ENDIF
      DEALLOCATE(SIG)
   70 CONTINUE
      SIGMA(IR1+IBM)=REAL(DP1)
   80 RRRR(IBM)=REAL(DP1/DP0)
*----
...
*----
      ...
      DO 120 K=1,NS(J)
  120 DP0=DP0+FRT*VOLK(J,K)*QKDEL(J,K,IBM)*SIGMA(MIXGR(K,J,IBM))
  130 CONTINUE
  ...
  150 RHS(IND2+K)=RRRR(IBM)*QKDEL(J,K,IBM)
      IND2=IND2+NS(J)
  160 CONTINUE
