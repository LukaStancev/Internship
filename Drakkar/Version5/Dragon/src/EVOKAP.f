...
*
   ...
   10 TEMP(NSUPL+I)=TEMP(NSUPL+I)+YSF(J,I,2)*Y(KFISS(J))
      DO 20 I=1,N
   ...
   20 DYSAV(I)=DYDX(I)
      ...
      DO 30 I=1,NSUPF
      DO 30 J=1,NFISS
   30 DFDX(NSUPL+I)=DFDX(NSUPL+I)+YSF(J,I,2)*YSAV(KFISS(J))
      DO 50 I=1,IMA(N)
   50 ASS(I)=-H*GAM*(ADPL(I,1)+XSAV*ADPL(I,2))
      DO 60 I=1,N
      DFDX(I)=DFDX(I)+BDPL(I,2)
   60 ASS(MU1(I))=1.+ASS(MU1(I))
      CALL ALLUF(N,ASS,MU1,IMA)
      DO 70 I=1,N
   70 AK(I,1)=H*DYSAV(I)+HSQ*GAM*DFDX(I)
      CALL ALLUS(NSUPL,MU1,IMA,ASS,AK(1,1))
      IF(NSUPF.GT.0) THEN
         DO 75 I=1,NSUPF
         DO 75 J=1,NFISS
   75    AK(NSUPL+I,1)=AK(NSUPL+I,1)+H*GAM*(YSF(J,I,1)+XSAV*YSF(J,I,2))
     1   *AK(KFISS(J),1)
         CALL ALLUS(NSUPF,MU1(NSUPL+1),IMA(NSUPL+1),ASS,AK(NSUPL+1,1))
      ENDIF
      DO 80 I=1,N
   80 Y(I)=YSAV(I)+ALF21*AK(I,1)
      X=XSAV+ALF21*H
      CALL ALLUM(N,ADPL(1,1),Y,DYDX,MU1,IMA)
      CALL ALLUM(N,ADPL(1,2),Y,TEMP,MU1,IMA)
      DO 90 I=1,NSUPF
      DO 90 J=1,NFISS
      DYDX(NSUPL+I)=DYDX(NSUPL+I)+YSF(J,I,1)*Y(KFISS(J))
   90 TEMP(NSUPL+I)=TEMP(NSUPL+I)+YSF(J,I,2)*Y(KFISS(J))
      DO 100 I=1,N
      DYDX(I)=DYDX(I)+X*TEMP(I)+BDPL(I,1)+X*BDPL(I,2)
  100 AK(I,2)=H*DYDX(I)+HSQ*GAM2X*DFDX(I)+GAM21*AK(I,1)
      CALL ALLUS(NSUPL,MU1,IMA,ASS,AK(1,2))
      IF(NSUPF.GT.0) THEN
         DO 105 I=1,NSUPF
         DO 105 J=1,NFISS
  105    AK(NSUPL+I,2)=AK(NSUPL+I,2)+H*GAM*(YSF(J,I,1)+XSAV*YSF(J,I,2))
     1   *AK(KFISS(J),2)
         CALL ALLUS(NSUPF,MU1(NSUPL+1),IMA(NSUPL+1),ASS,AK(NSUPL+1,2))
      ENDIF
      DO 110 I=1,N
      AK(I,2)=AK(I,2)-GAM21*AK(I,1)
  110 Y(I)=YSAV(I)+ALF31*AK(I,1)+ALF32*AK(I,2)
      ...
      DO 120 J=1,NFISS
      DYDX(NSUPL+I)=DYDX(NSUPL+I)+YSF(J,I,1)*Y(KFISS(J))
  120 TEMP(NSUPL+I)=TEMP(NSUPL+I)+YSF(J,I,2)*Y(KFISS(J))
      DO 130 I=1,N
      DYDX(I)=DYDX(I)+X*TEMP(I)+BDPL(I,1)+X*BDPL(I,2)
      TEMP(I)=GAM31*AK(I,1)+GAM32*AK(I,2)
  130 AK(I,3)=H*DYDX(I)+GAM3X*HSQ*DFDX(I)+TEMP(I)
      CALL ALLUS(NSUPL,MU1,IMA,ASS,AK(1,3))
      IF(NSUPF.GT.0) THEN
         DO 135 I=1,NSUPF
         DO 135 J=1,NFISS
  135    AK(NSUPL+I,3)=AK(NSUPL+I,3)+H*GAM*(YSF(J,I,1)+XSAV*YSF(J,I,2))
     1   *AK(KFISS(J),3)
  ...
  140 AK(I,4)=H*DYDX(I)+HSQ*GAM4X*DFDX(I)+TEMP(I)
      CALL ALLUS(NSUPL,MU1,IMA,ASS,AK(1,4))
      IF(NSUPF.GT.0) THEN
         DO 145 I=1,NSUPF
         DO 145 J=1,NFISS
  145    AK(NSUPL+I,4)=AK(NSUPL+I,4)+H*GAM*(YSF(J,I,1)+XSAV*YSF(J,I,2))
     1   *AK(KFISS(J),4)
  ...
  150 TEMP(I)=YSAV(I)+CC1*AK(I,1)+CC2*AK(I,2)+CC3*AK(I,3)
      X=XSAV+H
      IF (X.EQ.XSAV) THEN
     ...
     1   6H HTRY=,E11.4,2H).)') H,HTRY
      ...
      ERRMAX=0.
      DO 160 I=1,N
  160 ERRMAX=MAX(ERRMAX,ABS((Y(I)-TEMP(I))/YSCAL(I)))
      ERRMAX=ERRMAX/EPS
      IF (ERRMAX.EQ.0.) THEN
         HDID=H
         HNEXT=GROW*H
