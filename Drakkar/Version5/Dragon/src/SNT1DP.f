...
*         (i=1: X-  i=2: X+):
*         NCODE(I)=1: VOID;   NCODE(I)=2: REFL;   NCODE(I)=7: ZERO.
...
* LL4     order of the matrices per group /OR/ number of unknowns being
*         solved for.
* NUN     number of unknowns stored in the FLUX vector per group -- 
*         may include calculated/averaged unknowns or boundary angular 
*         fluxes.
*
*-----------------------------------------------------------------------
*
...
*----
      ...
      IF(NSCT.GT.1) THEN
         PL(2,M)=U(M)
         DO 140 L=2,NSCT-1
  140    PL(L+1,M)=((2.0*L-1.0)*U(M)*PL(L,M)-(L-1)*PL(L-1,M))/L
      ENDIF
  145 CONTINUE
*----
