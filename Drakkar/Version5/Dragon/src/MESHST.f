...
C---------------------------- MESHST -----------------------------------
C
C       FUNCTION : - NUMEROTATION GLOBALE DE L'ASSEMBLAGE AVEC PRISE EN
C                  COMPTE DES REGROUPEMENTS DES CELLULES ET DE LEURES
C                  ORIENTATION (MERGE ET TURN)
C                  - AFFECTATION DES ALBEDOS AUX SURFACES EXTERIEURES
C                  - CALCUL DES VOLUMES DE TOUTES LES REGIONS ET DES
C                  SURFACES DES FACES EXTERNES
C                  - COORDONNEES DES HEXAGONES ET DES CYLINDRES SUR
C                  TOUTE LA GEOMETRIE
C       CREATION : JUNE 16,1991
C       REVISED  :
C       AUTHOR   : M.OUISLOUMEN
C
C INPUT PARAMETERS
C
C  MCODE     :  =1 SI SYMETRIE Z-,=2 SI SYMETRIE Z+, =0 SI NON
C
C OUTPUT PARAMETERS
C  REMESH    : COORDONNEES DES GEOMETRIES (VOIR TRKHEX)
C  FVOL(I)   : NUMERO DE LA PREMIERE ZONE DE LA CELLULE I
C  NVOL(I)   : NUMERO DU PREMIER VOLUME DE LA CELLULE I AVEC SECTEURS
C  LVOL(I)   : NUMERO DU VOLUME OU DEVRAIT DEBUTER LA NUMEROTATION
C  STAIRS    : VOIR TRKHEX
C  FACST     : VOIR TRKHEX
C  NCEL      : NRBE DE CELLULES DE L'ASSEMBLAGE
C  LSENS(I)  : =1, SI LA NUMEROTATION EST DANS LE SENS NORMALE,=2,SINON
C  NSECT(I)  : 6*(NSECT(I)-1)= NUMBER OF SECTORS IN REGION I
C  NCYL(I)   : NBRE DE CYLINDRE DANS LA CELLULE I
C  NCPHY     : NBRE DE CELLULE PHYSIQUE
C  VOLSUR    : VOLUMES ET SUFACES EXTERNES
C  MATALB    : NUMERO DES ALBEDOS POUR DIFFERENTES SURFACES ET MIX POUR
C              DIFFERENTS VOLUMES
C  FACB(I)   : DEBUT DE NUMEROTATION DES FACES SUP OU INF POUR LA
C              CELLULE PERIPHERIQUE I
C  SURB(I)   : DEBUT DE NUMEROTATION DE LA CELLULE I
C  VSYM(I)   : CONTIENT LES NUMEROS DES VOLUMES DE LA SYMETRIE D'ENTREE
C  SSYM(I)   : CONTIENT LES NUMEROS DES SURFACES DE LA SYMETRIE D'ENTREE
C
C--------------------------- MESHST ------------------------------------
C
      USE         GANLIB
...
C
C              COORDONNEES DES CENTRES DES HEXAGONES
C
      X=0.
      Y=0.
      XM=0.
   ...
   30 CONTINUE
C
C         POUR EVITER DES EFFETS DE DIFFERENCE
C
      NCOUS=7
   ...
   31 CONTINUE
      ...
      IF(LG3D) THEN
        ...
        CALL LCMSIX(IPTRK,' ',2)
C
C--- TEST SUR L'ORDONANCE DES PLANS DES CELLULES SUIVANT L'AXE Z
C
        ZAUX=ZZZ(1)
        ZMIN=ZAUX
        DO 29 IAZ=1,ILENZ-1
   ...
   35   CONTINUE
      ENDIF
C
C           CALCUL DES SURFACES EXTERNES ET AFFECTATION DES ALBEDOS
C
      ISURFP=6
  ...
  117      CONTINUE
           ...
           DO 120 J=1,NRAY+1
             ...
             IF(LG3D) THEN
                VOLUMS=VOLUME*.25
C
C     SURFACES SUPERIEURES ET INFERIEURES (SELON L'AXE Z)
C
                IF(IPLANZ.GT.1) THEN
                  IF(I.LE.STAIRS(1)) THEN
                    KSURF1=KSURF1+1
  ...
  716   CONTINUE
        GO TO 800
      ENDIF
C         CAS D'UN ASSEMBLAGE DE CELLULE
C
      IPP1=0
      MROT=0
      MSMAX=0
      ...
      IF(LGCELL) THEN
C
C               SPLITING DES RAYONS ET DEFINITION DES SECTEURS POUR
C               LES CELLULES GENERATRICES
C
  ...
  109           CONTINUE
   ...
   10    CONTINUE
C--- NUMEROTATION DES VOLUMES DE LA SYMETRIE D'ENTREE
         IXZPL=IPLANZ
  ...
  582      CONTINUE
           ENDIF
  182      CONTINUE
C--- RECHERCHE DU NOMBRE DE SURFACES QUE PRESENTE LA SYMETRIE
           ICCOIN=-1
           JCCOIN=-1
           KCCOIN=-1
           MROT=6
  ...
  500      CONTINUE
   ...
   42       CONTINUE
            ...
            IF(MCYL.GT.0) THEN
C
C               STORAGE DES COORDONNEES DES CYLINDRES
C        ==> ATTENTION:  LES AXES DES CYLINDRES SONT SELON Z
C
              DO 43 J=1,MCYL
                 IAUX=IAUX+1
...
C
   ...
   45       CONTINUE
            ...
            IF(KSECTX.GT.3) THEN
C
C TRAITEMENT DU VOLUME A BORDURE HEXAGONALE DANS LE CAS OU KSECT>3
C
             KSECT=KSECTX
...
C
   ...
   65        CONTINUE
             ELSE
C
C  TRAITEMENT DU CAS OU NSECTO>3
C
             KSECT=NSECTO
             MIXX=IBB(NCEL+I)
             NAUX=6*(KSECT-1)
 ...
 555         CONTINUE
  ...
  200    CONTINUE
      ...
      IF(LG3D) DEALLOCATE(ZZZ)
C
C--- CONSTRUCTION DU VECTEUR VSYM QUI CONTIENT POUR CHAQUE VOLUME DE
C--- DE L'ASSEMBLAGE SON EQUIVALENT DANS LA SYMETRIE D'ENTREE
C
  ...
  300   CONTINUE
        DEALLOCATE(IAA)
C--- CONSTRUCTION DU VECTEUR SSYM QUI CONTIENT POUR CHAQUE SURFACE
C--- DE L'ASSEMBLAGE SON EQUIVALENT DANS LA SYMETRIE D'ENTREE
        ISY=0
        MSAUX=NS
        ISYX=NSMIN+1
...
C--- CAS DE LA SYMETRIE Z- (MCODE=1) SSYM EST REMPLIT PLUS LOIN
          IF(MCODE.NE.1) THEN
          IF(IHEX.NE.9) THEN
          DO 301 ISUR=1,NSMIN
             SSYM(ISUR)=VSYM(ISUR)
  ...
  600   CONTINUE
C
C---SURFACE LATERALES SUPERIEURES DANS LE CAS 3D
C
        IF(LG3D) THEN
        IYAUX=0
        IF(MCODE.LE.1) THEN
