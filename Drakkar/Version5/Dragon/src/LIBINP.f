...
*  LOCAL PARAMETERS
*----
      PARAMETER (IOUT=6,NHOBL=12,MAXPAR=5,NSTATE=40)
      TYPE(C_PTR) JPLIB
...
*----
     ...
     > IHLIB(2,MAXISO,3),ILLIB(MAXISO),IEVOL(MAXISO),ITYP(MAXISO))
...
*        THE LIBRARY IS UPDATED. READ OLD LIBRARY INFORMATION.
         ...
         TEXT4=' '
         READ(TEXT4,'(A4)') IVOID
         CALL XDISET(IHLIB,6*MAXISO,IVOID)
...
*        USE THE VALUES ALREADY THERE.
   ...
   90 CALL REDGET(INDIC,NITMA,FLOTT,TEXT12,DBLINP)
     ...
     >         //'T SET.')
            ...
            ENDIF
         ELSE
            CALL XABORT('LIBINP: REAL OR CHARACTER DATA EXPECTED(9).')
         ENDIF
         CALL REDGET(INDIC,NITMA,FLOTT,TEXT12,DBLINP)
         IF(INDIC.EQ.1) THEN
...
*----
      ...
      DEALLOCATE(ITYP,IEVOL,ILLIB,IHLIB,NIR,LSHI,NTFG,ISHINA,ISOMIX,
     > ISONRF,ISONAM)
      RETURN
*
  ...
  305 FORMAT(
     ...
     2 7H ISOADD,I6,37H   (0=COMPLETE BURNUP CHAIN/1=DO NOT)/
     3 7H MAXISM,I6,40H   (MAX. NUMBER OF ISOTOPES PER MIXTURE)/
     4 7H IPRECI,I6,34H   (CALENDF ACCURACY FLAG:1/2/3/4))
