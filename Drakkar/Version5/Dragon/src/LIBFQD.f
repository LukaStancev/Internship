...
*----
   ...
   30 SIGAF(IBIN+IGF)=SIGAF(IBIN+IGF)*REAL(SIGAIN/SIGAI2)
   40 IBIN=IBIN+NFS(IGRP)
*----
...
*----
   ...
   50 ERR=MAX(ERR,ABS(DELTA(IBIN+IGF)/DELI-REAL(LARGH)))
   60 IBIN=IBIN+NFS(IGRP)
...
*
     ...
     1   A12,2H''.)') IGRP,HNAMIS
      ...
      DO 110 IDIL=1,MDIL
      JDIL=IPERD(IDIL)
  110 XSREF(IDIL)=TOTAL(IGRP,JDIL)-SIGS(IGRP,1,JDIL)
*----
...
*----
     ...
     1   MML,STIS)
         ...
         DO 120 J=2,MML
         JGF=IBIN+IGF-J+1
  120    PHI(IBIN+IGF,IDIL,1)=PHI(IBIN+IGF,IDIL,1)+DBLE(STIS(J))*
...
*
  ...
  140    PHI(IBIN+IGF,IDIL,IL)=PHI(IBIN+IGF,IDIL,IL-1)/BONDAR
*
*        COMPUTE THE FINE-GROUP SLOWING-DOWN SOURCE.  
         DO 150 IGF=1,NFS(IGRP)
         CALL LIBECT(MAXTRA,IBIN+IGF,PRI(1,1),UUU,DELI,DELTA,NEXT+2,1,
     1   MML,STIS)
         DO 150 J=1,MML
         JGF=IBIN+IGF-J+1
  ...
  150    CONTINUE
         DO 160 IDIL=1,MDIL
         DO 160 IGF=1,NFS(IGRP)
         RSTAR(IBIN+IGF,IDIL)=RSTAR(IBIN+IGF,IDIL)*FACT(IDIL)
         DO 160 IL=1,NL
  160    PHI(IBIN+IGF,IDIL,IL)=PHI(IBIN+IGF,IDIL,IL)*FACT(IDIL)
      ENDIF
*----
*  TEST FINE FLUX.
*----
  ...
  230    T2=T2+RSTAR(IBIN+IGF,IDIL)*DELM
         JDIL=IPERD(IDIL)
...
*----
  ...
  260 NPAR=1
      IF(LPHYS) THEN
         NPART=3+NL+NED+NDEL
         DO 270 IL=1,NL
  270    NPART=NPART+MAX(ISMAX(IL,IGRP)-ISMIN(IL,IGRP)+1,0)
  ...
  280    PRTABS(JNOR)=PRTSIG(JNOR,2)-PRTSIG(JNOR,4)
         GO TO 780
...
*----
  ...
  290 INOR=INOR+1
      ...
      IF(ABS(PRTSIG(INOR,1)).LT.1.0E-10) THEN
         DO 300 JNOR=INOR+1,NOR(IGRP)
         DO 300 J=1,NPAR+2
  300    PRTSIG(JNOR-1,J)=PRTSIG(JNOR,J)
         INOR=INOR-1
         NOR(IGRP)=NOR(IGRP)-1
      ENDIF
      GO TO 290
*
  ...
  330    TEST(3)=TEST(3)+SIGAF(IBIN+I)*DELTA(IBIN+I)
         DO 340 J=2,3
  340    TEST(J)=TEST(J)/TEST(1)
         TEST(1)=1.0
...
*        COMPUTE THE BASE POINTS OF THE RESONANT FLUX.
  ...
  390    PHIMT(INOR)=PHIMT(INOR)/DELMAC
...
*----
  ...
  430    PHIMT(INOR)=PHIMT(INOR)/DELMAC
         CALL LIBMPA(NOR(IGRP),JINI,PRTSIG(1,1),PRTSIG(1,2),PHIMT,
     1   PRTRS(1,IDIL))
  440    CONTINUE
      ENDIF
...
*----
  ...
  450 T0=T0+PRTSIG(INOR,1)*PRTSIG(INOR,2)*PRTPH(INOR,IDIL,1)
      JDIL=IPERD(IDIL)
      FACTOR=FLUX(IGRP,JDIL)*TOTAL(IGRP,JDIL)/REAL(T0)
      DO 460 IL=1,NL
      DO 460 INOR=1,NOR(IGRP)
  460 PRTPH(INOR,IDIL,IL)=PRTPH(INOR,IDIL,IL)*FACTOR
*
      IF(IMPX.GT.9) THEN
  ...
  480       TEST(3)=TEST(3)+REAL(PHI(IBIN+IGF,IDIL,1))*SIGTF(IBIN+IGF)*
     1      DELM
            TEST(3)=TEST(3)/TEST(2)
            TEST(2)=TEST(2)/TEST(1)
            TEST(1)=1.0
...
*----
      ...
      DO 560 IL=1,NL
      IF(LSCAT(IL)) THEN
         DO 500 INOR=1,NOR(IGRP)
         WORK(MDIL+1,INOR)=1.0D0
         DO 500 IDIL=1,MDIL
  500    WORK(IDIL,INOR)=PRTPH(INOR,IDIL,1)
         CALL ALST2F(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT)
         DO 510 IDIL=1,MDIL+1
         JDIL=IPERD(IDIL)
  510    BB(IDIL)=SIGS(IGRP,IL,JDIL)*FLUX(IGRP,JDIL)
         CALL ALST2S(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT,BB,CC)
         DO 520 INOR=1,NOR(IGRP)
  520    PRTSIG(INOR,IOF1)=REAL(CC(INOR))/PRTSIG(INOR,1)
         DO 550 JGRP=ISMIN(IL,IGRP),ISMAX(IL,IGRP)
         DO 530 IDIL=1,MDIL+1
         JDIL=IPERD(IDIL)
  530    BB(IDIL)=SCAT(JGRP,IGRP,IL,JDIL)*FLUX(IGRP,JDIL)
         CALL ALST2S(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT,BB,CC)
         DO 540 INOR=1,NOR(IGRP)
  540    PRTSIG(INOR,IOF2)=REAL(CC(INOR))/PRTSIG(INOR,1)
  550    IOF2=IOF2+1
      ENDIF
  560 IOF1=IOF1+1
*----
...
*----
      DO 570 INOR=1,NOR(IGRP)
      WORK(MDIL+1,INOR)=1.0D0
      DO 570 IDIL=1,MDIL
  570 WORK(IDIL,INOR)=PRTPH(INOR,IDIL,1)
      CALL ALST2F(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT)
*----
*  USE A ROOT MEAN SQUARE TECHNIQUE TO FIND BASE POINTS OF THE
...
*----
      ...
      JDIL=IPERD(IDIL)
  580 BB(IDIL)=(TOTAL(IGRP,JDIL)-SIGS(IGRP,1,JDIL))*FLUX(IGRP,JDIL)
      CALL ALST2S(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT,BB,CC)
      DO 590 INOR=1,NOR(IGRP)
  590 PRTABS(INOR)=REAL(CC(INOR))/PRTSIG(INOR,1)
*----
...
*----
      IF(LSIGF) THEN
         ...
         JDIL=IPERD(IDIL)
  600    BB(IDIL)=SIGF(IGRP,JDIL)*FLUX(IGRP,JDIL)
         CALL ALST2S(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT,BB,CC)
         DO 610 INOR=1,NOR(IGRP)
  610    PRTSIG(INOR,3)=REAL(CC(INOR))/PRTSIG(INOR,1)
      ENDIF
*----
*  USE A ROOT MEAN SQUARE TECHNIQUE TO FIND BASE POINTS OF THE
...
*----
      ...
      IF(LADD(IED)) THEN
         ...
         JDIL=IPERD(IDIL)
  620    BB(IDIL)=SADD(IGRP,IED,JDIL)*FLUX(IGRP,JDIL)
         CALL ALST2S(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT,BB,CC)
         DO 630 INOR=1,NOR(IGRP)
  630    PRTSIG(INOR,IOF2)=REAL(CC(INOR))/PRTSIG(INOR,1)
      ENDIF
  640 IOF2=IOF2+1
*----
...
*----
      IF(LSIGF) THEN
        ...
        JDIL=IPERD(IDIL)
  650   BB(IDIL)=ZDEL(IGRP,IDEL,JDIL)*FLUX(IGRP,JDIL)
        CALL ALST2S(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT,BB,CC)
        DO 660 INOR=1,NOR(IGRP)
  660   PRTSIG(INOR,IOF2)=REAL(CC(INOR))/PRTSIG(INOR,1)
  670   IOF2=IOF2+1
      ENDIF
*----
...
*----
      IF(LPHYS.OR.LCALEN.OR.LPTMC) THEN
        DO 680 INOR=1,NOR(IGRP)
        PRTSIW(INOR)=PRTSIG(INOR,4)
        DO 680 JNOR=1,NOR(IGRP)
  680   WSLD((INOR-1)*NOR(IGRP)+JNOR)=PRTSIG(INOR,1)*PRTSIG(JNOR,1)
      ELSE
        DO 700 INOR=1,NOR(IGRP)
        DO 690 IDIL=1,MDIL+1
  690   BB(IDIL)=PRTRS(INOR,IDIL)
        CALL ALST2S(NDIL+1,MDIL+1,NOR(IGRP),WORK,PHIMT,BB,CC)
        DO 700 I=1,NOR(IGRP)
  700   WSLD((I-1)*NOR(IGRP)+INOR)=REAL(CC(I))*PRTSIG(INOR,1)
*
        DO 730 J=1,NOR(IGRP)
        T0=0.0D0
        DO 710 I=1,NOR(IGRP)
  710   T0=T0+WSLD((J-1)*NOR(IGRP)+I)
        DO 720 I=1,NOR(IGRP)
  720   WSLD((J-1)*NOR(IGRP)+I)=
     1                   REAL(WSLD((J-1)*NOR(IGRP)+I)*(PRTSIG(J,1)/T0))
  730   PRTSIW(J)=REAL(T0)/PRTSIG(J,1)
      ENDIF
      IDOMAX=0
      EROLD1=1.0E10
      EROLD2=1.0E10
...
*----
      ...
      DO 740 J=1,NOR(IGRP)
  740 MATRIX(I,J)=-WSLD((J-1)*NOR(IGRP)+I)*PRTSIW(J)
  750 MATRIX(I,I)=MATRIX(I,I)+(PRTSIG(I,2)+DILUT2(IDIL))*PRTSIG(I,1)
...
*----
  ...
  760 ACCUM4=ACCUM4+PRTSIG(I,1)*PRTPH(I,IDIL,1)
...
*        SAVE THE PROBABILITY TABLE INTO IPLIB.
  ...
  800    NPART=NPART+MAX(0,(ISMAX(IL,IGRP)-ISMIN(IL,IGRP)+1))
