...
*----      
   ...
   30       YIELD(IGR,1,1)=YIELD(IGR,1,1)+WORK(IGR)
         ENDIF
         ...
         IF((LENGTH.GT.0).AND.(LENGTH.EQ.NDFI)) THEN
            ...
            DO 40 I=1,NDFI
   40       PYIELD(I,1,1)=PYIELD(I,1,1)+WPY(I)
         ENDIF
*
*        SET ARRAY HMAKE.
...
*
   ...
   90             GAS(IGR,J)=GAS(IGR,J)+WORK(IGR)*DDEN
  ...
  100             GAS(IGR,J)=GAS(IGR,J)+WORK(IGR)*DDEN
               ELSE IF(HMAKE(J)(:3).EQ.'NWT') THEN
                  DO 110 IGR=1,NGCOND
  110             GAS(IGR,J)=WORK(IGR)
               ELSE IF((HMAKE(J).EQ.'CHI').OR.
     1                 (HMAKE(J)(:5).EQ.'CHI--')) THEN
                  DO 120 IGR=1,NGCOND
  120             GAS(IGR,J)=GAS(IGR,J)+WORK(IGR)*PNFIRA(IGR,0,1)
               ELSE IF(HMAKE(J)(:3).EQ.'CHI') THEN
                  IDEL=J-IOF1H-1
                  DO 130 IGR=1,NGCOND
  130             GAS(IGR,J)=GAS(IGR,J)+WORK(IGR)*PNFIRA(IGR,IDEL,1)
               ELSE
                  DO 140 IGR=1,NGCOND
  140             GAS(IGR,J)=GAS(IGR,J)+WORK(IGR)*DDEN
               ENDIF
            ENDIF
         ENDIF
  150    CONTINUE
     ...
     1      ITYPRO)
            ...
            DO 160 JGR=1,NGCOND
            DO 160 IGR=1,NGCOND
  160       WSCAT(IGR,JGR,IL)=WSCAT(IGR,JGR,IL)+WORK2(IGR,JGR,IL)*DDEN
         ENDIF
      ENDIF
  170 CONTINUE
  ...
  175    LYIEL=LYIEL.OR.(YIELD(IGR,1,1).GT.0.0)
  ...
  180        CONTINUE
         ...
         IF(HMAKE(J).EQ.'OVERV') THEN
            DO 185 IGR=1,NGCOND
  185       GAS(IGR,J)=GAS(IGR,J)/DENTOT(INM)
