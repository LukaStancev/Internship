...
C
      CALL XDRSET(JTAB,NISO,0)
      CALL XDRSET(TOTAL,NGRP*NBURN,0.0)
      CALL XDRSET(ZNUG,NGRP*NBURN,0.0)
      CALL XDRSET(DIFFX,NGRP*NBURN,0.0)
      CALL XDRSET(DIFFY,NGRP*NBURN,0.0)
      CALL XDRSET(DIFFZ,NGRP*NBURN,0.0)
      CALL XDRSET(H,NGRP*NBURN,0.0)
      CALL XDRSET(TMREF,NGRP*NBURN*NISO,0.0)
      CALL XDRSET(DMRFX,NGRP*NBURN*NISO,0.0)
      CALL XDRSET(DMRFY,NGRP*NBURN*NISO,0.0)
      CALL XDRSET(DMRFZ,NGRP*NBURN*NISO,0.0)
      CALL XDRSET(FMREF,NGRP*NBURN*NISO,0.0)
      CALL XDRSET(HMREF,NGRP*NBURN*NISO,0.0)
      CALL XDRSET(SCAT,NGRP*NGRP*NL*NBURN,0.0)
      CALL XDRSET(SMREF,NGRP*NGRP*NL*NBURN*NISO,0.0)
      DO 10 IGR=1,NGRP
      IJJ(IGR)=IGR
      NJJ(IGR)=1
      MIJ(IGR)=IGR
...
C
  ...
  140    SCAT(I,IL,IGR,JGR)=SCAT(I,IL,IGR,JGR)+DENSIT(ISO)*WORK3(IGAR)
      ENDIF
C
          CALL LCMSIX(IPLIST,' ',2)
       ENDIF
...
C
 ...
 899     CONTINUE
         ...
         IGARM=IGARM+1
  795   WORK3(IGARM)=SCAT(I,IL,IGR,JGR)
  799   CONTINUE
