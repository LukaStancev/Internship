...
*----
   ...
   30       CONTINUE
      ...
      ELSE IF(TYPSEG.EQ.'PMAIL') THEN
         ...
         DO 40 IG=1,NV
   40    ENERG(IG)=RTSEGM(IDK+IG-1)*1.0E6
         CALL AEXGNV(5,ITSEGM,ICHDIM,ICHTYP,ICHDKL,IDK,NV)
         IF(NV.EQ.NGRO) THEN
            DO 50 IG=1,NGRO
   50       DELTA(IG)=RTSEGM(IDK+IG-1)
         ELSE
            DO 60 IG=1,NGRO
   60       DELTA(IG)=LOG(ENERG(IG)/ENERG(IG+1))
         ENDIF
      ...
      ELSE IF(TYPSEG.EQ.'PCONST') THEN
         ...
         DO 70 IA=1,NAMASS
   70    AMASS(IA)=RTSEGM(IDK+IA-1)/ANEUT
...
*           RECOVER A VECTOR CROSS SECTION.
     ...
     1         SECT)
               ...
               ELSE IF(TEXT8.EQ.'NEXCESS') THEN
                  LN2N=.FALSE.
                  DO 170 IG=1,NGRO
  170             LN2N=LN2N.OR.(SECT(IG).NE.0.0)
                  IF(LN2N) THEN
  ...
  180             SIGS(IG,1)=SIGS(IG,1)+SECT(IG)
...
*           RECOVER A MATRIX CROSS SECTION.
     ...
     2         SCAT)
               ...
               IF(LGPROB) THEN
                  DO 200 IG=1,NGRO
                  DO 200 JG=1,NGRO
  200             SCAT(JG,IG,1)=SCAT(JG,IG,1)*SIGS(IG,1)
               ENDIF
...
*----
     ...
     2      SCAT)
            ...
            IF(LGPROB) THEN
               DO 280 IG=1,NGRO
               DO 280 JG=1,NGRO
  280          SCAT(JG,IG,1)=SCAT(JG,IG,1)*SIGS(IG,1)
            ENDIF
...
*----
     ...
     1      ICHDKL_PTR)
            ...
            CALL AEXGNV(1,RTSEGM,ICHDIM,ICHTYP,ICHDKL,IDK,ILENG)
            CALL LCMDRD(ICHDIM_PTR)
            CALL LCMDRD(ICHTYP_PTR)
            CALL LCMDRD(ICHDKL_PTR)
  ...
  350       CONTINUE
            ...
            IF(LPWD.AND.LPED) THEN
               ...
               IF(IPR(4,JMX).EQ.KISEG) THEN
                  ...
                  DO 370 IGR=1,NGRO
  370             SECT(IGR)=SECT(IGR)*PWD(IDEL)*PED(IGR)
...
*----
  ...
  490          SIGAF(IOF+I)=SIGAF(IOF+I)*F3
  500          IOF=IOF+NFS(IGG)
               DEALLOCATE(SQRTE)
*----
...
*----
  ...
  510             SIGAF(IBIN)=SIGAF(JBIN)*F3/F1
               ELSE
                  E0=ENERG(FGRESO)
               ENDIF
*
