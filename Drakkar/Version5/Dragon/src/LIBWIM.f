...
*----
      ...
      CALL REDIND(IUNIT,GENINX,LGENIN,GAR(1,1),NPZ(3),5)
*----
...
*----
 ...
 130      CONTINUE
        ENDIF
        CALL REDIND(IUNIT,SUBINX,LSUBIN,XSREC(1,8),NGF,13)
        NF=SUBINX(LSUBIN+5)
        IF(NF.GT.1) THEN
          CALL REDIND(IUNIT,SUBINX,LSUBIN,XSREC(1,3),NGFR,10)
          CALL REDIND(IUNIT,SUBINX,LSUBIN,XSREC(1,4),NGFR,12)
          DO 135 IG=1,NGROUP
            XSREC(IG,5)=GAR(IG,1)
 135      CONTINUE
        ENDIF
...
*----
          CALL REDIND(IUNIT,SUBINX,LSUBIN,XSREC(1,2),NGFR,5)
          CALL REDIND(IUNIT,SUBINX,LSUBIN,XSREC(1,7),NGFR,5)
        ELSE
*----
*  COMPUTE TOTAL XS FOR IENDF=0,1
*----
          CALL REDIND(IUNIT,SUBINX,LSUBIN,XSREC(1,2),NGFR,4)
          CALL REDIND(IUNIT,SUBINX,LSUBIN,XSREC(1,7),NGFR,6)
        ENDIF
*----
*  THERMAL XS
*----
...
*----
            DO 310 JRES=1,NRES
              IF(IDRES.EQ.0) THEN
                XRS1=FLOAT(INT((RS1(1,JRES)+0.01)*10.)
     >              -INT(RS1(1,JRES)+0.01)*10)/10.+0.02
                XRS1=ABS(RS1(1,JRES)-XRS1-RIND)
              ELSE
                XRS1=ABS(RS1(1,JRES)-RIND)
              ENDIF
              IF(XRS1.LE.0.01) THEN
                KRES=JRES
                NTMPR=INT(RS1(2,KRES)+0.1)
                NDILR=INT(RS1(3,KRES)+0.1)
