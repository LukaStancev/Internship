...
* VOL     volumes of the macro geometry.
...
*----
  ...
  100 IR=MAX(IR,MAT(I))
      ...
      DO IND=1,IR2-IR
        IF(IDIL(IND).EQ.0) CALL XABORT('READBH: INVALID MIXDIL VALUE.')
      ENDDO
      DO 130 J=1,NG
      DO 110 IND=1,NMILG
  110 FTEMP(IND)=FRACT(J,IND)
      DO 120 IND=1,IR2
  120 FRACT(J,IND)=0.0
      DO 130 IND=1,NMILG
  130 FRACT(J,MILIEU(IND))=FTEMP(IND)
*
      IF(IMPX.GE.1) THEN
         WRITE (6,300) GEONAM
         IF(MICRO.EQ.3) THEN
