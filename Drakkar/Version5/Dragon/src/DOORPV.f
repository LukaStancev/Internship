...
*----
        ...
        FACT=1.0/VOL(I)
        DO 10 J=1,NREG
   10   PREG(I,J)=PGAR(INDPOS(I,J))*FACT
...
*----
  110 ALLOCATE(SGAR(NB1*NGRP),SGAS(NB1*NANI*NGRP))
      ...
      ENDIF
  120 CONTINUE
      ALLOCATE(PGAR(IPIJK*NELPIJ*NGRP))
...
*       --- ALLG VECTORIZATION
        ...
        ENDIF
        CALL EXCELP(IPTRK,IFTRAK,IMPX,NREG,NBMIX,NANI,MAT,VOL,KNORM,
     >  SGAR,SGAS,NELPIJ,IPIJK,PGAR,LEAKSW,NGRP,NPSYS,NPST,PROBKS,TITR,
     >  NALBP,ALBP)
...
*----
     ...
     >    SGAS((IGR-1)*NB1*NANI+2),PGAR(KPIJSY+1),PIS)
          IF(LEAKSW) CALL LCMPUT(KPSYS,'DRAGON-WIS',NREG,2,PIS)
          DEALLOCATE(PIS)
        ENDIF
*----
...
*----
        ...
        FACT=1.0/VOL(I)
        DO 130 J=1,NREG
  130   PREG(I,J)=PGAR(KPIJSY+INDPOS(I,J))*FACT
        CALL LCMPPD(KPSYS,'DRAGON-PCSCT',NREG*NREG,2,PREG_PTR)
        IF(IPIJK.EQ.4) THEN
          DO 150 IJKS=1,3
          PREG_PTR=LCMARA(NREG*NREG)
          ...
          DO 140 J=1,NREG
          KS=NELPIJ*IJKS+INDPOS(I,J)
  140     PREG(I,J)=PGAR(KPIJSY+KS)*FACT
          CALL LCMPPD(KPSYS,'DRAGON'//CMS(IJKS)//'PCSCT',NREG*NREG,2,
     >    PREG_PTR)
  150     CONTINUE
        ENDIF
