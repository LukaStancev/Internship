...
* IPIFI   isotope index associated to each fissile isotope of microlib.
...
*----
   ...
   40    CONTINUE
         CALL LCMSIX(IPLIB,' ',2)
   50    WSTRD(IGRCND)=REAL((ZFL1/(3.0*ZNU))*ZFL2/ZDEN)
      ENDIF
*
      VOLMER=0.0D0
...
*
  ...
  185       CONTINUE
            ...
            ENDIF
            DO 186 IGR=1,NGROUP
  186       GAR(IGR,5+NED+NL+3*NW)=0.0
            IF(LMEVF) THEN
               CALL LCMGET(KPLIB,'NFTOT',WORK)
               HMAKE(5+NED+NL+3*NW)='H-FACTOR'
               DO 190 IGR=1,NGROUP
  190          GAR(IGR,5+NED+NL+3*NW)=GAR(IGR,5+NED+NL+3*NW)+WORK(IGR)*
     1         EMEVF(ISO)*REAL(CONV)
            ...
            IF(LMEVG) THEN
               ...
               HMAKE(5+NED+NL+3*NW)='H-FACTOR'
               DO 195 IGR=1,NGROUP
  195          GAR(IGR,5+NED+NL+3*NW)=GAR(IGR,5+NED+NL+3*NW)+WORK(IGR)*
  ...
  205          GAR(IGR,6+NED+NL+3*NW)=1.0/(REAL(SQFMAS)*SQRT(ENEAVG))
               DEALLOCATE(ENR)
            ENDIF
            HMAKE(6+NED+NL+3*NW)='OVERV'
*
            IGRFIN=0
            DO 240 IGRCND=1,NGCOND
            IGRDEB=IGRFIN+1
            IGRFIN=IGCOND(IGRCND)
...
*              IGRCND IS THE SECONDARY GROUP.
     ...
     >           FLUXES(IREGIO,JGR,IW)*DENVOL
               ENDIF
  220          IPO=IPO+1
  230          WSCAT(IGRCND,JGRCND,1+IL)=WSCAT(IGRCND,JGRCND,1+IL)+TMP
            ENDIF
  240       CONTINUE
            MASK(JSO)=.TRUE.
            GO TO 250
...
*
  ...
  390       CONTINUE
            ...
            DO 400 IG1=1,NGCOND
            IOF2=IOF2+1
  400       GA2(IOF2)=REAL(WSCAT(IG1,IG2,IL))
...
*
  ...
  460   MAXISM=MAX(MAXISM,MAX0)
        ...
        IF(JJISO.GT.0) THEN
           ...
           DO 500 IISO=1,JJISO
           DO 480 I0=1,3
  480      IHNISO(I0,IISO)=ISONRF(I0,ISTYP(IISO))
  ...
  520   ENR(IGRCND+1)=ENR(IGCOND(IGRCND)+1)
  ...
  530   ENR(IGRCND)=LOG(ENR(IGRCND)/ENR(IGRCND+1))
