...
* REFKEF  target K-effective fot type B or type L calculations.
* IMPX    print flag.
* PHI     homogeneous flux from heterogeneous calculation.
*
*Parameters: output
...
*
   ...
   15    GAMMA(I)=REAL(B1GAMA(IAPROX,B2,ST2))
         RETURN
...
*----
   ...
   60    BSTOC(K,J)=0.0D0
         ...
         IGAR=IGAR+1
   70    BSTOC(I,J)=BSTOC(I,J)+3.0D0*SCAT1(IGAR)*CSTOC(K)
         IGAR=0
         DO 90 I=1,NGRO
         ST2=ST(I)
         DD=D(I)
  ...
  130 CAET=CAET+SA(I)*ASTOC(I,NGRO+1)
      IF(IMPX.GT.1) WRITE (6,430) ITEX,ALAM1,B2
      B2ITX(ITEX)=B2
      FFITX(ITEX)=REFKEF-ALAM1
      IF(TYPE.EQ.'K') THEN
         DO 140 I=1,NGRO
  140    PHI(I)=ASTOC(I,NGRO+1)/ALAM1
...
*        CHECK THE CONVERGENCE.
  ...
  150    PHI(I)=ASTOC(I,NGRO+1)/ALAM1
...
*----
  ...
  170 ITEX=ITEX+1
      ...
      DD=D(I)
      DO 180 J=1,NGRO
  180 BSTOC(I,J)=XHI(I)*SFNU(J)/REFKEF
      DO 190 J=IJJ0(I),IJJ0(I)-NJJ0(I)+1,-1
      IGAR=IGAR+1
  190 BSTOC(I,J)=BSTOC(I,J)+SCAT0(IGAR)
      DO 200 J=1,NGRO
  200 ASTOC(I,J)=BSTOC(I,J)
      IF((OPTION.EQ.'P1').OR.(OPTION.EQ.'B1')) THEN
         DO 220 J=1,NGRO
         DO 210 K=1,NGRO
         CSTOC(K)=BSTOC(K,J)
  210    BSTOC(K,J)=0.0D0
         ...
         IGAR=IGAR+1
  220    BSTOC(I,J)=BSTOC(I,J)+3.0D0*SCAT1(IGAR)*CSTOC(K)
         IGAR=0
         DO 240 I=1,NGRO
         ST2=ST(I)
         DD=D(I)
  ...
  230    BSTOC(I,J)=ZXC
         ...
         IGAR=IGAR+1
  240    ASTOC(I,J)=ASTOC(I,J)+3.0D0*BETA*SCAT1(IGAR)*ST(J)
      ENDIF
      DO 250 I=1,NGRO
      ST2=ST(I)
      DD=D(I)
...
*----
  ...
  310    ASTOC(I,J)=0.0D0
         ...
         IGAR=IGAR+1
  320    ASTOC(I,J)=ASTOC(I,J)-3.0D0*BETA*SCAT1(IGAR)*PHI(J)/PHI(I)
         CALL B1SOL(NGRO,ASTOC,IER)
         IF(IER.NE.0) CALL XABORT('B1DIF: SINGULAR MATRIX(2).')
         DO 330 I=1,NGRO
  330    D(I)=REAL(ASTOC(I,NGRO+1))
      ELSE IF(OPTION.NE.'LKRD') THEN
         DO 340 I=1,NGRO
         ST2=ST(I)
         DD=D(I)
         BETA=B1BETA(IAPROX,B2,ST2,DD)
  340    D(I)=REAL(BETA*ST2/(1.0D0-B2*BETA))
      ENDIF
      A2=0.0D0
      CAET=0.0D0
      ZXC=0.0D0
      ...
      ST2=DBLE(ST(I))
  350 GAMMA(I)=REAL(B1GAMA(IAPROX,B2,ST2))
      A2=A2/CAET
      CAET=ZXC/CAET
      IF(CAET.EQ.0.0) THEN
        ALAM2=0.0
