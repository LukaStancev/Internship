...
*         of each isotope. A value of 1.0e10 is used for infinite
*         dilution.
...
*         surface).
* TITR    title.
* START   beginning-of-iteration flag (START=.true. if SHISN3 is called
*         for the first time).
...
* BIEFF   Livolant-Jeanpierre normalization flag (BIEFF=.true. to
*         activate).
* LGC     Goldstein-Cohen approximation flag (LGC=.true. to activate).
*
*Parameters: output
* SN      computed microscopic dilution cross section in each energy
...
*----
      ...
      LOGDO=.FALSE.
      DO 10 I=1,NREG
   10 LOGDO=LOGDO.OR.(MAT(I).EQ.IBM)
...
*
      ...
      LOGDO=.FALSE.
      DO 70 IRS=1,NBNRS
   70 LOGDO=LOGDO.OR.NOCONV(MIX(ISONR(IRS)),LLL)
      IF(LOGDO) THEN
         NPSYS(LLL)=LLL
*
...
*        CROSS SECTIONS IN EACH RESONANT MIXTURE.
         ...
         ISO=ISONR(IRS)
         IBM=MIX(ISO)
   80    SIGT(IBM,LLL)=SIGT(IBM,LLL)-TOTAL(LLL,IRS)*DEN(ISO)
         DO 90 IBM=1,NBM
         SIG0(IBM,LLL)=0.0
   90    SIG3(IBM,LLL)=SIGT3(IBM,LLL)
         DO 100 IRS=1,NBNRS
         ISO=ISONR(IRS)
         SIG0(MIX(ISO),LLL)=TOTAL(LLL,IRS)*DEN(ISO)
  100    SIG3(MIX(ISO),LLL)=0.0
         IF(IMPX.GE.10) THEN
...
*        USE A VERY CHEAP APPROXIMATION TO START ITERATIONS.
         ...
         SIGX(IALP)=0.0
  140    DILUT(IALP,LLL)=SIGE(LLL)
      ELSE
         AVDEN=0.0
         VOLTOT=0.0
         DO 150 IRS=1,NBNRS
         AVDEN=AVDEN+DEN(ISONR(IRS))*VST(IRS)
  150    VOLTOT=VOLTOT+VST(IRS)
         AVDEN=AVDEN/VOLTOT
         DO 160 IRS=1,NBNRS
         ISO=ISONR(IRS)
  160    DIST(IRS)=DEN(ISO)/AVDEN
         ...
         IF(NPSYS(LLL).NE.0) THEN
            ...
            DO 170 IBM=1,NBM
            SIG0(IBM,LLL)=0.0
  170       SIG3(IBM,LLL)=SIGT3(IBM,LLL)
            DO 180 IRS=1,NBNRS
            ISO=ISONR(IRS)
            SIG0(MIX(ISO),LLL)=SIGX(IALP)*DIST(IRS)
  180       SIG3(MIX(ISO),LLL)=0.0
         ENDIF
  190    CONTINUE
...
*----
  ...
  230    SIGX(IALP)=FACT(IALP)*SIGE(LLL)
*        **********************************************************
...
*        COMPUTE THE PIC BASE POINTS FOR A N-TERM RATIONAL APPROXIMATION
  ...
  240       XDENO(I,IRS,LLL)=DENOM(I,LLL)
         ELSE
         ...
         IF(.NOT.START.AND.BIEFF) THEN
            ...
            IF(NOCONV(MIX(ISO),LLL)) THEN
               ...
               IF(NBNRS.EQ.1) THEN
                  SUM=0.0
                  DO 245 I=1,NRAT
  245             SUM=SUM+COEF(I,LLL)/(SIGRES+DENOM(I,LLL))
                  PX0=REAL(SUM)
               ELSE
                  PX0=SB(LLL,ISO)
               ENDIF
     ...
     1   (LSHI(JSO).NE.0)) THEN
            ...
            IF(LENGT.EQ.NGRO) THEN
               ...
               IF((NOCONV(MIX(JSO),LLL)).AND.(GAR(LLL).NE.1.0)) THEN
                  ...
                  DO 270 I=1,NRAT
  270             XDENO(I,IRS,LLL)=XDENO(I,IRS,LLL)-DDD
               ENDIF
  280          CONTINUE
            ENDIF
         ENDIF
...
*
      ...
      LOGDO=.FALSE.
      DO 300 IRS=1,NBNRS
  300 LOGDO=LOGDO.OR.NOCONV(MIX(ISONR(IRS)),LLL)
      IF(LOGDO) THEN
         DO 310 IRS=1,NBNRS
         ISO=ISONR(IRS)
         IBM=MIX(ISO)
     ...
     1      SN(LLL,ISO),0.97,LLL
         ...
         ENDIF
  310    SIGT(IBM,LLL)=SIGT(IBM,LLL)+TOTAL(LLL,IRS)*DEN(ISO)
      ENDIF
  320 CONTINUE
*----
...
*----
  ...
  330 MASKI(ISO)=(LSHI(ISO).EQ.INRS).AND.LOGDO
      IMPX2=MAX(0,IMPX-1)
  ...
  340 SIGT(IBM,LLL)=SIGT(IBM,LLL)-DEN(ISO)*TOTAL(LLL,IRS)
*----
