...
*         optional read-only type(L_FLUX) => unperturbed solution;
*         optional read-only type(L_GPT)  => GPT sources.
...
*----
      ...
      TYPE(C_PTR) IPFLUX,IPTRK,IPMACR,IPSYS,IPFLUP,IPGPT
...
*----
     ...
     1  THEN
          IPFLUP=KENTRY(I1)
          HPFLUP=HENTRY(I1)
        ELSE IF((HSIGN.EQ.'L_GPT').AND.(.NOT.C_ASSOCIATED(IPGPT))) THEN
          IPGPT=KENTRY(I1)
          HPGPT=HENTRY(I1)
        ELSE
          GO TO 10
...
*----
     ...
     1  //'FLUX OBJECT AT RHS.')
        IF(.NOT.C_ASSOCIATED(IPGPT)) CALL XABORT('FLU: NO GPT SOURCE O'
     1  //'BJECT AT RHS.')
      ENDIF
*----
...
*       FIXED SOURCE EIGENVALUE PROBLEM
        CALL FLUGPT(IPRINT,IPFLUX,IPTRK,IPMACR,IPFLUP,IPGPT,IFTRAK,
     ...
     4  NSOUT,MATCOD,KEYFLX,VOL,REFKEF)
      ELSE
        CALL FLUDRV(IPRINT,IPFLUX,IPTRK,IPMACR,IFTRAK,IPSYS,IPHASE,
     1  ITPIJ,CXDOOR,ITRANC,TITLE,B2,INITFL,LFORW,LEAKSW,IREBAL,NGRP,
     2  MAXMIX,NIFIS,NANIS,NLF,NLIN,NFUNL,COPTIO,NUN,MAXINR,EPSINR,
