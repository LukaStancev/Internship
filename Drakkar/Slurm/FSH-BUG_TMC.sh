...
do
  ...
  do
    ...
    cp ../data/Fessenheim-Bugey.x2m    Fessenheim-Bugey_${iso}_${irand}.x2m
    cp ../data/Fessenheim-Bugey.access Fessenheim-Bugey_${iso}_${irand}.access
    cp ../data/Fessenheim-Bugey.save   Fessenheim-Bugey_${iso}_${irand}.save
    # Change the isotope evaluation from JEFF-3.3 best-estimate to a randomly sampled one
    sed -i 's/'$iso'.*/'$iso' '$irand' ;/' Fessenheim-Bugey_${iso}_${irand}.x2m
    # Writing Slurm instruction file
    echo "#!/bin/bash"                                                 > ${iso}_${irand}_Fessenheim-Bugey.sh
    echo "#SBATCH -c1  ## Number of cores to be reserved"             >> ${iso}_${irand}_Fessenheim-Bugey.sh
    echo "#SBATCH -t 0-00:30:00"                                      >> ${iso}_${irand}_Fessenheim-Bugey.sh
    echo "#SBATCH --mem-per-cpu=4G"                                   >> ${iso}_${irand}_Fessenheim-Bugey.sh
    echo "srun ../runVersion5.sh Fessenheim-Bugey_${iso}_${irand}.x2m ${intg}" >> ${iso}_${irand}_Fessenheim-Bugey.sh
    # Executing the instruction file
    chmod 755 ${iso}_${irand}_Fessenheim-Bugey.sh
    sbatch ${iso}_${irand}_Fessenheim-Bugey.sh
  done
done
